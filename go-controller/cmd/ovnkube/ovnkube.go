	...
	if runMode.ovnkubeController {
		...
		go func() {
			...
			if err = libovsdbutil.WaitUntilNorthdSyncOnce(ctx, libovsdbOvnNBClient, libovsdbOvnSBClient); err != nil {
				klog.Errorf("Failed waiting for northd to sync OVN Northbound DB to Southbound: %w", err)
			}
			...
			if err = os.WriteFile(sbDBHotFileName, []byte(time.Now().String()), 0o644); err != nil {
				klog.Errorf("Failed to write ovnkube controller sb db hot file: %w", err)
			}
			defer func() {
