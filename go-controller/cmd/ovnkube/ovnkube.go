	...
	if config.OvnKubeNode.Mode != types.NodeModeDPUHost && config.Metrics.OVNMetricsBindAddress != "" {
		...
		if ovsClient != nil {
			...
			go func() {
				...
				metricsServer, err := metrics.StartOVNMetricsServer(opts, ovsClient, ovnClientset.KubeClient, ctx.Done(), wg)
				if err != nil {
					ovsCLIErr = fmt.Errorf("failed to start OVN Metrics Server: %w", err)
					cancel()
				}
