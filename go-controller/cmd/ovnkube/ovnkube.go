...
func runOvnKube(ctx context.Context, runMode *ovnkubeRunMode, ovnClientset *util.OVNClientset, eventRecorder record.EventRecorder) (err error) {
	...
	if runMode.ovnkubeController {
		...
		go func() {
			...
			setOVNAuthManuallyFn := func() {}
			if config.OVNKubernetesFeature.EnableInterconnect {
				ovnDBAuthTimeout := time.Duration(config.Default.OVNAuthLimit) * time.Second
				setSBDBAuthManuallyFn := config.OvnSouth.SetOVNAuthAfterLimit(ctx, cancel, ovnDBAuthTimeout)
				setNBDBAuthManuallyFn := config.OvnNorth.SetOVNAuthAfterLimit(ctx, cancel, ovnDBAuthTimeout)
				setOVNAuthManuallyFn = func() {
					setSBDBAuthManuallyFn()
					setNBDBAuthManuallyFn()
					klog.Infof("OVN database auth written to ovn-remote")
				}
			}
			...
			// wait for changes to flow to SB DB
			if err = waitUntilNorthdSyncOnce(ctx, libovsdbOvnNBClient, libovsdbOvnSBClient); err != nil {
				controllerErr = fmt.Errorf("failed to wait for northd to sync from NB to SB DB: %w", err)
			}
			// For IC, allow ovn-controller to connect to OVN post sync
			setOVNAuthManuallyFn()
			...
			if config.OVNKubernetesFeature.EnableInterconnect {
				for _, dbAuth := range []config.OvnAuthConfig{config.OvnSouth, config.OvnNorth} {
					if err := dbAuth.ClearOVNRemote(); err != nil {
						klog.Errorf("Failed clearing ovn DB auth (%q): %v", dbAuth.Address, err)
					}
				}
			}
		}()
