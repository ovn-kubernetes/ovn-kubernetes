...
func TestNewMetricServerRunAndShutdown(t *testing.T) {
	...
	server, err := NewMetricServer(opts, ovsDBClient, kubeClient)
	if err != nil {
	...
	registry := server.GetOVNRegistry()
	if registry == nil {
		t.Fatal("GetOVNRegistry should return non-nil registry")
	}

	// Test that we can set a new registry
	newRegistry := prometheus.NewRegistry()
	server.SetOVNRegistry(newRegistry)
	if server.GetOVNRegistry() != newRegistry {
		t.Fatal("SetOVNRegistry should update the registry")
	}
...
var (
	doctlShowOutput = `system@ovs-system:
	...
	testCases := []metricsTestCase{
		{
			...
			mockRunCommands: []ovntest.TestifyMockHelper{
				...
				// dpctl/show
				{
					...
					RetArgList:       []interface{}{bytes.NewBuffer([]byte(doctlShowOutput)), bytes.NewBuffer([]byte("")), nil},
				},
				// memory/show
				{
					OnCallMethodName: "RunCmd",
