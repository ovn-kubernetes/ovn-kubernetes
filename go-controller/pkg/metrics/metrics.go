...
func StartOVNMetricsServer(opts MetricServerOptions,
	...
	stopChan <-chan struct{}, wg *sync.WaitGroup) (*MetricServer, error) {

	klog.Infof("Create OVN Metrics Server on address: %s", opts.BindAddress)
	metricsServer, err := NewMetricServer(opts, ovsClient, kubeClient)
	if err != nil {
		return nil, fmt.Errorf("failed to create OVN Metrics Server: %v", err)
	}
	...
	go func() {
		...
		if err := metricsServer.Run(stopChan); err != nil {
			klog.Errorf("Failed to run OVN Metrics Server: %v", err)
		}
	}()
