...
// getOVNSBZone retrieves the OVN Southbound DB zone name from the SB_Global options.
// It returns the configured zone name, or types.OvnDefaultZone if the `options:name` key is not present; an error is returned if the ovn-sbctl command fails.
