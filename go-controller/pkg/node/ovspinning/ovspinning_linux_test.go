...
import (
	...
	"github.com/ovn-org/ovn-kubernetes/go-controller/pkg/node/mocks"
)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			go func() {
				...
				defer wg.Done()
				Run(context.Background(), stopCh, mocks.NewPodResourcesAPIClient(tc.allocatableCPUs, tc.usedCPUs))
			}()
