...
func TestAlignCPUAffinity(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			allocatableCPUs: []int64{0, 1, 2, 3},
			reservedCPUs:    []int{4, 5},
			usedCPUs:        [][]int64{{1, 2}},
		},
		{
			name:            "All allocatable used",
			allocatableCPUs: []int64{0, 1, 2},
			reservedCPUs:    []int{3, 4, 5},
			usedCPUs:        [][]int64{{0}, {1}, {2}},
		},
		{
			name:            "No used CPUs",
			allocatableCPUs: []int64{0, 1, 2, 3, 4},
			reservedCPUs:    []int{5, 6, 7, 8, 9},
			usedCPUs:        [][]int64{},
		},
		{
			name:            "Partial usage with multiple containers",
			allocatableCPUs: []int64{0, 1, 2, 3, 4, 5},
			reservedCPUs:    []int{6, 7, 8, 9},
			usedCPUs:        [][]int64{{0}, {3, 4}},
		},
		{
...
func assertPIDHasSchedAffinity(t *testing.T, pid int, expectedCPUSet unix.CPUSet) {
	...
	for _, task := range tasks {
		...
		assert.Equal(t, expectedCPUSet, actual,
			"task[%d] of process[%d] Expected CPUSet %#x != Actual CPUSet %#x", task, pid, expectedCPUSet, actual)
	}
}
