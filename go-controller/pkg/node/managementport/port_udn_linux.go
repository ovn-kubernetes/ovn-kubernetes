...
func NewUDNManagementPortController(
	...
	if (config.OvnKubeNode.Mode != types.NodeModeFull || config.OvnKubeNode.MgmtPortDPResourceName != "") && mpdev == nil {
		return nil, fmt.Errorf("management port resource not allocated for network %s", cfg.GetNetworkName())
	}
	...
	case types.NodeModeFull:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get management port representor name for device %s network %s",
				mpdev.DeviceId, netInfo.GetNetworkName())
		}
	...
	case types.NodeModeDPU:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get management port representor for pfID %v vfID %v network %s",
				mpdev.PfId, mpdev.FuncId, netInfo.GetNetworkName())
		}
		c.ports[representorPort] = newUDNManagementPortRep(cfg, repDeviceName)
...
func syncUDNManagementPort(cfg *udnManagementPortConfig, mgmtIfName string, mpdev *util.NetworkDeviceDetails) error {
	...
	if config.OvnKubeNode.MgmtPortDPResourceName == "" && config.OvnKubeNode.Mode == types.NodeModeFull {
		...
		if ovsRepIfName != "" {
			...
			err = DeleteManagementPortRepInterface(cfg.GetNetworkName(), ovsRepIfName, "")
			if err != nil {
...
func (mp *udnManagementPortRep) delete() error {
	return DeleteManagementPortRepInterface(mp.GetNetworkName(), mp.repDevice, "")
}
