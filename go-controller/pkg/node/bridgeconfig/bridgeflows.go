...
func (b *BridgeConfiguration) flowsForDefaultBridge(extraIPs []net.IP) ([]string, error) {
	...
	if config.IPv4Mode {
		...
		if ofPortPhys != "" {
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=205, in_port=%s, dl_dst=%s, udp, udp_dst=%d, "+
					"actions=output:%s", nodeutil.DefaultOpenFlowCookie, ofPortPhys, bridgeMacAddress, config.Default.EncapPort,
					ofPortHost))
			// perform NORMAL action otherwise.
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=200, in_port=%s, udp, udp_dst=%d, "+
					"actions=NORMAL", nodeutil.DefaultOpenFlowCookie, ofPortPhys, config.Default.EncapPort))

			// table0, Geneve packets coming from LOCAL/Host OFPort. Skip conntrack and go directly to external
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=200, in_port=%s, udp, udp_dst=%d, "+
					"actions=output:%s", nodeutil.DefaultOpenFlowCookie, ofPortHost, config.Default.EncapPort, ofPortPhys))
		}
		...
		for _, netConfig := range b.patchedNetConfigs() {
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=500, in_port=%s, ip, ip_dst=%s, ip_src=%s,"+
					...
					nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, config.Gateway.MasqueradeIPs.V4HostMasqueradeIP.String(), physicalIP.IP,
			...
			for _, netConfig := range b.patchedNetConfigs() {
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=500, in_port=%s, ip, ip_dst=%s, ip_src=%s,"+
						...
						nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, ip.String(), physicalIP.IP,
		...
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=500, in_port=%s, ip, ip_dst=%s,"+
				...
				nodeutil.DefaultOpenFlowCookie, ofPortHost, config.Gateway.MasqueradeIPs.V4OVNMasqueradeIP.String(), config.Default.OVNMasqConntrackZone))
	}
	if config.IPv6Mode {
		if ofPortPhys != "" {
			// table0, Geneve packets coming from external. Skip conntrack and go directly to host
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=200, in_port=%s, udp6, udp_dst=%d, "+
					"actions=output:%s", nodeutil.DefaultOpenFlowCookie, nodeutil.OvsLocalPort, config.Default.EncapPort, ofPortPhys))
		}
		...
		for _, netConfig := range b.patchedNetConfigs() {
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=500, in_port=%s, ipv6, ipv6_dst=%s, ipv6_src=%s,"+
					"actions=ct(commit,zone=%d,nat(dst=%s),table=4)",
					nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, config.Gateway.MasqueradeIPs.V6HostMasqueradeIP.String(), physicalIP.IP,
					config.Default.HostMasqConntrackZone, physicalIP.IP))
		}
		...
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=500, in_port=%s, ipv6, ipv6_dst=%s,"+
				"actions=ct(zone=%d,nat,table=5)",
				nodeutil.DefaultOpenFlowCookie, ofPortHost, config.Gateway.MasqueradeIPs.V6OVNMasqueradeIP.String(), config.Default.OVNMasqConntrackZone))
	}
	...
	for _, svcCIDR := range config.Kubernetes.ServiceCIDRs {
		...
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=500, in_port=%s, %s, %s_dst=%s, "+
				...
				nodeutil.DefaultOpenFlowCookie, ofPortHost, protoPrefix, protoPrefix,
			...
			if util.IsRouteAdvertisementsEnabled() {
				...
				for _, netConfig := range b.patchedNetConfigs() {
					...
					if netConfig.Advertised.Load() {
						...
						matchingIPFamilySubnet, err := util.MatchFirstIPNetFamily(net2.IsIPv6CIDR(svcCIDR), udnAdvertisedSubnets)
						if err != nil {
							klog.Infof("Unable to determine UDN subnet for the provided family isIPV6: %t, %v", net2.IsIPv6CIDR(svcCIDR), err)
							continue
						}
		...
		for _, netConfig := range b.patchedNetConfigs() {
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=500, in_port=%s, %s, %s_src=%s, %s_dst=%s,"+
					...
					nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, protoPrefix, protoPrefix, svcCIDR,
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=115, in_port=%s, %s, %s_dst=%s,"+
					"actions=drop", nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, protoPrefix, protoPrefix, svcCIDR))
		}
	}
	...
	if ofPortPhys != "" {
			...
			if config.IPv4Mode {
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=100, table=1, ip, ct_state=+trk+est, ct_mark=%s, "+
						"actions=%s", nodeutil.DefaultOpenFlowCookie, netConfig.MasqCTMark, actions))
		...
		if config.IPv4Mode {
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=100, table=1, %s ip, ct_state=+trk+est, ct_mark=%s, "+
					"actions=%soutput:%s",
					nodeutil.DefaultOpenFlowCookie, match_vlan, ctMarkHost, strip_vlan, ofPortHost))
		...
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=10, table=1, %s dl_dst=%s, actions=%soutput:%s",
				nodeutil.DefaultOpenFlowCookie, match_vlan, bridgeMacAddress, strip_vlan, ofPortHost))
	}
	...
	dftFlows = append(dftFlows,
		fmt.Sprintf("cookie=%s, priority=100, table=2, "+
			"actions=set_field:%s->eth_dst,%soutput:%s", nodeutil.DefaultOpenFlowCookie,
	...
	if config.IPv4Mode {
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, table=4,ip,"+
				...
				nodeutil.DefaultOpenFlowCookie, config.Default.OVNMasqConntrackZone, config.Gateway.MasqueradeIPs.V4OVNMasqueradeIP.String()))
	}
	if config.IPv6Mode {
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, table=4,ipv6, "+
				"actions=ct(commit,zone=%d,nat(src=%s),table=3)",
				nodeutil.DefaultOpenFlowCookie, config.Default.OVNMasqConntrackZone, config.Gateway.MasqueradeIPs.V6OVNMasqueradeIP.String()))
	}
	...
	if config.IPv4Mode {
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, table=5, ip, "+
				...
				nodeutil.DefaultOpenFlowCookie, config.Default.HostMasqConntrackZone))
	}
	if config.IPv6Mode {
		dftFlows = append(dftFlows,
	...
	if config.IPv4Mode {
		flows = append(flows,
			fmt.Sprintf("cookie=%s, priority=110, table=0, in_port=%s, ip, nw_frag=yes, actions=ct(table=0,zone=%d)",
				nodeutil.DefaultOpenFlowCookie,
				ofPortPhys,
	...
	if ofPortPhys != "" {
		...
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=10, table=0, in_port=%s, %s dl_dst=%s, actions=%s",
				nodeutil.DefaultOpenFlowCookie, ofPortPhys, match_vlan, bridgeMacAddress, actions))
	}
	...
	for _, netConfig := range b.patchedNetConfigs() {
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=10, table=0, in_port=%s, dl_src=%s, actions=output:NORMAL",
				nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress))
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=9, table=0, in_port=%s, actions=drop",
				nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch))
	}
		...
		if ofPortPhys != "" {
			for _, netConfig := range b.patchedNetConfigs() {
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=105, in_port=%s, dl_src=%s, ip, pkt_mark=%s "+
						...
						nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, nodeutil.OvnKubeNodeSNATMark,
						...
						for mark, eip := range b.eipMarkIPs.GetIPv4() {
							dftFlows = append(dftFlows,
								fmt.Sprintf("cookie=%s, priority=105, in_port=%s, dl_src=%s, ip, pkt_mark=%d, "+
									"actions=ct(commit, zone=%d, nat(src=%s), exec(set_field:%s->ct_mark)), output:%s",
									nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, mark,
				...
				if netConfig.IsDefaultNetwork() {
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, dl_src=%s, ip, "+
							...
							nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, config.Default.ConntrackZone,
					...
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, dl_src=%s, ip, ip_src=%s, "+
							"actions=ct(commit, zone=%d, nat(src=%s), exec(set_field:%s->ct_mark)), output:%s",
							nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, netConfig.V4MasqIPs.GatewayRouter.IP, config.Default.ConntrackZone,
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=100, in_port=%s, ip, "+
					...
					nodeutil.DefaultOpenFlowCookie, ofPortHost, config.Default.ConntrackZone, ctMarkHost, mod_vlan_id, ofPortPhys))
		}
			...
			for _, netConfig := range b.patchedNetConfigs() {
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=175, in_port=%s, tcp, nw_src=%s, "+
						...
						nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, physicalIP.IP, config.Default.HostMasqConntrackZone))
				dftFlows = append(dftFlows,
				...
				if ofPortPhys != "" {
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=650, table=0, in_port=%s, dl_src=%s, udp, tp_dst=3784, actions=output:%s",
							nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, ofPortPhys))
				}
			}
		}
		...
		if ofPortPhys != "" {
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=50, in_port=%s, ip, "+
					"actions=ct(zone=%d, nat, table=1)", nodeutil.DefaultOpenFlowCookie, ofPortPhys, config.Default.ConntrackZone))
		}
	}
				...
				if netConfig.IsDefaultNetwork() {
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, dl_src=%s, ipv6, "+
							"actions=ct(commit, zone=%d, exec(set_field:%s->ct_mark)), output:%s",
							nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, config.Default.ConntrackZone, netConfig.MasqCTMark, ofPortPhys))
					...
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=100, in_port=%s, dl_src=%s, ipv6, ipv6_src=%s, "+
							"actions=ct(commit, zone=%d, nat(src=%s), exec(set_field:%s->ct_mark)), output:%s",
							nodeutil.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, netConfig.V6MasqIPs.GatewayRouter.IP, config.Default.ConntrackZone,
							physicalIP.IP, netConfig.MasqCTMark, ofPortPhys))
				}
			}
		...
		for _, clusterEntry := range config.Default.ClusterSubnets {
			...
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=104, in_port=%s, %s, %s_src=%s, actions=drop",
					nodeutil.DefaultOpenFlowCookie, defaultNetConfig.OfPortPatch, ipv, ipv, cidr))
		}
			...
			if ofPortPhys != "" {
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=109, in_port=%s, dl_src=%s, %s, %s_src=%s"+
						"actions=ct(commit, zone=%d, exec(set_field:%s->ct_mark)), output:%s",
						nodeutil.DefaultOpenFlowCookie, defaultNetConfig.OfPortPatch, bridgeMacAddress, ipv, ipv, subnet,
			...
			for _, clusterEntry := range netConfig.Subnets {
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=15, table=1, %s, %s_dst=%s, "+
						"actions=output:%s",
						nodeutil.DefaultOpenFlowCookie, ipv, ipv, cidr, output))
			}
				...
				for _, subnet := range netConfig.NodeSubnets {
					...
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=16, table=1, %s, %s_dst=%s, "+
							"actions=output:%s",
							nodeutil.DefaultOpenFlowCookie, ipv, ipv, mgmtIP.IP, nodeutil.OvsLocalPort),
					)
				}
			}
		}
			...
			for _, icmpType := range []int{types.RouteAdvertisementICMPType, types.NeighborAdvertisementICMPType} {
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=14, table=1,icmp6,icmpv6_type=%d actions=FLOOD",
						nodeutil.DefaultOpenFlowCookie, icmpType))
			}
			if ofPortPhys != "" {
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=13, table=1, in_port=%s, udp6, tp_dst=3784, actions=output:%s,output:%s",
						nodeutil.DefaultOpenFlowCookie, ofPortPhys, defaultNetConfig.OfPortPatch, ofPortHost))
			}
		}
		...
		if config.Gateway.Mode == config.GatewayModeLocal && !config.Gateway.DisablePacketMTUCheck {
			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=10, table=11, reg0=0x1, "+
					"actions=output:%s", nodeutil.DefaultOpenFlowCookie, ofPortHost))
				...
				if netConfig.MasqCTMark != CtMarkOVN {
					dftFlows = append(dftFlows,
						fmt.Sprintf("cookie=%s, priority=5, table=11, ct_mark=%s, "+
							"actions=output:%s", nodeutil.DefaultOpenFlowCookie, netConfig.MasqCTMark, netConfig.OfPortPatch))
				}
			}

			dftFlows = append(dftFlows,
				fmt.Sprintf("cookie=%s, priority=1, table=11, "+
					"actions=output:%s", nodeutil.DefaultOpenFlowCookie, defaultNetConfig.OfPortPatch))
		}

		// table 1, all other connections do normal processing
		dftFlows = append(dftFlows,
			fmt.Sprintf("cookie=%s, priority=0, table=1, actions=output:NORMAL", nodeutil.DefaultOpenFlowCookie))
	}
		...
		for _, icmpType := range []int{types.NeighborSolicitationICMPType, types.NeighborAdvertisementICMPType} {
			flows = append(flows,
				fmt.Sprintf("cookie=%s, priority=102, in_port=%s, dl_src=%s, icmp6, icmpv6_type=%d, "+
					"actions=ct(commit, zone=%d, exec(set_field:%s->ct_mark)), output:NORMAL",
					nodeutil.DefaultOpenFlowCookie, inPort, srcMAC, icmpType,
