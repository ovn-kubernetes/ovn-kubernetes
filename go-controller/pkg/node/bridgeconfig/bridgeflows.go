	...
	if ofPortPhys != "" {
		...
		for _, netConfig := range b.patchedNetConfigs() {
			...
			for _, subnet := range netConfig.NodeSubnets {
				...
				if (isAdvertisedLGW || !disableSNATMultipleGWs) && !config.OVNKubernetesFeature.EnableEgressIP {
					// skip adding egress flows for pod traffic to leave the node without SNAT if either:
					// - the network is advertised in LGW mode,
					// - or MEG is disabled
					// and EgressIP feature is disabled.
					continue
				}
				...
				// Select priority 104 for the senario when both EgressIP and advertised UDN are active:
				...
				dftFlows = append(dftFlows,
					fmt.Sprintf("cookie=%s, priority=104, in_port=%s, dl_src=%s, %s, %s_src=%s, "+
						"actions=output:%s",
						nodetypes.DefaultOpenFlowCookie, netConfig.OfPortPatch, bridgeMacAddress, ipv, ipv, subnet, ofPortPhys))
			}
		}
