		...
		if svcPort.NodePort > 0 {
			...
			for _, flowProtocol := range flowProtocols {
				...
				if isServiceTypeETPLocal && !localHostNetworkEndpoints.Empty() {
					...
					// table 0, This rule matches on all traffic with dst port == NodePort, DNAT's the nodePort to the
					...
					// table 0 - For each endpoint, matches on return traffic individually, i.e traffic coming from the
...
func (npw *nodePortWatcher) createLbAndExternalSvcFlows(service *corev1.Service, netConfig *bridgeconfig.BridgeUDNConfiguration,
	...
	for _, externalIPOrLBIngressIP := range externalIPOrLBIngressIPs {
		...
		if isServiceTypeETPLocal && !localHostNetworkEndpoints.Empty() {
			...
			// table 0, This rule matches on all traffic with dst ip == LoadbalancerIP / externalIP, DNAT's the nodePort
			...
			// table 0 - For each endpoint, matches on return traffic individually, i.e traffic coming from the
			// host networked pod's port, and unDNATs.
			// Use targetPortCookies, as the flow will be the same for each ExternalIP / LB status IP.
			for _, lbe := range lbes {
				targetPort := lbe.Port
				targetPortCookie, err := svcToCookie(service.Namespace, service.Name, "", targetPort)
				if err != nil {
...
func (npw *nodePortWatcher) UpdateEndpointSlice(oldEpSlice, newEpSlice *discovery.EndpointSlice) error {
	...
	oldLocalHostEndpoints, _ := npw.GetLocalEligibleEndpointAddresses([]*discovery.EndpointSlice{oldEpSlice}, svc).ExtractHostNetworkEndpoints(nodeIPs)
	newLocalHostEndpoints, newLocalEndpoints := npw.GetLocalEligibleEndpointAddresses(epSlices, svc).ExtractHostNetworkEndpoints(nodeIPs)

	localEndpointsHaveChanged := serviceInfo != nil && !serviceInfo.localEndpoints.Equals(newLocalEndpoints)
	localHostNetworkEndpointsPresenceHasChanged := !oldLocalHostEndpoints.Equals(newLocalHostEndpoints) ||
		serviceInfo != nil && !serviceInfo.localHostNetworkEndpoints.Equals(newLocalHostEndpoints)
