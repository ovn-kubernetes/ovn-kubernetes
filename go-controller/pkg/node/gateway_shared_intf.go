...
// The conntrack entries are deleted for the old IP:port:targetPort combinations if they are not present in the
// corresponding new IP:port:targetPort combinations for External and LB IP, ClusterIP and Node IP.
func (npw *nodePortWatcher) deleteConntrackForUpdatedUDPServiceIPPorts(old, new *corev1.Service) error {
	...
	// Get the new IP:port:targetPort combinations for External and LB IP, ClusterIP and Node IP and delete the
	// conntrack entries for the old IP:port:targetPort combinations that are not present in the
	// corresponding new IP:port:targetPort combinations
	deletedCount := 0
	var errs []error
	...
	extIPsDeletedCount, extIPsErrs := deleteStaleUDPConntrackForUDPPortsAndIPs(new.Spec.Ports, newExternalIPs, old.Spec.Ports, oldExternalIPs, old.Namespace, old.Name, "svcPort")

	// Cluster IPs
	newClusterIPs := util.GetClusterIPs(new)
	oldClusterIPs := util.GetClusterIPs(old)
	clusterIPsDeletedCount, clusterIPsErrs := deleteStaleUDPConntrackForUDPPortsAndIPs(new.Spec.Ports, newClusterIPs, old.Spec.Ports, oldClusterIPs, old.Namespace, old.Name, "svcPort")

	// Node IPs
	var newNodeIPs []net.IP
	if util.ServiceTypeHasNodePort(new) {
		newNodeIPs, _ = npw.nodeIPManager.ListAddresses()
	}
	var oldNodeIPs []net.IP
	if util.ServiceTypeHasNodePort(old) {
		oldNodeIPs, _ = npw.nodeIPManager.ListAddresses()
	}
	nodePortDeletedCount, nodePortErrs := deleteStaleUDPConntrackForUDPPortsAndIPs(new.Spec.Ports, newNodeIPs, old.Spec.Ports, oldNodeIPs, old.Namespace, old.Name, "nodePort")
...
// ipToString converts various IP types to string
func ipToString[T IPStringer](ip T) string {
...
// The conntrack entries are deleted for the old IP:port:targetPort combinations that are not present in the
// corresponding new IP:port:targetPort combinations.
func deleteStaleUDPConntrackForUDPPortsAndIPs[T IPStringer](
	...
	// Get the new IP:port:targetPort combinations
	...
	for _, newPort := range newServicePorts {
		...
		for _, newIP := range newIPs {
			newIPPortTargetPorts.Insert(util.JoinServiceIPPortTargetPort(ipToString(newIP), newPort.Port, newPort.TargetPort))
		}
	}

	// Delete the conntrack entries for the old IP:port:targetPort combinations that are not present in the
	// corresponding new IP:port:targetPort combinations
	for _, oldPort := range oldServicePorts {
		...
		for _, oldIP := range oldIPs {
			...
			if !newIPPortTargetPorts.Has(util.JoinServiceIPPortTargetPort(ipToString(oldIP), port, oldPort.TargetPort)) {
				if err := util.DeleteConntrackServicePort(ipToString(oldIP), port, oldPort.Protocol,
					...
					errs = append(errs, fmt.Errorf("failed to delete conntrack entry for service %s/%s with %s %s, %s %d, protocol %s: %v",
						namespace, name, ipType, ipToString(oldIP), portType, port, oldPort.Protocol, err))
				} else {
					deletedCount++
				}
			}
