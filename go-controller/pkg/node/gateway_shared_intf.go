	...
	if util.IsNetworkSegmentationSupportEnabled() {
		...
		if util.IsUDNEnabledService(ktypes.NamespacedName{Namespace: service.Namespace, Name: service.Name}.String()) {
			...
			for _, clusterIP := range clusterIPs {
				...
				// Table 2 flow for routing traffic from UDN networks to default network services
				...
				if util.IsRouteAdvertisementsEnabled() {
					...
					flows = append(flows, fmt.Sprintf("cookie=%s, priority=490, in_port=%s, %s, %s=%s,actions=ct(zone=%d,nat,table=3)",
