...
// a. from the OVN network to the host network
// b. from the host to itself (through breth0)
// when the destination is on the same node as the sender.
// When a localnet is mapped to breth0, these flows are necessary for:
// a. pods in the default network to reach localnet pods on the same node
// b. pods in a localnet to reach the host on the same node.
...
func hostNetworkNormalActionFlows(netConfig *bridgeUDNConfiguration, srcMAC string, hostSubnets []*net.IPNet, isV6 bool) []string {
	...
	// Traffic path (a): OVN->host for shared gw mode
	...
	// Traffic path (a): OVN->host for local gw mode, equivalent to host->host
	// Traffic path (b): Host->host for both gw modes
	for _, hostSubnet := range hostSubnets {
		if (hostSubnet.IP.To4() == nil) != isV6 {
			continue
		}
	...
	if isV6 {
		...
		for _, icmpType := range []int{types.NeighborSolicitationICMPType, types.NeighborAdvertisementICMPType} {
			// Traffic path (a) for ICMP: OVN->host for shared gw mode
			if config.Gateway.Mode == config.GatewayModeShared {
				flows = append(flows,
					formatICMPFlow(netConfig.ofPortPatch, netConfig.masqCTMark, icmpType))
			}

			// Traffic path (a) for ICMP: OVN->host for local gw mode, equivalent to host->host
			// Traffic path (b) for ICMP: Host->host for both gw modes
