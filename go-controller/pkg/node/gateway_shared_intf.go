...
func (npw *nodePortWatcher) AddEndpointSlice(epSlice *discovery.EndpointSlice) error {
	...
	if !out.localHostNetworkEndpoints.Equals(localHostNetworkEndpoints) ||
		...
		if err = addServiceRules(svc, netInfo, localHostNetworkEndpoints, localEndpoints, npw); err != nil {
			...
			npw.updateServiceInfo(*svcNamespacedName, svc, localHostNetworkEndpoints, localEndpoints)
		}
		return utilerrors.Join(errors...)
	}
	return nil
...
func (npw *nodePortWatcher) DeleteEndpointSlice(epSlice *discovery.EndpointSlice) error {
	...
	if svcConfig, exists := npw.updateServiceInfo(*namespacedName, nil, nil, localEndpoints); exists {
		...
		if err = addServiceRules(svcConfig.service, netInfo, nil, localEndpoints, npw); err != nil {
			errors = append(errors, err)
		}
		return utilerrors.Join(errors...)
	}
...
// generateETPLocalDNATActions generates the actions string for ETP=local services with host-networked endpoints.
// It handles both single endpoint and multiple endpoints (using OVS groups with type=select).
// Returns the actions string and potentially an error if group creation fails.
