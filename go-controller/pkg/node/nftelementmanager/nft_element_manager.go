...
// OwnMap declares that this controller fully owns all elements in the given map.
// During periodic reconciliation, any elements found in the map that are not tracked
// by this controller will be removed.
func (c *Controller) OwnMap(mapName string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.ownedMaps.Insert(mapName)
}

// Sync replaces the tracked element set with the given elements and runs a
// full reconcile including owned-map cleanup. Any previously-tracked element
// not present in elems is marked for deletion. Use for startup repair where
// the complete desired state is known.
func (c *Controller) Sync(elems []*knftables.Element) error {
	c.mu.Lock()
	defer c.mu.Unlock()
