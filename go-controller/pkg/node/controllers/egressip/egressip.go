...
func (c *Controller) initNFTables(tx *knftables.Transaction) {
	...
	if c.v4 {
		...
		tx.Add(&knftables.Rule{
			...
			Rule:  knftables.Concat("snat to", "ip saddr map", "@", NFTablesMapV4),
		})
	}
	if c.v6 {
		tx.Add(&knftables.Map{
		...
		tx.Add(&knftables.Rule{
			Chain: NFTablesChainName,
			Rule:  knftables.Concat("snat to", "ip6 saddr map", "@", NFTablesMapV6),
		})
	}
...
func (c *Controller) deleteIPConfig(podIPConfigToDelete *podIPConfig) error {
	...
	return c.nftElementManager.Delete(&knftables.Element{
		Map:   mapName,
		Key:   []string{podIPConfigToDelete.snatKey},
...
func (c *Controller) applyPodConfig(existingPodIPsConfig *podIPConfigList, updatedPodIPsConfig *podIPConfigList) error {
	...
	for _, newPodIPConfig := range newPodIPConfigs.elems {
		...
		if err := c.nftElementManager.Add(&knftables.Element{
			Map:   mapName,
			Key:   []string{newPodIPConfig.snatKey},
...
func (c *Controller) repairNode() error {
	...
	expectedV4Elems := map[string]string{} // podIP -> egressIP
	expectedV6Elems := map[string]string{} // podIP -> egressIP
				...
				if namespaceSelector.Matches(namespaceLabels) {
					...
					for _, pod := range pods {
						...
						for _, podIP := range podIPs {
							...
							if isPodIPV6 {
								expectedV6Elems[podIP.String()] = status.EgressIP
							} else {
								expectedV4Elems[podIP.String()] = status.EgressIP
							}
	...
	for podIP, egressIP := range expectedV4Elems {
		expectedElems = append(expectedElems, &knftables.Element{
			Map:   NFTablesMapV4,
			Key:   []string{podIP},
			Value: []string{egressIP},
		})
	}
	for podIP, egressIP := range expectedV6Elems {
		expectedElems = append(expectedElems, &knftables.Element{
			Map:   NFTablesMapV6,
			Key:   []string{podIP},
			Value: []string{egressIP},
