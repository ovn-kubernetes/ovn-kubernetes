...
type UserDefinedNetworkGateway struct {
	...
	*gateway
	...
	// gwInterfaceId holds the link index of gateway interface
	gwInterfaceId int
}
...
func NewUserDefinedNetworkGateway(netInfo util.NetInfo, node *corev1.Node, nodeLister listers.NodeLister,
	...
	if gw.openflowManager != nil {
		...
		return nil, fmt.Errorf("openflow manager has not been invoked for network: %s", netInfo.GetNetworkName())
	}
	...
	return &UserDefinedNetworkGateway{
		...
		gwInterfaceId: link.Attrs().Index,
	}, nil
}
...
func (udng *UserDefinedNetworkGateway) AddNetwork() error {
	if udng.openflowManager == nil {
		return fmt.Errorf("openflow manager has not been invoked for network: %s", udng.NetInfo.GetNetworkName())
	}
