...
// that returns a non-NotFound error from GetActiveNetworkForNamespace
...
func (m *mockNetworkManagerWithError) GetActiveNetworkForNamespace(namespace string) (util.NetInfo, error) {
	return nil, fmt.Errorf("some other error for namespace \"%s\"", namespace)
}

// mockNetworkManagerWithNamespaceNotFoundError is a mock implementation of networkmanager.Interface
// that returns a Kubernetes NotFound error wrapped with "failed to get namespace"
...
// mockNetworkManagerWithInvalidPrimaryNetworkError is a mock implementation of networkmanager.Interface
// that returns an InvalidPrimaryNetworkError (UDN deleted scenario)
...
var _ = Describe("addServiceRules", func() {
	...
	Context("when GetActiveNetworkForNamespace returns error", func() {
		It("should wrap non-NotFound errors with service context", func() {
			service := &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
			...
			// Create a mock networkManager that returns a non-NotFound error
		...
		It("should silently ignore namespace NotFound errors", func() {
			service := &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-service-ns-not-found",
			...
			// Create a mock networkManager that returns a namespace NotFound error
			mockNetworkManager := &mockNetworkManagerWithNamespaceNotFoundError{}

			err := addServiceRules(service, nil, false, nil, mockNetworkManager)

			// When namespace is not found, the function should return nil (no error)
			Expect(err).NotTo(HaveOccurred())
		})

		It("should silently ignore InvalidPrimaryNetworkError (deleted UDN)", func() {
			service := &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-service-invalid-primary-network",
					Namespace: "udn-deleted-namespace",
				},
				Spec: corev1.ServiceSpec{
					Type:      corev1.ServiceTypeNodePort,
					ClusterIP: "10.96.0.4",
			...
			// Create a mock networkManager that returns InvalidPrimaryNetworkError
			// This simulates the scenario where a UDN has been deleted
			mockNetworkManager := &mockNetworkManagerWithInvalidPrimaryNetworkError{}

			err := addServiceRules(service, nil, false, nil, mockNetworkManager)

			// InvalidPrimaryNetworkError should be treated as a graceful skip (return nil)
			Expect(err).NotTo(HaveOccurred())
		})
	})
