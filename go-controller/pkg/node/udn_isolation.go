...
func (m *UDNHostIsolationManager) resolveKubeletCgroupPath() {
	path, procErr := detectKubeletCgroupPath(procMountpoint, unifiedMountpoint)
	if procErr == nil {
	...
	klog.V(5).Infof("Could not resolve kubelet cgroup from its process, "+
		"falling back to the systemd cgroup layout: %v", procErr)
	if systemdErr := m.findKubeletCgroupPath(unifiedMountpoint); systemdErr != nil {
		// kubelet is not running under a systemd-managed cgroup, which is the case
		// on distributions that don't use systemd to run it. Report both failures so
		// the node event carries the full picture.
		m.reportKubeletProbesUnsupported(fmt.Sprintf("%v, and %v", procErr, systemdErr))
	}
}
