...
func setupManagementPortIPFamilyConfig(routeManager *routemanager.Controller, mpcfg *managementPortConfig, cfg *managementPortIPFamilyConfig) ([]string, error) {
	...
	snatLocal := nodeipt.Rule{
		...
		Args: []string{
			...
			"-m", "addrtype",
			"--src-type", "LOCAL",
			"!", "-s", cfg.ifAddr.IP.String(),
			"-j", "SNAT",
			"--to-source", cfg.ifAddr.IP.String(),
			"-m", "comment", "--comment", "OVN SNAT to Management Port (LOCAL)"},
	}
	snatAll := nodeipt.Rule{
		Protocol: protocol,
		Table:    "nat",
		Chain:    iptableMgmPortChain,
		Args: []string{
	...
	var deleteRules []nodeipt.Rule
	var appendRules []nodeipt.Rule

	switch mpcfg.isRoutingAdvertised.Load() {
	case true:
		// even if routing is advertised for the network we still need to SNAT
		// local traffic so that it doesn't use the host masquerade address
		deleteRules = []nodeipt.Rule{snatAll}
		appendRules = []nodeipt.Rule{snatLocal}
	case false:
		deleteRules = []nodeipt.Rule{snatLocal}
		appendRules = []nodeipt.Rule{snatAll}
	}

	err = appendIptRules(appendRules)
	if err != nil {
		return warnings, fmt.Errorf("failed to append iptables rules for management port: %w", err)
	}

	err = deleteIptRules(deleteRules)
	if err != nil {
		return warnings, fmt.Errorf("failed to delete iptables rules for management port: %w", err)
	}
