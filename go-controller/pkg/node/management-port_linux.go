...
func setupManagementPortIPFamilyConfig(routeManager *routemanager.Controller, mpcfg *managementPortConfig, cfg *managementPortIPFamilyConfig) ([]string, error) {
	...
	for _, subnet := range cfg.allSubnets {
		route, err := util.LinkRouteExists(mpcfg.link, cfg.gwIP, subnet)
		if err != nil {
			return warnings, err
		}
		// Check MTU manually since netlink doesn't support it as an RT_FILTER
		if route != nil {
			if route.MTU == config.Default.RoutableMTU {
				continue
			}
		}
		...
		warnings = append(warnings, fmt.Sprintf("missing route entry for subnet %s via gateway %s on link %v"+
			" with MTU: %d", subnet, cfg.gwIP, mpcfg.ifName, config.Default.RoutableMTU))
		subnetCopy := *subnet
