...
// The empty value keeps zero-valued UDN gateways on the default bridge path.
const defaultOpenFlowBridgeSetName = ""

type openflowBridgeTarget struct {
	name   string
	bridge *bridgeconfig.BridgeConfiguration
}

func defaultOpenFlowBridgeTarget() openflowBridgeTarget {
	return openflowBridgeTarget{name: defaultOpenFlowBridgeSetName}
}

func uplinkOpenFlowBridgeTarget(bridge *bridgeconfig.BridgeConfiguration) openflowBridgeTarget {
	return openflowBridgeTarget{
		name:   bridge.GetBridgeName(),
		bridge: bridge,
	}
}

func (t openflowBridgeTarget) isDefaultBridgeSet() bool {
	return t.name == defaultOpenFlowBridgeSetName
}
...
func (c *openflowManager) addNetwork(target openflowBridgeTarget, nInfo util.NetInfo, nodeSubnets, mgmtIPs []*net.IPNet, masqCTMark, pktMark uint, v6MasqIPs, v4MasqIPs *udn.MasqueradeIPs) error {
	if !target.isDefaultBridgeSet() {
		return c.addNetworkToUplinkBridge(target, nInfo, nodeSubnets, mgmtIPs, masqCTMark, pktMark, v6MasqIPs, v4MasqIPs)
	}
...
func (c *openflowManager) addNetworkToUplinkBridge(target openflowBridgeTarget, nInfo util.NetInfo, nodeSubnets, mgmtIPs []*net.IPNet, masqCTMark, pktMark uint, v6MasqIPs, v4MasqIPs *udn.MasqueradeIPs) error {
	c.uplinkBridgesMu.Lock()
	defer c.uplinkBridgesMu.Unlock()

	uplinkBridge, found := c.uplinkBridges[target.name]
	if !found {
		uplinkBridge = newOpenflowBridge(target.bridge)
		c.uplinkBridges[target.name] = uplinkBridge
	}
