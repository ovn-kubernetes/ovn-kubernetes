...
func (g *BridgeEIPAddrManager) UpdateEgressIP(oldEIP, newEIP *egressipv1.EgressIP) (bool, error) {
	// UpdateEgressIP handles three distinct scenarios:
	// 1. Cleanup: Secondary network IP mistakenly configured by old code (delete only)
	// 2. No-op: Status unchanged, no action needed (return early)
	// 3. Normal update: IP changed or removed from this node (delete old, then add new)

	// Phase 1: Handle deletion of old EgressIP if previously assigned to this node
	oldDeleted, err := g.deleteOldEgressIPIfNeeded(oldEIP, newEIP)
	if err != nil {
		return false, fmt.Errorf("failed to update EgressIP gateway config because %v", err)
	}

	// Phase 2: Add new EgressIP if assigned to this node
	newAdded, err := g.addNewEgressIPIfNeeded(newEIP)
	if err != nil {
		return oldDeleted, fmt.Errorf("failed to update EgressIP gateway config because %v", err)
	}

	return oldDeleted || newAdded, nil
}

// deleteOldEgressIPIfNeeded handles the deletion phase of an EgressIP update.
// Returns true if an IP was deleted (config changed).
// Three scenarios:
// 1. Secondary network IP cleanup: Delete if mistakenly configured, even if status unchanged
// 2. No change: Return false if status unchanged and IP is valid OVN network IP
// 3. Normal deletion: Delete old IP if it's being removed or changed
func (g *BridgeEIPAddrManager) deleteOldEgressIPIfNeeded(oldEIP, newEIP *egressipv1.EgressIP) (bool, error) {
	// Find old status for this node
	var oldStatus egressipv1.EgressIPStatusItem
	var oldStatusFound bool
	...
	if !oldStatusFound {
		return false, nil // No old IP on this node, nothing to delete
	}

	if !util.IsEgressIPMarkSet(oldEIP.Annotations) {
		return false, nil // Upgrade scenario: old code didn't set marks
	}

	// Parse and validate old EgressIP
	ip, pktMark, shouldSkip, err := g.parseAndValidateEIP(oldEIP)
	if err != nil {
		return false, fmt.Errorf("unable to parse and validate old EgressIP: %v", err)
	}

	// Scenario 1: Cleanup mistakenly configured secondary network IP
	if shouldSkip && g.isIPInAnnotation(ip) {
		if err = g.deleteEgressIPConfig(ip, pktMark); err != nil {
			return false, err
		}
		return true, nil
	}

	// Scenario 2: No change - status entry exists in both old and new
	if util.IsItemInSlice(newEIP.Status.Items, oldStatus) {
		return false, nil // Nothing to do, status unchanged
	}

	// Scenario 3: Normal deletion - IP changed or removed (skip secondary IPs)
	if !shouldSkip {
		if err = g.deleteEgressIPConfig(ip, pktMark); err != nil {
			return false, err
		}
...
// addNewEgressIPIfNeeded handles the addition phase of an EgressIP update.
// Returns true if an IP was added (config changed).
func (g *BridgeEIPAddrManager) addNewEgressIPIfNeeded(newEIP *egressipv1.EgressIP) (bool, error) {
	if !util.IsEgressIPMarkSet(newEIP.Annotations) {
		return false, nil // Upgrade scenario: new code hasn't set marks yet
	}

	ip, pktMark, shouldSkip, err := g.parseAndValidateEIP(newEIP)
	if err != nil {
		return false, fmt.Errorf("unable to parse and validate new EgressIP: %v", err)
	}

	if shouldSkip {
		return false, nil // Secondary network IP, skip configuration
	}

	// Add IP to cache, annotation, and bridge
	// Must add to cache before adding IP to inform node IP handler this is not a valid node IP
	g.cache.insertMarkIP(pktMark, ip)
	if err = g.addIPToAnnotation(ip); err != nil {
		return false, fmt.Errorf("unable to add EgressIP IP to Node annotation: %v", err)
	}
	if err = g.addIPBridge(ip); err != nil {
		return false, fmt.Errorf("failed to add address to link: %v", err)
	}
...
func (g *BridgeEIPAddrManager) DeleteEgressIP(eip *egressipv1.EgressIP) (bool, error) {
	...
	// Skip secondary network IPs. Cleanup of stale secondary IPs from old code is handled in UpdateEgressIP.
	if shouldSkip {
		return false, nil
	}
	if err = g.deleteEgressIPConfig(ip, pktMark); err != nil {
		return false, fmt.Errorf("failed to delete EgressIP gateway config because %v", err)
	}
	return true, nil
}
...
func (g *BridgeEIPAddrManager) SyncEgressIP(objs []interface{}) error {
	...
	for _, obj := range objs {
		...
		if err != nil {
			...
			if ip != nil {
				klog.V(5).Infof("Skipping EgressIP %s during sync because it does not belong to the OVN network", ip.String())
			}
		} else {
...
// - shouldSkip: true if this EgressIP should be skipped (e.g., belongs to secondary network)
...
// isIPInAnnotation checks if the given IP exists in the node's bridge EgressIP annotation.
// Returns false if ip is nil or if there's an error reading the annotation.
func (g *BridgeEIPAddrManager) isIPInAnnotation(ip net.IP) bool {
	if ip == nil {
		return false
	}
	annotIPs, err := g.getAnnotationIPs()
	if err != nil {
		klog.Errorf("Failed to get annotation IPs while checking if IP %s is present: %v", ip.String(), err)
		return false
	}
	ipStr := ip.String()
	for _, annotIP := range annotIPs {
		if annotIP.String() == ipStr {
			return true
		}
	}
	return false
...
func (g *BridgeEIPAddrManager) deleteEgressIPConfig(ip net.IP, pktMark util.EgressIPMark) error {
	...
	if err := g.deleteIPsFromAnnotation(ip); err != nil {
		return fmt.Errorf("failed to delete EgressIP IP from Node annotation: %v", err)
	}
	return nil
}
