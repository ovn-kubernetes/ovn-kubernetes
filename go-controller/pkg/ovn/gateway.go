...
func (gw *GatewayManager) GatewayInit(
	...
	var nat *nbdb.NAT
	snatMatch, err := gw.GetNetworkScopedClusterSubnetSNATMatch(nodeName, gw.isRoutingAdvertised(nodeName))
	if err != nil {
...
func (gw *GatewayManager) GetNetworkScopedClusterSubnetSNATMatch(nodeName string, isNetworkAdvertised bool) (string, error) {
	if !isNetworkAdvertised {
		if gw.netInfo.TopologyType() != types.Layer2Topology {
			return "", nil
		}
		return fmt.Sprintf("outport == %q", types.GWRouterToExtSwitchPrefix+gw.netInfo.GetNetworkScopedGWRouterName(nodeName)), nil
	} else {
		...
		if config.IPv4Mode && config.IPv6Mode {
			destinationMatch = fmt.Sprintf("ip4.dst == %s || ip6.dst == %s", ipv4ClusterNodeIPAS, ipv6ClusterNodeIPAS)
		} else if config.IPv4Mode {
			destinationMatch = fmt.Sprintf("ip4.dst == %s", ipv4ClusterNodeIPAS)
		} else if config.IPv6Mode {
			destinationMatch = fmt.Sprintf("ip6.dst == %s", ipv6ClusterNodeIPAS)
		}
		if gw.netInfo.TopologyType() != types.Layer2Topology {
			return destinationMatch, nil
		}
		return fmt.Sprintf("outport == %q && %s", types.GWRouterToExtSwitchPrefix+gw.netInfo.GetNetworkScopedGWRouterName(nodeName), destinationMatch), nil
	}
}
