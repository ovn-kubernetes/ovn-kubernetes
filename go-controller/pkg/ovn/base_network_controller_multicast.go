...
func (bnc *BaseNetworkController) createMulticastAllowPolicy(ns string, nsInfo *namespaceInfo) error {
	...
	egressACL := libovsdbutil.BuildDefaultTierACL(dbIDs, types.DefaultMcastAllowPriority, egressMatch, nbdb.ACLActionAllow, nil, aclPipeline)
	...
	dbIDs = getNamespaceMcastACLDbIDs(ns, aclDir, bnc.controllerName)
	aclPipeline = libovsdbutil.ACLDirectionToACLPipeline(aclDir)
	ingressACL := libovsdbutil.BuildDefaultTierACL(dbIDs, types.DefaultMcastAllowPriority, ingressMatch, nbdb.ACLActionAllow, nil, aclPipeline)
...
func (bnc *BaseNetworkController) createDefaultDenyMulticastPolicy() error {
	...
	for _, aclDir := range []libovsdbutil.ACLDirection{libovsdbutil.ACLEgress, libovsdbutil.ACLIngress} {
		...
		aclPipeline := libovsdbutil.ACLDirectionToACLPipeline(aclDir)
		acl := libovsdbutil.BuildDefaultTierACL(dbIDs, types.DefaultMcastDenyPriority, match, nbdb.ACLActionDrop, nil, aclPipeline)
...
func (bnc *BaseNetworkController) createDefaultAllowMulticastPolicy() error {
	...
	for _, aclDir := range []libovsdbutil.ACLDirection{libovsdbutil.ACLEgress, libovsdbutil.ACLIngress} {
		...
		aclPipeline := libovsdbutil.ACLDirectionToACLPipeline(aclDir)
		acl := libovsdbutil.BuildDefaultTierACL(dbIDs, types.DefaultMcastAllowPriority, match, nbdb.ACLActionAllow, nil, aclPipeline)
		acls = append(acls, acl)
	}
