...
func (zch *ZoneChassisHandler) createOrUpdateNodeChassis(node *corev1.Node, isRemote bool) error {
	...
	// Get transport zones from node annotation.
	transportZones, err := util.GetNodeTransportZones(node)
	if err != nil {
		return fmt.Errorf("failed to determine transport zones for node - %s, error: %w",
			node.Name, err)
	}
