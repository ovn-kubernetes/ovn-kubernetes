...
var _ = ginkgo.Describe("Zone Interconnect Operations", func() {
	...
	ginkgo.Context("Error scenarios", func() {
		...
		ginkgo.It("DeleteNode should cleanup interconnect resources for remote nodes", func() {
			app.Action = func(ctx *cli.Context) error {
				dbSetup := libovsdbtest.TestSetup{
					NBData: initialNBDB,
					SBData: initialSBDB,
				...
				// When transport is no-overlay, DeleteNode should cleanup any existing interconnect resources
				// This simulates what the caller (master.go/layer3_udn_controller.go) would do
				err = zoneICHandler.DeleteNode(&testNode4)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())

				// Verify that no interconnect resources exist for the remote node
