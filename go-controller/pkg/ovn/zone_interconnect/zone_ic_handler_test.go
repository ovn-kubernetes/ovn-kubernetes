...
func CheckNorthboundResources(nbClient libovsdbclient.Client, transitSwitchName string, routerName string, expectedTransitSwitchPorts []string, expectedClusterRouterPorts []string, expecctedStaticRoutes []string) error {
	...
	gomega.Expect(actualStaticRoutes).To(gomega.ConsistOf(expecctedStaticRoutes))
	return nil
}
...
var _ = ginkgo.Describe("Zone Interconnect Operations", func() {
	...
	ginkgo.Context("Default network with multi-VTEP nodes", func() {
		var (
		...
		ginkgo.BeforeEach(func() {
			...
			// pod with VF interfaceon first VTEP of node1
			testPod3 = corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: "test",
					Name:      "pod-3",
			...
			// pod with veth interfaceon on node1 -- no encap IP specified
			testPod4 = corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: "test",
					Name:      "pod-4",
