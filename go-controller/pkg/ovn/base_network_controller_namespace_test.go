...
func TestBaseNetworkController_shouldFilterNamespace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "filters when namespace is NotFound in informer cache",
			netInfo: func(t *testing.T) util.NetInfo {
				t.Helper()
				return mustNetInfo(t, udnName, types.NetworkRolePrimary)
			},
			networkManager: &primaryNADNetworkManager{
				err: fmt.Errorf("failed to fetch namespace %q: %w", nsName,
					apierrors.NewNotFound(corev1.Resource("namespaces"), nsName)),
			},
			wantFilter: true,
		},
		{
