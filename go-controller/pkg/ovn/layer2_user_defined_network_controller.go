...
func (oc *Layer2UserDefinedNetworkController) ReconcilePod(oldPod, newPod *corev1.Pod, lastState interface{}) (interface{}, error) {
	...
	if newPod != nil && oc.isPodScheduledOnLocalNode(newPod) {
		if err := oc.updateLocalPodEvent(newPod); err != nil {
			// The base reconcile committed ports, but this handler has not
			// succeeded yet. Retain them even if node replay evicts the cache.
			ports, _ := state.(map[string]*lpInfo)
			oc.rememberIncompletePodPorts(newPod, ports)
			return nil, err
		}
	}
	return state, nil
