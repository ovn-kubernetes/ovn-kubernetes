...
const (
	// BaseTransitRouterTunnelKey is the base key for the transit router tunnel ports.
	// it is in the middle of the transit key range, where the first half is used by
	// transit switches of the Layer3 networks (see BaseTransitSwitchTunnelKey)
	BaseTransitRouterTunnelKey = 16711683 + 32768
)
...
func (oc *Layer2UserDefinedNetworkController) addRouterPortForRemoteNodeGR(node *corev1.Node) error {
...
func (oc *Layer2UserDefinedNetworkController) cleanupRemoteNodeGR(nodeName string) error {
	transitPort := &nbdb.LogicalRouterPort{
...
func (oc *Layer2UserDefinedNetworkController) newTransitRouter() (*nbdb.LogicalRouter, error) {
...
func (oc *SecondaryLayer2NetworkController) ensureUpgradeTopology(node *corev1.Node) error {
	...
	// create switch to router connection with MAC only
