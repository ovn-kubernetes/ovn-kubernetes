...
func (e *EgressIPController) addPodEgressIPAssignment(ni util.NetInfo, egressIPName string, status egressipv1.EgressIPStatusItem, mark util.EgressIPMark,
	...
	if loadedEgressNode && isLocalZoneEgressNode {
		...
		if ni.IsDefault() && !isOVNNetwork && (loadedPodNode && !isLocalZonePod) {
			...
			ops, err = e.createReroutePolicyOps(ni, ops, podIPs, status, mark, egressIPName, nextHopIP, routerName, pod.Namespace, pod.Name, isOVNNetwork, isLocalZoneEgressNode)
			if err != nil {
...
func (e *EgressIPController) createReroutePolicyOps(ni util.NetInfo, ops []ovsdb.Operation, podIPNets []*net.IPNet, status egressipv1.EgressIPStatusItem,
	mark util.EgressIPMark, egressIPName, nextHopIP, routerName, podNamespace, podName string, isOVNNetwork, isLocalZoneEgressNode bool) ([]ovsdb.Operation, error) {
	...
	// For default network with secondary host interface, set egress ip secondary interface mark
	// This mark will be cleared by node controller when SNAT is ready
	if ni.IsDefault() && !isOVNNetwork && isLocalZoneEgressNode {
		addPktMarkToLRPOptions(options, types.EgressIPSecondaryInterfaceMark)
	}
