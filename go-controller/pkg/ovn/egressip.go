...
func (e *EgressIPController) addPodEgressIPAssignment(ni util.NetInfo, egressIPName string, status egressipv1.EgressIPStatusItem, mark util.EgressIPMark,
	...
	if loadedEgressNode && isLocalZoneEgressNode {
		...
		// create LRPs with allow action (aka GW policy marks).
		if isOVNNetwork {
			if ni.IsDefault() {
...
func (e *EgressIPController) getGatewayNextHop(ni util.NetInfo, node *corev1.Node, isIPv6 bool) (string, error) {
	...
	return "", fmt.Errorf("unsupported network topology %s", ni.TopologyType())
}
...
func (e *EgressIPController) getRouterPortIP(portName string, wantsIPv6 bool) (string, error) {
	gatewayIPs, err := libovsdbutil.GetLRPAddrs(e.nbClient, portName)
	if err != nil {
		return "", fmt.Errorf("failed to retrieve port %s IP(s): %w", portName, err)
	}
	if gatewayIP, err := util.MatchFirstIPNetFamily(wantsIPv6, gatewayIPs); err != nil {
		return "", fmt.Errorf("failed to find IP for port %s for IP family %v: %v", portName, wantsIPv6, err)
	} else {
