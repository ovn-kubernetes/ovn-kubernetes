...
func (bsnc *BaseUserDefinedNetworkController) removePodForUserDefinedNetwork(pod *corev1.Pod, portInfoMap map[string]*lpInfo) error {
	...
	// The shared reconciler records handler state only after a successful
	// reconcile. A failed add can nevertheless leave recoverable cache state,
	// and durable state may also be restored between attempts. On a true delete,
	// merge that controller-owned cache state into cleanup. Do not do this when
	// the name has been reused: the name-keyed cache may already belong to the
	// replacement, whose state must be preserved.
	if portInfoMap == nil && replacementPod == nil {
