...
func TestPortCacheNilEntriesAreHandled(t *testing.T) {
	cache := NewPortCache(make(chan struct{}))
	pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: "namespace", Name: "pod"}}
	const nadKey = "namespace/nad"
	cache.cache["namespace/pod"] = map[string]*lpInfo{nadKey: nil}

	info, err := cache.get(pod, nadKey)
	if err == nil {
		t.Fatalf("get returned nil cache entry without an error: %#v", info)
	}
	if info != nil {
		t.Fatalf("get returned unexpected info for nil cache entry: %#v", info)
	}
	...
	cache.RLock()
	defer cache.RUnlock()
	if _, ok := cache.cache["namespace/pod"]; ok {
		t.Fatal("remove did not clean up nil cache entry and its empty pod map")
	}
}

func TestPortCacheRemoveAllForNetworkToleratesNilEntries(t *testing.T) {
	cache := NewPortCache(make(chan struct{}))
	const podName = "namespace/pod"
	cache.cache[podName] = map[string]*lpInfo{
		"nil-entry": nil,
		"owned": {
			name:               "owned-port",
			appliedNetworkName: "network-a",
		},
		"other": {
			name:               "other-port",
			appliedNetworkName: "network-b",
		},
	}

	cache.removeAllForNetwork("network-a")

	cache.RLock()
	defer cache.RUnlock()
	infoMap := cache.cache[podName]
	if _, ok := infoMap["nil-entry"]; ok {
		t.Fatal("nil cache entry was not cleaned up")
	}
	if _, ok := infoMap["owned"]; ok {
		t.Fatal("network-owned cache entry was not cleaned up")
	}
	if _, ok := infoMap["other"]; !ok {
		t.Fatal("cache entry for another network was removed")
	}
}
