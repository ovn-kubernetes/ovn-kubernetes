...
func (oc *DefaultNetworkController) syncNodesPeriodic() {
	...
	localNodes := make([]*corev1.Node, 0, len(kNodes))
	remoteZoneNodes := make([]*corev1.Node, 0, len(kNodes))
	for i := range kNodes {
		if oc.isLocalNode(kNodes[i]) {
			localNodes = append(localNodes, kNodes[i])
		} else {
			remoteZoneNodes = append(remoteZoneNodes, kNodes[i])
		}
	}

	if err := oc.syncChassis(localNodes, remoteZoneNodes); err != nil {
...
// corresponding nodes.
func (oc *DefaultNetworkController) syncChassis(localNodes, remoteZoneNodes []*corev1.Node) error {
	chassisList, err := libovsdbops.ListChassis(oc.sbClient)
	if err != nil {
...
func (oc *DefaultNetworkController) addUpdateLocalNodeEvent(node *corev1.Node, nSyncs *nodeSyncs) error {
	...
	_, _ = oc.localNodes.LoadOrStore(node.Name, true)
	...
	if nSyncs.syncZoneIC {
		...
		if oc.Transport() != types.NetworkTransportNoOverlay {
			...
			// interconnect resources in the OVN Northbound db for this local node.
	...
	// The address set contains cluster CIDRs + local node IPs and is used in SNAT
...
func (oc *DefaultNetworkController) addUpdateRemoteNodeEvent(node *corev1.Node, syncZoneIC bool) error {
	...
	start := time.Now()
	// Check if the remote node is present in the local node cache. If present,
	// it means the node moved from this controller to a remote one.
...
func (oc *DefaultNetworkController) addUpdateHoNodeEvent(node *corev1.Node) error {
	...
	for _, node := range nodes {
		node := *node
		// reconcile hybrid overlay subnets for local nodes.
		if !util.NoHostSubnet(&node) && oc.isLocalNode(&node) {
			if err := oc.handleHybridOverlayPort(&node, annotator); err != nil {
				return err
			}
		}
