...
func getExpectedPortNetworks(v4AllocatedSubnet, v6AllocatedSubnet *net.IPNet, nodeID int) (connectRouterNetworks, networkRouterNetworks []string) {
	...
	p2pSubnets, err := calculateP2PSubnets(allocatedSubnets, nodeID)
	if err != nil {
		return nil, nil
	}

	connectIPs, networkIPs := getP2PIPs(p2pSubnets)
...
func getExpectedPortTunnelKey(allocatedSubnetV4, allocatedSubnetV6 string, topologyType string, nodeID int) int {
	...
	networkIndex, maxNodes := getNetworkIndexAndMaxNodes(defaultConnectSubnets(), subnets)
...
// Returns error for use in Eventually
func verifyRouterPolicy(nbClient libovsdbclient.Client, routerName, cncName, srcOwner string, dstNetworkID, expectedCount int) error {
	policies, err := libovsdbops.FindALogicalRouterPoliciesWithPredicate(nbClient, routerName,
		func(policy *nbdb.LogicalRouterPolicy) bool {
			return policy.ExternalIDs != nil &&
				...
				policy.ExternalIDs[libovsdbops.NetworkKey.String()] == srcOwner &&
				policy.ExternalIDs[libovsdbops.NetworkIDKey.String()] == strconv.Itoa(dstNetworkID)
		})
	if err != nil {
	...
	if len(policies) != expectedCount {
		return fmt.Errorf("expected %d policies on %s for CNC %s srcOwner %s dstNetworkID %d, got %d",
			expectedCount, routerName, cncName, srcOwner, dstNetworkID, len(policies))
	}
	return nil
}
...
// Returns error for use in Eventually
func verifyRouterPolicyCount(nbClient libovsdbclient.Client, routerName, cncName, networkOwner string,
	networkID int, expectedCount int) error {
	policies, err := libovsdbops.FindALogicalRouterPoliciesWithPredicate(nbClient, routerName,
		func(policy *nbdb.LogicalRouterPolicy) bool {
			return policy.ExternalIDs != nil &&
				policy.ExternalIDs[libovsdbops.ObjectNameKey.String()] == cncName &&
				policy.ExternalIDs[libovsdbops.NetworkKey.String()] == networkOwner &&
				policy.ExternalIDs[libovsdbops.NetworkIDKey.String()] == strconv.Itoa(networkID)
		})
	if err != nil {
		return fmt.Errorf("failed to get policies from %s: %v", routerName, err)
	}
...
// Returns error for use in Eventually
// networkOwner is the format "{topology}_{networkID}" (e.g., "layer3_1")
...
func verifyRouterStaticRoutes(nbClient libovsdbclient.Client, routerName, cncName, networkOwner string,
	...
	routes, err := libovsdbops.GetRouterLogicalRouterStaticRoutesWithPredicate(nbClient, &nbdb.LogicalRouter{Name: routerName},
		func(route *nbdb.LogicalRouterStaticRoute) bool {
			return route.ExternalIDs != nil &&
				...
				route.ExternalIDs[libovsdbops.NetworkKey.String()] == networkOwner &&
	...
	if len(routes) != expectedCount {
		return fmt.Errorf("expected %d static routes on %s for CNC %s, network %s, node %d, got %d",
			expectedCount, routerName, cncName, networkOwner, nodeID, len(routes))
	}
...
// Returns error for use in Eventually
func verifyRouterStaticRoutesCount(nbClient libovsdbclient.Client, routerName, cncName, networkOwner string,
	nodeID int, expectedCount int) error {
	routes, err := libovsdbops.GetRouterLogicalRouterStaticRoutesWithPredicate(nbClient, &nbdb.LogicalRouter{Name: routerName},
		func(route *nbdb.LogicalRouterStaticRoute) bool {
			return route.ExternalIDs != nil &&
				route.ExternalIDs[libovsdbops.ObjectNameKey.String()] == cncName &&
		...
		Context("["+ipMode.name+"]", func() {
			...
			Context("CNC Lifecycle", func() {
				...
				It("should create ports and routes when CNC has only one allocated network", func() {
					...
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(),
							"ports-cnc", "layer3_1", networks[0].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					for i, net := range networks {
						...
						Eventually(func() error {
							return verifyRouterPolicy(nbClient, net.RouterName(),
								"ports-cnc", data.networkOwner, otherNetworkID, expectedPolicyCount)
						}).WithTimeout(5 * time.Second).Should(Succeed())
				...
				It("should delete all ports, policies and routes when CNC with two networks is deleted", func() {
					...
					for i, net := range networks {
						networkOwner := fmt.Sprintf("layer3_%d", net.id)
						otherNetworkID := networks[1-i].id
						Eventually(func() error {
							return verifyRouterPolicyCount(nbClient, net.RouterName(),
								"delete-cnc", networkOwner, otherNetworkID, expectedPerIPFamily)
						}).WithTimeout(5 * time.Second).Should(Succeed())
					}
					...
					for _, net := range networks {
						...
						Eventually(func() error {
							return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("delete-cnc"),
								"delete-cnc", networkOwner, 1, expectedPerIPFamily)
						}).WithTimeout(5 * time.Second).Should(Succeed())
					}
					...
					for i, net := range networks {
						...
						Eventually(func() error {
							return verifyRouterPolicyCount(nbClient, net.RouterName(),
								"delete-cnc", networkOwner, otherNetworkID, 0)
						}).WithTimeout(5 * time.Second).Should(Succeed())
					}
			...
			Context("Network Changes", func() {
				...
				It("should add ports, policies and routes when new network is connected via annotation update", func() {
					...
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"net1": {"10.128.1.0/24", "fd00:10:128:1::/64"}, "net2": {"10.129.1.0/24", "fd00:10:129:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
					start(initialDB, nodes, map[string]testNetwork{"net1": networks[0], "net2": networks[1]})

					// Create CNC with only first network
					connectSubnets := []networkconnectv1.ConnectSubnet{{CIDR: "192.168.0.0/16", NetworkPrefix: 24}}
					subnetAnnotation := buildConnectSubnetAnnotation(map[string]subnetPair{
						"layer3_1": {"192.168.0.0/24", "fd00:192:168::/64"},
					})
					cnc := createTestCNC("update-cnc", 400, connectSubnets, subnetAnnotation)
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("update-cnc"),
							"update-cnc", "layer3_1", 1, expectedRouteCount)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify no policies since only 1 network (nothing to connect to)
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(),
							"update-cnc", "layer3_1", networks[1].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("update-cnc"),
							"update-cnc", "layer3_2", 1, expectedRouteCount)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(),
							"update-cnc", "layer3_1", networks[1].id, expectedRouteCount)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[1].RouterName(),
							"update-cnc", "layer3_2", networks[0].id, expectedRouteCount)
					}).WithTimeout(5 * time.Second).Should(Succeed())
				})
				...
				It("should remove ports, policies and routes when network is disconnected via annotation update", func() {
					...
					networks := []testNetwork{
						{name: "remove-net1", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
						{name: "remove-net2", id: 2, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.132.0.0/14/23", "fd00:10:132::/48/64"}},
					}
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"remove-net1": {"10.128.1.0/24", "fd00:10:128:1::/64"}, "remove-net2": {"10.129.1.0/24", "fd00:10:129:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
					...
					for i, net := range networks {
						...
						Eventually(func() error {
							return verifyRouterPolicyCount(nbClient, net.RouterName(),
								"remove-net-cnc", networkOwner, otherNetworkID, expectedPerIPFamily)
						}).WithTimeout(5 * time.Second).Should(Succeed())

						// Static routes
						Eventually(func() error {
							return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("remove-net-cnc"),
								"remove-net-cnc", networkOwner, 1, expectedPerIPFamily)
						}).WithTimeout(5 * time.Second).Should(Succeed())
					}
					...
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(),
							"remove-net-cnc", "layer3_1", networks[1].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify first network still has routes
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("remove-net-cnc"),
							"remove-net-cnc", "layer3_1", 1, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify second network has no routes
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("remove-net-cnc"),
							"remove-net-cnc", "layer3_2", 1, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
				...
				It("should handle Layer2 networks with 0 -> 1 -> 2 -> 1 -> 0 network transitions", func() {
					...
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(),
							"l2-cnc", "layer2_1", networks[1].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[1].RouterName(),
							"l2-cnc", "layer2_2", networks[0].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("l2-cnc"),
							"l2-cnc", "layer2_1", 0, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					for i, net := range networks {
						networkOwner := fmt.Sprintf("layer2_%d", net.id)
						otherNetworkID := networks[1-i].id
						...
						Eventually(func() error {
							return verifyRouterPolicyCount(nbClient, net.RouterName(),
								"l2-cnc", networkOwner, otherNetworkID, expectedPerIPFamily)
						}).WithTimeout(5 * time.Second).Should(Succeed())

						// Static routes (Layer2 uses nodeID=0)
						Eventually(func() error {
							return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("l2-cnc"),
								"l2-cnc", networkOwner, 0, expectedPerIPFamily)
						}).WithTimeout(5 * time.Second).Should(Succeed())
					}
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("l2-cnc"),
							"l2-cnc", "layer2_2", 0, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
				...
				It("should handle mixed Layer2 and Layer3 networks with 0 -> 1 -> 2 -> 1 -> 0 transitions", func() {
					...
					l2Net := testNetwork{name: "mixed-l2", id: 1, topologyType: ovntypes.Layer2Topology, subnets: []string{"10.200.0.0/16", "fd00:10:200::/48"}}
					l3Net := testNetwork{name: "mixed-l3", id: 2, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}}
					networks := []testNetwork{l2Net, l3Net}
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"mixed-l3": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("mixed-cnc"),
							"mixed-cnc", "layer2_1", 0, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("mixed-cnc"),
							"mixed-cnc", "layer3_2", 1, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("mixed-cnc"),
							"mixed-cnc", "layer2_1", 0, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// No policies with only 1 network
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, l2Net.RouterName(),
							"mixed-cnc", "layer2_1", l3Net.id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("mixed-cnc"),
							"mixed-cnc", "layer3_2", 1, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify policies on both routers pointing to each other
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, l2Net.RouterName(),
							"mixed-cnc", "layer2_1", l3Net.id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, l3Net.RouterName(),
							"mixed-cnc", "layer3_2", l2Net.id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
			...
			Context("Node Events", func() {
				...
				It("should add ports and routes when a new node is added", func() {
					...
					networks := []testNetwork{
						{name: "node-add-net", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
					}
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"node-add-net": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
				...
				It("should remove ports and routes when a node is deleted", func() {
					...
					networks := []testNetwork{
						{name: "node-del-net", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
					}
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"node-del-net": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
						{name: "node2", id: 2, zone: "node2", nodeSubnets: map[string]subnetPair{"node-del-net": {"10.128.2.0/24", "fd00:10:128:2::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
				...
				It("should not create ports or routes until node ID is allocated", func() {
					...
					networks := []testNetwork{
						{name: "nodeid-wait-net", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
					}
					// Create node without node ID (id: 0 means no annotation)
					nodes := []testNode{
						{name: "node1", id: 0, zone: "node1", nodeSubnets: map[string]subnetPair{"nodeid-wait-net": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
				...
				It("should reconcile when node zone annotation is updated", func() {
					...
					zoneName = "node1"
					networks := []testNetwork{
						{name: "zone-update-net", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
					}
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"zone-update-net": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
				...
				It("should handle multiple zones", func() {
					...
					zoneName = "local-node"
					networks := []testNetwork{
						{name: "zone-net", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
					}
					nodes := []testNode{
						// local-node has zone annotation "local-node" which matches c.zone
						{name: "local-node", id: 1, zone: "local-node", nodeSubnets: map[string]subnetPair{"zone-net": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
						// remote-node is in different zone
						{name: "remote-node-1", id: 2, zone: "remote-zone", nodeSubnets: map[string]subnetPair{"zone-net": {"10.128.2.0/24", "fd00:10:128:2::/64"}}},
						{name: "remote-node-2", id: 3, zone: "remote-zone", nodeSubnets: map[string]subnetPair{"zone-net": {"10.128.3.0/24", "fd00:10:128:3::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("zone-cnc"),
							"zone-cnc", "layer3_1", 1, expectedPerIPFamily) // local-node routes
					}).WithTimeout(5 * time.Second).Should(Succeed())

					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("zone-cnc"),
							"zone-cnc", "layer3_1", 2, expectedPerIPFamily) // remote-node-1 routes
					}).WithTimeout(5 * time.Second).Should(Succeed())

					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("zone-cnc"),
							"zone-cnc", "layer3_1", 3, expectedPerIPFamily) // remote-node-2 routes
					}).WithTimeout(5 * time.Second).Should(Succeed())
				})
				...
				It("should update routes when node subnet annotation is updated", func() {
					// Setup
					networks := []testNetwork{
						{name: "subnet-update-net", id: 1, topologyType: ovntypes.Layer3Topology, subnets: []string{"10.128.0.0/14/23", "fd00:10:128::/48/64"}},
					}
					nodes := []testNode{
						{name: "node1", id: 1, zone: "node1", nodeSubnets: map[string]subnetPair{"subnet-update-net": {"10.128.1.0/24", "fd00:10:128:1::/64"}}},
					}
					initialDB := createInitialDBWithRouters(networks)
				...
				It("should create ports for all nodes when multiple networks are connected", func() {
					...
					for _, node := range nodes {
						Eventually(func() error {
							return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc"),
								"multi-cnc", "layer3_1", node.id, expectedPerIPFamily)
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc"),
							"multi-cnc", "layer2_2", 0, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify policies on Layer3 network router pointing to Layer2 network
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, l3Net.RouterName(),
							"multi-cnc", "layer3_1", l2Net.id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify policies on Layer2 network router pointing to Layer3 network
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, l2Net.RouterName(),
							"multi-cnc", "layer2_2", l3Net.id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
				})
			})
				...
				It("should handle multiple CNCs independently", func() {
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc1"),
							"multi-cnc1", "layer3_1", 1, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc1"),
							"multi-cnc1", "layer3_1", 2, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					// Routes for net2
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc1"),
							"multi-cnc1", "layer3_2", 1, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc1"),
							"multi-cnc1", "layer3_2", 2, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc2"),
							"multi-cnc2", "layer3_3", 1, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc2"),
							"multi-cnc2", "layer3_3", 2, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					// Routes for net4
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc2"),
							"multi-cnc2", "layer3_4", 1, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					Eventually(func() error {
						return verifyRouterStaticRoutesCount(nbClient, getConnectRouterName("multi-cnc2"),
							"multi-cnc2", "layer3_4", 2, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())

					// Verify routing policies on network routers
					// net1's router has policy for traffic to net2 (srcOwner=layer3_1, dstNetworkID=2)
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(), "multi-cnc1", "layer3_1", networks[1].id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					// net2's router has policy for traffic to net1 (srcOwner=layer3_2, dstNetworkID=1)
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[1].RouterName(), "multi-cnc1", "layer3_2", networks[0].id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					// net3's router has policy for traffic to net4 (srcOwner=layer3_3, dstNetworkID=4)
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[2].RouterName(), "multi-cnc2", "layer3_3", networks[3].id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					// net4's router has policy for traffic to net3 (srcOwner=layer3_4, dstNetworkID=3)
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[3].RouterName(), "multi-cnc2", "layer3_4", networks[2].id, expectedPerIPFamily)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					...
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[0].RouterName(), "multi-cnc1", "layer3_1", networks[1].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
					Eventually(func() error {
						return verifyRouterPolicyCount(nbClient, networks[1].RouterName(), "multi-cnc1", "layer3_2", networks[0].id, 0)
					}).WithTimeout(5 * time.Second).Should(Succeed())
