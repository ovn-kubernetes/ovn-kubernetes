...
func cncNeedsUpdate(oldObj, newObj *networkconnectv1.ClusterNetworkConnect) bool {
	...
	// Process if network selectors changed
	if !reflect.DeepEqual(oldObj.Spec.NetworkSelectors, newObj.Spec.NetworkSelectors) {
		return true
	}
...
func nodeNeedsUpdate(oldObj, newObj *corev1.Node) bool {
	...
	// Process if zone annotation changed
	...
	// Process if node ID annotation changed (shouldn't happen normally)
	return util.NodeIDAnnotationChanged(oldObj, newObj)
}
