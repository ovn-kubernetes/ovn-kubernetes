...
var _ = Describe("NetworkQoS DHCP-IPAM helpers", func() {
	...
	DescribeTable("util.IsIPsEqual (pod-update filter)",
		func(a, b []net.IP, equal bool) {
			Expect(util.IsIPsEqual(a, b)).To(Equal(equal))
			Expect(util.IsIPsEqual(b, a)).To(Equal(equal), "must be symmetric")
		},
		Entry("both empty", nil, nil, true),
		Entry("identical single IP", []net.IP{ip("172.18.0.208")}, []net.IP{ip("172.18.0.208")}, true),
		Entry("same addresses, different order", []net.IP{ip("172.18.0.208"), ip("fd00::5")}, []net.IP{ip("fd00::5"), ip("172.18.0.208")}, true),
		Entry("same count, different IP (DHCP lease change)", []net.IP{ip("172.18.0.208")}, []net.IP{ip("172.18.0.209")}, false),
		Entry("IP added", []net.IP{ip("172.18.0.208")}, []net.IP{ip("172.18.0.208"), ip("fd00::5")}, false),
		Entry("IP removed", []net.IP{ip("172.18.0.208")}, nil, false),
	)
...
var _ = Describe("NetworkQoS on a DHCP-IPAM localnet network", func() {
	It("reflects DHCP-learned pod IPs in the source address set and provisions valid QoS", func() {
		...
		// subnet-less localnet NAD with DHCP IPAM
		nad := ovnk8stesting.GenerateNADWithConfig("dhcpnet", "default", fmt.Sprintf(
			`{"cniVersion":"1.1.0","name":"dhcpnet","type":"ovn-k8s-cni-overlay","topology":"localnet",`+
				`"ipam":{"type":"dhcp"},"netAttachDefName":"%s","role":"secondary"}`, dhcpNadKey))
		...
		pod, err := fakeKubeClient.CoreV1().Pods(nqosNamespace).Get(context.TODO(), clientPodName, metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = fakeKubeClient.CoreV1().Pods(nqosNamespace).Update(context.TODO(), pod, metav1.UpdateOptions{})
		Expect(err).NotTo(HaveOccurred())
