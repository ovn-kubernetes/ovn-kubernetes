...
// isPodScheduledOnLocalNode returns whether the pod is scheduled on the node managed by this controller.
func (nb *northBoundClient) isPodScheduledOnLocalNode(pod *corev1.Pod) bool {
...
func (nb *northBoundClient) deleteGatewayIPs(podNsName ktypes.NamespacedName, toBeDeletedGWIPs, _ sets.Set[string]) error {
	return nb.externalGatewayRouteInfo.Cleanup(podNsName, func(routeInfo *RouteInfo) error {
		...
		if err == nil {
			if !nb.isPodScheduledOnLocalNode(pod) {
