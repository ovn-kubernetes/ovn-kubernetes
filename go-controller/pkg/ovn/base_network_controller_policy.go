...
func (bnc *BaseNetworkController) addLocalPodsToNetworkPolicy(np *networkPolicy, objs ...interface{}) error {
	...
	return bnc.addLocalPodsToNetworkPolicyLocked(np, objs...)
}

// addLocalPodsToNetworkPolicyLocked also serves policy bootstrap, which must
// retain the write lock from index publication through the initial pod snapshot.
func (bnc *BaseNetworkController) addLocalPodsToNetworkPolicyLocked(np *networkPolicy, objs ...interface{}) error {
...
func (bnc *BaseNetworkController) createNetworkPolicy(policy *knet.NetworkPolicy, aclLogging *libovsdbutil.ACLLoggingLevels) (*networkPolicy, error) {
	...
	// 6. Sync existing local pods while pod handlers wait on the policy lock.
	// 7. Unlock networkPolicy before requesting retries for unresolved pods.
	...
	err := bnc.networkPolicies.DoWithLock(npKey, func(npKey string) error {
		...
		// Pod events observing the published policy must wait until the initial
		// snapshot is committed, so an older snapshot cannot undo a label update.
		...
		podSyncErr := bnc.addLocalPodsToNetworkPolicyLocked(np, objs...)
		// Never acquire retry pod locks while holding the policy lock.
		np.Unlock()
		npLocked = false
		if podSyncErr != nil {
			klog.Errorf("Failed to sync local pods for network policy %s/%s on network %s, pods will be retried individually: %v",
				policy.Namespace, policy.Name, bnc.GetNetworkName(), podSyncErr)
