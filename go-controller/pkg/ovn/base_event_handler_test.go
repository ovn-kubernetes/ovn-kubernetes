...
func TestNamespaceAreResourcesEqual(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "different ExternalGatewayPodIPs annotation is not equal",
			old: &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name:        "test-ns",
					Annotations: map[string]string{util.ExternalGatewayPodIPsAnnotation: `["10.0.0.1"]`},
				},
			},
			new: &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name:        "test-ns",
					Annotations: map[string]string{util.ExternalGatewayPodIPsAnnotation: `["10.0.0.2"]`},
				},
			},
			expected: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("areResourcesEqual(%q) returned unexpected error: %v", tt.name, err)
			}
			if result != tt.expected {
				t.Errorf("areResourcesEqual(%q) = %v, want %v", tt.name, result, tt.expected)
			}
		})
	}
}
