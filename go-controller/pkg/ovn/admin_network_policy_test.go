		...
		ginkgo.DescribeTable("should create/update/delete address-sets, acls, port-groups correctly",
			func(anpSubjectNetwork, anpPeerNetwork string, isUDNEnabled bool) {
				app.Action = func(ctx *cli.Context) error {
					...
					anpPeerNetworkSwitch := node1Switch
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							gNAD := ovntest.GenerateNAD(anpSubjectNetwork, nadName, anpSubjectNamespaceName,
						...
						} else if anpPeerNetwork != types.DefaultNetworkName {
							gNAD := ovntest.GenerateNAD(anpPeerNetwork, nadName, anpPeerNamespaceName,
								types.Layer3Topology, "200.128.0.0/16/24,2020:200:200::0/60/64", types.NetworkRolePrimary)
							k8sObjects = append(k8sObjects,
								&nadapi.NetworkAttachmentDefinitionList{
									Items: []nadapi.NetworkAttachmentDefinition{
										*gNAD,
							...
							anpPeerNetworkSwitch = &nbdb.LogicalSwitch{
								Name: anpPeerNetwork + "_" + node1Name,
								UUID: anpPeerNetwork + "_" + node1Name + "-UUID",
							}
							// we don't call WatchNodes for ANP tests; so the UDN isolation PG won't be
							// created; hence we need to manually add it during setup phase so that pod
							// creation on default network succeeds
							udnIsolationPG = getUDNPG()
							dbSetup.NBData = append(dbSetup.NBData, anpPeerNetworkSwitch, udnIsolationPG)
						}
					}
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							// ANP is not yet matching blue P-UDN since subject is part of default network; so no representation yet.
							expectedDatabaseState = append(expectedDatabaseState, anpPeerNetworkSwitch, udnIsolationPG)
						}
					}
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							expectedDatabaseState[0] = pg // subject pod is in default network
							// fill in the blue network's representation for this ANP
							udnACLs = getUDNACLsForANPRules(anp, anpPeerNetwork)
							udnPG = getUDNPGForANPSubject(anp.Name, nil, udnACLs, false, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, udnPG)
							peerUDNASIngressRule0v4, peerUDNASIngressRule0v6 = buildUDNANPAddressSets(anp, 0, []string{}, libovsdbutil.ACLIngress, anpPeerNetwork) // address-set will be empty since no pods match it yet
							for _, acl := range udnACLs {
								acl := acl
								expectedDatabaseState = append(expectedDatabaseState, acl)
							}
					...
					if isUDNEnabled {
						if anpPeerNetwork != types.DefaultNetworkName {
							secondaryNetController, ok := fakeOVN.secondaryControllers[anpPeerNetwork]
							gomega.Expect(ok).To(gomega.BeTrue())
							t2.addNetwork(anpPeerNetwork, util.GetNADName(anpPeerNamespaceName, nadName),
								"200.128.0.0/24", "200.128.0.2", "", "200.128.0.3", anpPodMAC2, types.NetworkRolePrimary, 1, nil)
							gomega.Expect(secondaryNetController.bnc.WatchNamespaces()).To(gomega.Succeed())
							gomega.Expect(secondaryNetController.bnc.WatchPods()).To(gomega.Succeed())
							t2.populateSecondaryNetworkLogicalSwitchCache(fakeOVN, secondaryNetController)
						}
					}
					...
					if isUDNEnabled {
						if anpPeerNetwork != types.DefaultNetworkName {
							...
							anpPeerPod.Annotations = map[string]string{
								"k8s.ovn.org/pod-networks": fmt.Sprintf(`{%q:{"ip_addresses":["%s/24"],"mac_address":"%s"}}`,
									util.GetNADName(anpPeerNamespaceName, nadName), "200.128.0.3",
									util.IPAddrToHWAddr(ovntest.MustParseIP("200.128.0.3")).String()),
							}
						}
					}
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							subjectUDNNSASIPv4, subjectUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpSubjectNetwork, anpSubjectNamespaceName, []string{copyT.podIP})
							expectedDatabaseState = append(expectedDatabaseState, subjectUDNNSASIPv4, subjectUDNNSASIPv6, udnIsolationPG)
						} else if anpPeerNetwork != types.DefaultNetworkName {
							netInfo := fakeOVN.controller.networkManager.GetNetwork(anpPeerNetwork)
							// we need to update the pod's UDN copy so that we can fetch the appropriate items for building the expected database
							// for the subject pod's blue P-UDN
							copyT2.podIP = "200.128.0.3"
							copyT2.podMAC = util.IPAddrToHWAddr(ovntest.MustParseIP("200.128.0.3")).String()
							copyT2.portName = util.GetSecondaryNetworkLogicalPortName(anpPeerNamespaceName, anpPeerPodName, netInfo.GetNADs()[0])
							copyT2.portUUID = copyT2.portName + "-UUID"
							udnIsolationPG.Ports = []string{t2.portUUID} // add the default network port for this pod
							// fill in the blue network's representation for this ANP
							expectedDatabaseState = append(expectedDatabaseState, udnPG)
							for _, acl := range udnACLs {
								acl := acl
							...
							peerUDNASIngressRule0v4, peerUDNASIngressRule0v6 := buildUDNANPAddressSets(anp, 0, []string{copyT2.podIP}, libovsdbutil.ACLIngress, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASIngressRule0v4, peerUDNASIngressRule0v6)
							expectedDatabaseState = append(expectedDatabaseState, getExpectedPodsAndSwitches(
								fakeOVN.controller.networkManager.GetNetwork(anpPeerNetwork), []testPod{copyT2}, []string{node1Name})...)
							peerUDNNSASIPv4, peerUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpPeerNetwork, anpPeerNamespaceName, []string{copyT2.podIP})
							expectedDatabaseState = append(expectedDatabaseState, peerUDNNSASIPv4, peerUDNNSASIPv6, udnIsolationPG)
							peerASIngressRule0v4, peerASIngressRule0v6 = buildANPAddressSets(anp, 0, nil, libovsdbutil.ACLIngress)
						}
					}
					expectedDatabaseState = append(expectedDatabaseState, peerASIngressRule0v4, peerASIngressRule0v6)
					gomega.Eventually(fakeOVN.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							expectedDatabaseState = append(expectedDatabaseState, getExpectedPodsAndSwitches(
								fakeOVN.controller.networkManager.GetNetwork(anpSubjectNetwork), []testPod{copyT}, []string{node1Name})...)
							subjectUDNASIPv4, subjectUDNASIPv6 := buildUDNNamespaceAddressSets(anpSubjectNetwork, anpSubjectNamespaceName, []string{copyT.podIP})
							expectedDatabaseState = append(expectedDatabaseState, subjectUDNASIPv4, subjectUDNASIPv6, udnIsolationPG)
							...
							// peer address-sets will contain the matching pod in blue P-UDN for allow rule
							peerUDNASIngressRule0v4, peerUDNASIngressRule0v6 = buildUDNANPAddressSets(anp,
								0, []string{copyT2.podIP}, libovsdbutil.ACLIngress, anpPeerNetwork) // podIP should be added to v4
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASIngressRule0v4, peerUDNASIngressRule0v6)
							peerUDNASIngressRule1v4, peerUDNASIngressRule1v6 = buildUDNANPAddressSets(
								anp, 1, []string{}, libovsdbutil.ACLIngress, anpPeerNetwork) // address-set will be empty since no pods match it yet
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASIngressRule1v4, peerUDNASIngressRule1v6)
							peerUDNASIngressRule2v4, peerUDNASIngressRule2v6 = buildUDNANPAddressSets(
								anp, 2, []string{}, libovsdbutil.ACLIngress, anpPeerNetwork) // address-set will be empty since no pods match it yet
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASIngressRule2v4, peerUDNASIngressRule2v6)
							expectedDatabaseState = append(expectedDatabaseState, getExpectedPodsAndSwitches(
								fakeOVN.controller.networkManager.GetNetwork(anpPeerNetwork), []testPod{copyT2}, []string{node1Name})...)
							peerUDNNSASIPv4, peerUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpPeerNetwork, anpPeerNamespaceName, []string{copyT2.podIP})
							expectedDatabaseState = append(expectedDatabaseState, peerUDNNSASIPv4, peerUDNNSASIPv6, udnIsolationPG)
							expectedDatabaseState[13].(*nbdb.AddressSet).Addresses = nil // since peer pod is part of blue UDN not default network
						}
					}
					gomega.Eventually(fakeOVN.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							// new egress rule addresssets
							peerUDNASEgressRule0v4, peerUDNASEgressRule0v6 = buildUDNANPAddressSets(
								anp, 0, []string{copyT2.podIP}, libovsdbutil.ACLEgress, anpPeerNetwork) // address-set will be empty since no pods match it yet
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASEgressRule0v4, peerUDNASEgressRule0v6)
							peerUDNASEgressRule1v4, peerUDNASEgressRule1v6 = buildUDNANPAddressSets(
								anp, 1, []string{}, libovsdbutil.ACLEgress, anpPeerNetwork) // address-set will be empty since no pods match it yet
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASEgressRule1v4, peerUDNASEgressRule1v6)
							peerUDNASEgressRule2v4, peerUDNASEgressRule2v6 = buildUDNANPAddressSets(
								anp, 2, []string{}, libovsdbutil.ACLEgress, anpPeerNetwork) // address-set will be empty since no pods match it yet
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASEgressRule2v4, peerUDNASEgressRule2v6)
							expectedDatabaseState = append(expectedDatabaseState, getExpectedPodsAndSwitches(
								fakeOVN.controller.networkManager.GetNetwork(anpPeerNetwork), []testPod{copyT2}, []string{node1Name})...)
							peerUDNNSASIPv4, peerUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpPeerNetwork, anpPeerNamespaceName, []string{copyT2.podIP})
					...
					if isUDNEnabled && anpPeerNetwork != types.DefaultNetworkName {
						peerASEgressRule0v4, peerASEgressRule0v6 = buildANPAddressSets(anp,
							0, nil, libovsdbutil.ACLEgress) // podIP should be added to blue network's addressset
					}
					...
					if isUDNEnabled && anpPeerNetwork != types.DefaultNetworkName {
						expectedDatabaseState[22].(*nbdb.AddressSet).Addresses = []string{copyT2.podIP} // podIP should be added to v4 allow address-set
						expectedDatabaseState[24].(*nbdb.AddressSet).Addresses = []string{copyT2.podIP} // podIP should be added to v4 deny address-set
					} else {
					...
					if isUDNEnabled && anpPeerNetwork != types.DefaultNetworkName {
						expectedDatabaseState[20].(*nbdb.AddressSet).Addresses = []string{copyT2.podIP} // podIP should be added to v4 Pass address-set
						expectedDatabaseState[18].(*nbdb.AddressSet).Addresses = []string{}             // podIP should be removed from allow rule AS
						expectedDatabaseState[16].(*nbdb.AddressSet).Addresses = []string{}             // podIP should be removed from deny rule AS
					} else {
					...
					if isUDNEnabled && anpPeerNetwork != types.DefaultNetworkName {
						expectedDatabaseState[26].(*nbdb.AddressSet).Addresses = []string{copyT2.podIP} // podIP should be added to v4 Pass address-set
						expectedDatabaseState[24].(*nbdb.AddressSet).Addresses = []string{}             // podIP should be removed from allow rule AS
						expectedDatabaseState[22].(*nbdb.AddressSet).Addresses = []string{}             // podIP should be removed from deny rule AS
					} else {
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							// the peer pod is not part of default network
							// so only add the subject podIP to default network's address-set rep
							expectedDatabaseState[len(expectedDatabaseState)-8].(*nbdb.AddressSet).Addresses = []string{t.podIP} // podIP should be added to v4 Pass address-set
							expectedDatabaseState[len(expectedDatabaseState)-2].(*nbdb.AddressSet).Addresses = []string{t.podIP} // podIP should be added to v4 Pass address-set
						}
					} else {
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							// the peer pod is not part of default network
							expectedDatabaseState[len(expectedDatabaseState)-6].(*nbdb.AddressSet).Addresses = []string{} // podIP should be removed from v4 allow address-set
							expectedDatabaseState[len(expectedDatabaseState)-2].(*nbdb.AddressSet).Addresses = []string{} // podIP should be removed from v4 allow address-set
							// fill in the blue network's representation for this ANP
							udnACLs = getUDNACLsForANPRules(anp, anpPeerNetwork)
							udnPG = getUDNPGForANPSubject(anp.Name, nil, udnACLs, false, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, udnPG)
							...
							// peer address-sets will contain the podIPs of pod matching blue UDN
							peerUDNASIngressRule0v4, peerUDNASIngressRule0v6 = buildUDNANPAddressSets(
								anp, 0, []string{}, libovsdbutil.ACLIngress, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASIngressRule0v4, peerUDNASIngressRule0v6)
							peerUDNASIngressRule1v4, peerUDNASIngressRule1v6 = buildUDNANPAddressSets(
								anp, 1, []string{copyT2.podIP}, libovsdbutil.ACLIngress, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASIngressRule1v4, peerUDNASIngressRule1v6)

							// peer address-sets will contain the podIPs of pod matching blue UDN
							peerUDNASEgressRule0v4, peerUDNASEgressRule0v6 = buildUDNANPAddressSets(
								anp, 0, []string{}, libovsdbutil.ACLEgress, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASEgressRule0v4, peerUDNASEgressRule0v6)
							peerUDNASEgressRule1v4, peerUDNASEgressRule1v6 = buildUDNANPAddressSets(
								anp, 1, []string{copyT2.podIP}, libovsdbutil.ACLEgress, anpPeerNetwork)
							expectedDatabaseState = append(expectedDatabaseState, peerUDNASEgressRule1v4, peerUDNASEgressRule1v6)
					...
					if isUDNEnabled && anpPeerNetwork != types.DefaultNetworkName {
						expectedDatabaseState[len(expectedDatabaseState)-3].(*nbdb.AddressSet).Addresses = nil // pod is gone from peer namespace address-set
						expectedDatabaseState[35].(*nbdb.AddressSet).Addresses = []string{}                    // pod is gone from ANP I0 address-set
						expectedDatabaseState[39].(*nbdb.AddressSet).Addresses = []string{}                    // pod is gone from peer namespace address-set
						expectedDatabaseState[42].(*nbdb.LogicalSwitch).Ports = []string{}                     // remove peer pod's LSP from switch
						expectedDatabaseState[len(expectedDatabaseState)-1].(*nbdb.PortGroup).Ports = nil
						expectedDatabaseState = append(expectedDatabaseState[:41], expectedDatabaseState[42:]...) // remove peer pod's LSP
					}
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							expectedDatabaseState = append(expectedDatabaseState, getExpectedPodsAndSwitches(
								fakeOVN.controller.networkManager.GetNetwork(anpPeerNetwork), nil, []string{node1Name})...)
							peerUDNNSASIPv4, peerUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpPeerNetwork, anpPeerNamespaceName, []string{})
							udnIsolationPG.Ports = nil
							expectedDatabaseState = append(expectedDatabaseState, peerUDNNSASIPv4, peerUDNNSASIPv6, udnIsolationPG)
						}
					}
					...
					if isUDNEnabled {
						if anpPeerNetwork != types.DefaultNetworkName {
							...
							anpPeerPod.Annotations = map[string]string{
								"k8s.ovn.org/pod-networks": fmt.Sprintf(`{%q:{"ip_addresses":["%s/24"],"mac_address":"%s"}}`,
									util.GetNADName(anpPeerNamespaceName, nadName), "200.128.0.4",
									util.IPAddrToHWAddr(ovntest.MustParseIP("200.128.0.4")).String()),
							}
						}
					}
					_, err = fakeOVN.fakeClient.KubeClient.CoreV1().Pods(anpPeerPod.Namespace).Update(context.TODO(), &anpPeerPod, metav1.UpdateOptions{})
					...
					copyT2.podIP = "200.128.0.4"
					copyT2.podMAC = "0a:58:c8:80:00:04"
					peerNSASIPv4, peerNSASIPv6 = buildNamespaceAddressSets(anpPeerNamespaceName, []string{t2.podIP})
					expectedDatabaseState = []libovsdbtest.TestData{pg, subjectNSASIPv4, subjectNSASIPv6, peerNSASIPv4, peerNSASIPv6}
					for _, acl := range newACLs {
						acl := acl
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							// TODO(tssurya): Check with Peri; this looks like a bug
							subjectUDNASIPv4, subjectUDNASIPv6 := buildUDNNamespaceAddressSets(anpSubjectNetwork, anpSubjectNamespaceName, []string{copyT.podIP})
							...
							// TODO(tssurya): Check with Peri; this looks like a bug
							peerUDNNSASIPv4, peerUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpPeerNetwork, anpPeerNamespaceName, []string{copyT2.podIP})
							expectedDatabaseState = append(expectedDatabaseState, peerUDNNSASIPv4, peerUDNNSASIPv6)
						}
					}
					...
					peerASEgressRule1v4, peerASEgressRule1v6 = buildANPAddressSets(anp,
						1, []string{t2.podIP}, libovsdbutil.ACLEgress)
					if isUDNEnabled && anpPeerNetwork != types.DefaultNetworkName {
						// peers were added to the blue P-UDN representation
						// so nothing to represent in deefault network for peers
						peerASIngressRule1v4, peerASIngressRule1v6 = buildANPAddressSets(anp,
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							// reset subject represention on blue P-UDN
							expectedDatabaseState[32].(*nbdb.AddressSet).Addresses = []string{} // peer podIP should be removed from namespace address-set
							expectedDatabaseState[24].(*nbdb.AddressSet).Addresses = []string{} // peer podIP should be removed from ANP rule address-set
							expectedDatabaseState[28].(*nbdb.AddressSet).Addresses = []string{} // peer podIP should be removed from ANP rule address-set
							expectedDatabaseState[34].(*nbdb.PortGroup).Ports = nil
							expectedDatabaseState[31].(*nbdb.LogicalSwitch).Ports = nil
							expectedDatabaseState = append(expectedDatabaseState[:30], expectedDatabaseState[31:]...) // remove peer pod's LSP
						}
					}
					gomega.Eventually(fakeOVN.nbClient, "3s").Should(libovsdbtest.HaveData(expectedDatabaseState))
					...
					if isUDNEnabled {
						if anpSubjectNetwork != types.DefaultNetworkName {
							...
							peerUDNNSASIPv4, peerUDNNSASIPv6 := buildUDNNamespaceAddressSets(anpPeerNetwork, anpPeerNamespaceName, []string{})
							udnIsolationPG.Ports = []string{} // add the default network port for this pod
							expectedDatabaseState = append(expectedDatabaseState, peerUDNNSASIPv4, peerUDNNSASIPv6, udnIsolationPG)
							expectedDatabaseState = append(expectedDatabaseState, getExpectedPodsAndSwitches(
								fakeOVN.controller.networkManager.GetNetwork(anpPeerNetwork), []testPod{}, []string{node1Name})...)
						}
					}
					gomega.Eventually(fakeOVN.nbClient).Should(libovsdbtest.HaveData(expectedDatabaseState))
