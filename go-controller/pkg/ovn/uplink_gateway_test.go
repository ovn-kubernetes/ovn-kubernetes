...
func TestL3GatewayConfigFromUplinkState(t *testing.T) {
	g := gomega.NewWithT(t)
	g.Expect(config.PrepareTestConfig()).To(gomega.Succeed())
	config.Gateway.Mode = config.GatewayModeShared
	config.Gateway.NodeportEnable = true

	gwConfig, err := l3GatewayConfigFromUplinkState(
		testUplinkState(metav1.ConditionTrue),
		"blue",
		"node-a",
		"chassis-a",
	)
...
func TestL3GatewayConfigFromUplinkStateRequiresReady(t *testing.T) {
	g := gomega.NewWithT(t)
	g.Expect(config.PrepareTestConfig()).To(gomega.Succeed())

	_, err := l3GatewayConfigFromUplinkState(
		testUplinkState(metav1.ConditionFalse),
		"blue",
		"node-a",
		"chassis-a",
	)
