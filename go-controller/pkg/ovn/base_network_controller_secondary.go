...
// getMasqueradeManagementIPSNATMatch returns an OVN match string for SNAT rules targeting packets with the specified destination MAC address.
func getMasqueradeManagementIPSNATMatch(dstMac string) string {
	return fmt.Sprintf("eth.dst == %s", dstMac)
}

// getClusterNodesDestinationBasedSNATMatch returns an OVN match string for SNAT rules targeting cluster node destination IPs based on the enabled IP mode.
