...
func ReconcileClusterDefaultNetworkLocalZone(watchFactory *factory.WatchFactory, nbClient libovsdbclient.Client,
	lsManager *logicalswitchmanager.LogicalSwitchManager, pod *corev1.Pod, clusterSubnets []config.CIDRNetworkEntry, nodeName string,
) error {
...
// reconcileClusterDefaultNetworkLocalZonePodAddressesToNodeRoute will add static routes and policies to ovn_cluster_route logical router
...
func reconcileClusterDefaultNetworkLocalZoneNeighbours(watchFactory *factory.WatchFactory, nbClient libovsdbclient.Client, lsManager *logicalswitchmanager.LogicalSwitchManager, pod *corev1.Pod, podAnnotation *util.PodAnnotation, nodeName string) error {
	// TODO: Implement neighbours update for non IC
	if !config.OVNKubernetesFeature.EnableInterconnect {
		return nil
	}

	currentNodeOwnsSubnet, err := nodeContainsPodSubnet(watchFactory, nodeName, podAnnotation, types.DefaultNetworkName)
	if err != nil {
		return err
	}

	vmRunningAtCurrentNode := pod.Spec.NodeName == nodeName

	if vmRunningAtCurrentNode {
		nodeOwningSubnet, err := findNodeOwningSubnet(watchFactory, lsManager, podAnnotation, types.DefaultNetworkName)
		if err != nil {
			return err
		}
		...
		gwIPs := ipsToIPNets(podAnnotation.Gateways)
		// Now that the VM is running at a node not owning its subnet, using pod's mac
		...
		if !currentNodeOwnsSubnet {
			if err := execOnNodePodsAnnotation(watchFactory, pod, nodeOwningSubnet, types.DefaultNetworkName, func(currentPodAnnotation *util.PodAnnotation) error {
				...
				klog.Errorf("Failed notifying neighbours with arp_proxy MAC at node %s: %v", nodeOwningSubnet.Name, err)
			}
			...
			if err := execOnNodePodsAnnotation(watchFactory, pod, nodeOwningSubnet, types.DefaultNetworkName, func(currentPodAnnotation *util.PodAnnotation) error {
				...
				klog.Errorf("Failed notifying neighbours with real MACs at node %s: %v", nodeOwningSubnet.Name, err)
			}
...
func ReconcileClusterDefaultNetworkRemoteZone(watchFactory *factory.WatchFactory, nbClient libovsdbclient.Client, pod *corev1.Pod, nodeName string) error {
	vmReady, err := virtualMachineReady(watchFactory, pod)
	if err != nil {
		return err
	}
	...
	if err := reconcileClusterDefaultNetworkRemoteZoneNeighbours(watchFactory, nbClient, pod, podAnnotation, nodeName); err != nil {
...
// migrated pod belongs to remote zone to send traffic over transit switch
...
//
// NOTE: Only applies for IC.
func reconcileClusterDefaultNetworkRemoteZoneNeighbours(watchFactory *factory.WatchFactory, nbClient libovsdbclient.Client, pod *corev1.Pod, podAnnotation *util.PodAnnotation, nodeName string) error {
	...
	if !vmRunningAtNodeOwningSubnet && currentNodeOwnsSubnet {
		// If we don't do this ARPs will be answered again with wrong mac
