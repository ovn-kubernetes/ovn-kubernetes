...
func ReconcileClusterDefaultNetworkLocalZone(watchFactory *factory.WatchFactory, nbClient libovsdbclient.Client,
	...
	if err := reconcileClusterDefaultNetworkLocalZoneNeighbours(watchFactory, pod, podAnnotation, nadName); err != nil {
...
func ReconcileClusterDefaultNetworkRemoteZone(watchFactory *factory.WatchFactory, nbClient libovsdbclient.Client, pod *corev1.Pod, nadName string) error {
	vmReady, err := virtualMachineReady(watchFactory, pod)
	if err != nil {
		return err
	}
	...
	if err := ensureRemoteZonePodAddressesToNodeRoute(watchFactory, nbClient, pod, podAnnotation, nadName); err != nil {
		return fmt.Errorf("failed to reconcile pod address to node route at kubevirt cluster default network remote zone: %w", err)
	}
	if err := reconcileClusterDefaultNetworkRemoteZoneNeighbours(watchFactory, nbClient, pod, podAnnotation, nadName); err != nil {
