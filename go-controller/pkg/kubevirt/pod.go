...
func DiscoverLiveMigrationStatus(client *factory.WatchFactory, pod *corev1.Pod) (*LiveMigrationStatus, error) {
	...
	if util.PodCompleted(targetPod) {
		...
		if len(livingPods) != 1 {
			return nil, fmt.Errorf("unexpected live migration state: should be one living pod")
		}
	...
	if len(livingPods) > 2 {
		...
		return nil, fmt.Errorf("unexpected live migration state at pods: %s", strings.Join(podNames, ","))
	}
