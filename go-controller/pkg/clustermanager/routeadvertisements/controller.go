...
func NewController(
	...
	eipConfig := &controllerutil.ControllerConfig[eiptypes.EgressIP]{
		RateLimiter:    workqueue.DefaultTypedControllerRateLimiter[string](),
		Reconcile:      c.reconcileRAsAdvertisingEgressIPs,
		Threadiness:    1,
...
func (c *Controller) getEgressIPsByNodesByNetworks(networks sets.Set[string]) (map[string]map[string]sets.Set[string], error) {
	nodeEIPConfigs := map[string]*util.ParsedNodeEgressIPConfiguration{}
	isEgressIPForSecondaryInterface := func(nodeName, ip string) (bool, error) {
		if nodeEIPConfigs[nodeName] == nil {
			node, err := c.nodeLister.Get(nodeName)
			if err != nil {
				return false, err
			}
			nodeEIPConfigs[nodeName], err = util.ParseNodePrimaryIfAddr(node)
			if err != nil {
				return false, err
			}
		}
		return !util.IsOVNNetwork(nodeEIPConfigs[nodeName], net.ParseIP(ip)), nil
	}
	...
	for _, eip := range eips {
		...
		for _, item := range eip.Status.Items {
			...
			// skip EIPs for secondary interfaces
			secondary, err := isEgressIPForSecondaryInterface(item.Node, item.EgressIP)
			if err != nil {
				return nil, err
			}
			if secondary {
...
func (c *Controller) reconcileRAsAdvertisingEgressIPs(string) error {
