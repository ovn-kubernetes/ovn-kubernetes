...
func NewController(
	...
	nodeConfig := &controllerutil.ControllerConfig[corev1.Node]{
		RateLimiter:    workqueue.DefaultTypedControllerRateLimiter[string](),
		Reconcile:      func(_ string) error { c.clearAllCache(); c.raController.ReconcileAll(); return nil },
		Threadiness:    1,
...
func checkSubnetSliceOverlaps(subnets []*net.IPNet) ([]string, error) {
	for i, subnet := range subnets {
		overlaps := util.IPNetOverlaps(subnet, subnets[i+1:]...)
		if len(overlaps) > 0 {
			return []string{subnet.String(), overlaps[0].String()}, nil
		}
	}
	return []string{}, nil
}
...
type raNetworkCacheEntry struct {
	nads             []*nadtypes.NetworkAttachmentDefinition
...
func (c *Controller) clearAllCache() {
	c.raCacheMutex.Lock()
	defer c.raCacheMutex.Unlock()
	c.raNetworkCache = make(map[string]*raNetworkCacheEntry)
}
