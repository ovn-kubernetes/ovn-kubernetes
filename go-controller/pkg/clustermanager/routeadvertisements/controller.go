...
const (
	...
	// evpnGlobalRouterKey is a reserved key added to matchedNetworks when a global
	// router is found. This allows tracking whether any FRRConfiguration
	// has a global router, which is required for MAC-VRF EVPN configuration.
	// Use a value that has no way of conflicting with a network name.
	evpnGlobalRouterKey = "@evpn_global_router"
)
...
func (c *Controller) generateFRRConfigurations(ra *ratypes.RouteAdvertisements) ([]*frrtypes.FRRConfiguration, []*nadtypes.NetworkAttachmentDefinition, error) {
	...
	for _, nad := range nads {
		...
		if (len(selectedNetworks.macVRFConfigs) > 0 || len(selectedNetworks.ipVRFConfigs) > 0) && (ra.Spec.TargetVRF != "auto" && ra.Spec.TargetVRF != vrf) {
	...
	// a global router to create one from.
	for _, cfg := range selectedNetworks.ipVRFConfigs {
		// The global router is represented by an empty string in the set.
		if !frrRouterVRFs.Has(cfg.VRFName) && !frrRouterVRFs.Has("") {
			return nil, nil, fmt.Errorf("%w: IP-VRF EVPN network %q requires a router with VRF %q or a global router, but none were found in selected FRRConfigurations", errConfig, cfg.NetworkName, cfg.VRFName)
		}
	}
	...
	for nodeName, frrConfigs := range nodeToFRRConfig {
		...
		// check that we found a global router if MAC-VRF EVPN is configured.
		if len(selectedNetworks.macVRFConfigs) > 0 && !matchedNetworks.Has(evpnGlobalRouterKey) {
			return nil, nil, fmt.Errorf("%w: MAC-VRF EVPN requires a global router with neighbors but none were found in selected FRRConfigurations", errConfig)
		}
	}
...
func (c *Controller) generateFRRConfiguration(
	...
	if len(selectedNetworks.macVRFConfigs) > 0 || len(selectedNetworks.ipVRFConfigs) > 0 {
		...
		// Track that we found a global router with neighbors
		if globalRouterASN > 0 && len(neighbors) > 0 {
			matchedNetworks.Insert(evpnGlobalRouterKey)
		}
	}
	...
	if len(routers) == 0 && len(selectedNetworks.macVRFConfigs) == 0 {
		// we ended up with no routers and there is no macVRF to configure, bail out
		return nil, nil
	}
