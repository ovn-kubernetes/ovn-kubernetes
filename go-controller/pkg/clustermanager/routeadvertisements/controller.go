...
func (c *Controller) generateFRRConfigurations(ra *ratypes.RouteAdvertisements) ([]*frrtypes.FRRConfiguration, []*nadtypes.NetworkAttachmentDefinition, error) {
	...
	if len(selectedNetworks.macVRFConfigs) > 0 && !frrRouterVRFs.Has("") {
		return nil, nil, fmt.Errorf("%w: MAC-VRF EVPN requires a global router but none were found in selected FRRConfigurations", errConfig)
	}
