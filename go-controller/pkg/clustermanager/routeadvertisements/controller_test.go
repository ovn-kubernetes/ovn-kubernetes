...
func (tn testNAD) NAD() *nadtypes.NetworkAttachmentDefinition {
	...
	if tn.EVPNMACVRFVNI > 0 || tn.EVPNIPVRFVNI > 0 {
		...
		cniConfig["evpnConfig"] = evpnConfig
	}
...
`,
		...
		{
			name: "reconciles EVPN IP-VRF network and creates a router",
			ra:   &testRA{Name: "ra", TargetVRF: "blue", AdvertisePods: true, NetworkSelector: map[string]string{"selected": "true"}},
			frrConfigs: []*testFRRConfig{
				{
					Name:      "frrConfig",
					Namespace: frrNamespace,
...
`,
		...
		{
			name: "fails to reconcile EVPN without global router",
			ra:   &testRA{Name: "ra", TargetVRF: "red", AdvertisePods: true, NetworkSelector: map[string]string{"selected": "true"}},
			frrConfigs: []*testFRRConfig{
				{
					Name:      "frrConfig",
