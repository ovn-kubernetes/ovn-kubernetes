	...
	for otherCNCName, otherCNCState := range c.cncCache {
		...
		if len(overlappingNetworks) != 0 {
			...
			if util.NetworksOverlap(cncState.connectSubnets, otherCNCState.connectSubnets) {
				// overlapping subnets are stored in the owner format <topology_networkID>,
				// which may be confusing for users, let's reformat to show network names
				networkNames := make([]string, 0, overlappingNetworks.Len())
				for networkKey := range overlappingNetworks {
					_, networkID, err := util.ParseNetworkOwner(networkKey)
					if err != nil {
						// this should never happen since the keys in selectedNetworks are generated by us in a consistent format
						klog.Errorf("Invalid network key %s in selected networks of CNC %s: %v", networkKey, cncState.name, err)
						networkNames = append(networkNames, networkKey)
						continue
					}
					netInfo := c.networkManager.GetNetworkByID(networkID)
					if netInfo == nil {
						// also unexpected
						klog.Errorf("Failed to get network info for network ID %d when reformatting overlapping network keys for CNC %s: network not found", networkID, cncState.name)
						networkNames = append(networkNames, networkKey)
						continue
					}
					networkNames = append(networkNames, netInfo.GetNetworkName())
				}
				return fmt.Errorf("connectSubnets overlap detected between CNC %s and CNC %s which both select networks %v",
					cncState.name, otherCNCName, networkNames)
			}
		}
	}
	return nil
		...
		case apitypes.ClusterUserDefinedNetworks:
			...
			for _, nad := range nads {
				...
				if !network.IsPrimaryNetwork() {
					errs = append(errs, fmt.Errorf("network %s has role 'Secondary'. Only 'Primary' role networks are supported", network.GetNetworkName()))
					continue
				}
