...
func TestHybridConnectSubnetAllocator_AllocateLayer2Subnet(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			owners:   []string{"layer2_1"},
			expectSubnets: map[string][]string{
				// Layer2 pool gets /64 block from layer3 (fd00:0:0:1::/64), then allocates /127 from it
		...
		},
		{
			...
			owners:   []string{"layer2_1"},
			expectSubnets: map[string][]string{
				// Layer2 pool gets /24 block from layer3 (192.168.0.0/24), then allocates /31 from it
				// IPv6 pool gets /64 block from layer3 (fd00:0:0:1::/64), then allocates /127 from it
...
func TestHybridConnectSubnetAllocator_AllocateMixedLayer3AndLayer2Subnets(t *testing.T) {
	...
	// Layer2 subnets are /31s within a /24 block allocated from layer3 for the layer2-pool.
...
func TestHybridConnectSubnetAllocator_ReleaseLayer2Subnets(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			allocateAgain:     []string{"layer2_3"},
			expectSubnets: map[string][]string{
				// Allocator continues from next position (layer2 pool gets /64, then allocates /127s)
				"layer2_3": {"fd00:0:0:1::6/127"},
			},
		},
		{
...
func TestHybridConnectSubnetAllocator_ReleaseMixedLayer3AndLayer2Subnets(t *testing.T) {
	...
	// IPv4: 192.168.2.0/24 is taken by layer2-pool, so next available is 192.168.3.0/24
	// IPv6: fd00:0:0:3::/64 is taken by layer2-pool, so next available is fd00:0:0:4::/64
	...
	// Allocate more layer2 - should continue in layer2 pool
...
func TestHybridConnectSubnetAllocator_Layer2PoolExpansionFromLayer3(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	allocator := NewHybridConnectSubnetAllocator()
	// Use small CIDRs to test pool exhaustion and expansion
...
func TestHybridConnectSubnetAllocator_Layer2CanReuseFromEarlierRange(t *testing.T) {
	// This test confirms that when the layer2 allocator has multiple ranges (pool blocks),
	// it can reuse a released slot from an EARLIER range, not just the most recent one.
	// This is important for understanding pool block release behavior.
	g := gomega.NewWithT(t)
	...
	// Allocate 8 layer2 networks - this fills the first /28 pool block
