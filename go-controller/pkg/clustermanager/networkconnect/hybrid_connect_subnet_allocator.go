...
func (hca *hybridConnectSubnetAllocator) AllocateLayer2Subnet(owner string) ([]*net.IPNet, error) {
	...
	if err := hca.expandLayer2Allocator(owner); err != nil {
...
func (hca *hybridConnectSubnetAllocator) ReleaseLayer2Subnet(owner string) {
	...
	// TODO(tssurya): We need to also release the pool blocks from the layer3 allocator if that block is now empty? or do we just keep it for the next layer2 network?
}
