...
type Controller struct {
	...
	// lastValidationError tracks the last validation error message to avoid spamming events
	lastValidationError string
	// validationHasRun tracks if validation has run at least once
	validationHasRun bool
}
...
func (c *Controller) Start() error {
	...
	err := controllerutil.StartWithInitialSync(func() error {
		c.runValidation()
		return nil
	}, c.raController)
	if err != nil {
...
func (c *Controller) runValidation() {
	...
	// Emit event if: (1) first run, or (2) error state changed
	shouldEmit := !c.validationHasRun || currentError != c.lastValidationError
...
func (c *Controller) validate() error {
	...
	// Sort names for stable error messages to support event deduplication
