...
var _ = ginkgo.Describe("No-Overlay Controller", func() {
	...
	ginkgo.Context("Controller creation", func() {
		...
		ginkgo.It("should have initial validation state on creation", func() {
			fakeClient := &util.OVNClusterManagerClientset{
				KubeClient:                fake.NewSimpleClientset(),
				NetworkAttchDefClient:     nadfake.NewSimpleClientset(),
				RouteAdvertisementsClient: rafake.NewSimpleClientset(),
			...
			controller := NewController(wf, recorder)
			gomega.Expect(controller.validationHasRun).To(gomega.BeFalse())
	...
	ginkgo.DescribeTable("Validation logic",
		...
		ginkgo.Entry("should fail with sorted RA names when multiple RAs are not accepted",
			[]*testRA{
				{
					Name:           "zebra-ra",
					SelectsDefault: true,
