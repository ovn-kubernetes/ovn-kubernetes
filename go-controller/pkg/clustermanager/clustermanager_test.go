	...
	ginkgo.Context("starting the cluster manager", func() {
		...
		ginkgo.When("the required features are not enabled", func() {
			ginkgo.It("does *not* automatically provision a NAD for the default network", func() {
				app.Action = func(ctx *cli.Context) error {
					...
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					gomega.Expect(clusterMngr.Start(ctx.Context)).NotTo(gomega.HaveOccurred())
		...
		ginkgo.When("the multi-network, network-segmentation, are customized-network features are enabled", func() {
			ginkgo.BeforeEach(func() {
				...
				config.OVNKubernetesFeature.EnableCustomNetworkConfig = true
			})
			...
			ginkgo.It("automatically provisions a NAD for the default network", func() {
				app.Action = func(ctx *cli.Context) error {
					...
					c, cancel := context.WithCancel(ctx.Context)
					defer cancel()
					gomega.Expect(clusterMngr.Start(c)).NotTo(gomega.HaveOccurred())
