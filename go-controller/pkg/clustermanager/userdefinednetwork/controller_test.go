		...
		Context("reconcile CUDN CR", func() {
			...
			It("should continue startup and allocate new VID when all NADs are corrupted", func() {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(err).NotTo(HaveOccurred())
					macVID, _ := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "Should allocate new VID since recovery failed")
				}).Should(Succeed())
			})
			...
			It("should continue startup and allocate new VID when VID recovery encounters a conflict", func() {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(err).NotTo(HaveOccurred())
					macVID, _ := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "Should allocate new VID since 5 is taken by another network")
				}).Should(Succeed())
			})
			...
			It("should continue startup and preserve MAC-VRF VID when only IP-VRF VID recovery encounters a conflict", func() {
				...
				// IP-VRF VID 7 conflicted, so during reconciliation it gets new VID 1.
				Eventually(func(g Gomega) {
					nad, err := cs.NetworkAttchDefClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(testNs.Name).Get(context.Background(), cudn.Name, metav1.GetOptions{})
					g.Expect(err).NotTo(HaveOccurred())
					macVID, ipVID := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(3), "MAC-VRF VID should be preserved (recovery succeeded)")
					g.Expect(ipVID).To(Equal(1), "IP-VRF gets new VID (first available, 7 is taken)")
				}).Should(Succeed())
			})
			...
			It("should continue startup and preserve IP-VRF VID when only MAC-VRF VID recovery encounters a conflict", func() {
				...
				// MAC-VRF VID 3 conflicted, so during reconciliation it gets new VID 1.
				Eventually(func(g Gomega) {
					nad, err := cs.NetworkAttchDefClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(testNs.Name).Get(context.Background(), cudn.Name, metav1.GetOptions{})
					g.Expect(err).NotTo(HaveOccurred())
					macVID, ipVID := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "MAC-VRF gets new VID (first available, 3 is already taken)")
			...
			It("should allocate new VID when namespace and NAD are created at runtime", func() {
				...
				// 4. Controller reconciles and allocates VID 1 (first available)
				vtep := testVTEP("vtep-test")
				...
				// Controller reconciles and allocates VID 1 (first available)
				Eventually(func(g Gomega) {
					nad, err := cs.NetworkAttchDefClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(testNs.Name).Get(context.Background(), cudn.Name, metav1.GetOptions{})
					g.Expect(err).NotTo(HaveOccurred())
					macVID, _ := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "VID should be allocated (first available)")
				}).Should(Succeed())
			})
			...
			It("should allocate new VID when existing NAD has VID taken by another CUDN", func() {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(err).NotTo(HaveOccurred())
					macVID, _ := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "VID should be newly allocated since 42 is taken by another CUDN")
				}).Should(Succeed())
			})

			It("should revert manual NAD VID change when allocator already has VID for this key", func() {
				// This tests the case where:
				// - Allocator has VID 1 for this key (from initial NAD creation)
				// - Someone manually changes NAD to VID 42
				// - Allocator's VID 1 should win, NAD reverted to 1
				...
				// Wait for initial NAD creation (will get VID 1)
				Eventually(func(g Gomega) {
					nad, err := cs.NetworkAttchDefClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(testNs.Name).Get(context.Background(), cudn.Name, metav1.GetOptions{})
					g.Expect(err).NotTo(HaveOccurred())
					macVID, _ := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "Initial VID should be 1")
				}).Should(Succeed())
				...
				// The NAD update triggers reconciliation. The allocator already has VID 1
				// for this key, so NAD is reverted to 1.
				Eventually(func(g Gomega) {
					nad, err := cs.NetworkAttchDefClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(testNs.Name).Get(context.Background(), cudn.Name, metav1.GetOptions{})
					g.Expect(err).NotTo(HaveOccurred())
					macVID, _ := evpnVIDsFromNAD(nad)
					g.Expect(macVID).To(Equal(1), "VID should be reverted to allocator's VID")
				}).Should(Succeed())
			})
