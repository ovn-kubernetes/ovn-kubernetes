	...
	Context("manager", func() {
		...
		Context("reconcile CUDN CR", func() {
			...
			It("should recover VIDs from existing NADs on controller restart", func() {
				...
				newCUDN := testEVPNClusterUDN("evpn-new", vtep.Name, testNs.Name)
				newCUDN.UID = "2"
			...
			It("should recover both MAC-VRF and IP-VRF VIDs from existing symmetric IRB NAD on controller restart", func() {
				...
				existingNAD.Spec.Config = `{"cniVersion":"1.0.0","name":"cluster.udn.evpn-irb-existing","type":"ovn-k8s-cni-overlay","netAttachDefName":"evpn-irb-recovery-test/evpn-irb-existing","topology":"layer2","role":"primary","subnets":"10.10.10.0/24","joinSubnet":"100.65.0.0/16,fd99::/64","transitSubnet":"100.88.0.0/16","transport":"evpn","evpnConfig":{"vtep":"vtep-irb-recovery","macVRF":{"vni":100,"vid":10},"ipVRF":{"vni":200,"vid":20}}}`
			...
			It("should fail startup when IP-VRF VID recovery encounters a conflict", func() {
				...
				existingNAD.Spec.Config = `{"cniVersion":"1.0.0","name":"cluster.udn.evpn-ipvrf-conflict","type":"ovn-k8s-cni-overlay","topology":"layer2","transport":"evpn","evpnConfig":{"vtep":"vtep-test","macVRF":{"vni":100,"vid":3},"ipVRF":{"vni":200,"vid":7}}}`
			...
			It("should skip VTEP validation and succeed when VTEP informer is not available", func() {
				testNs := testNamespace("evpn-no-vtep-informer-test")
				// CUDN references a non-existent VTEP, but since VTEP informer is nil,
				// validation should be skipped and NAD should be created
				...
				// CUDN should succeed (validation skipped) and create NAD
				Eventually(func() []metav1.Condition {
					cudn, err := cs.UserDefinedNetworkClient.K8sV1().ClusterUserDefinedNetworks().Get(context.Background(), cudn.Name, metav1.GetOptions{})
					Expect(err).NotTo(HaveOccurred())
					return normalizeConditions(cudn.Status.Conditions)
					...
					Status:  "True",
					Reason:  "NetworkAttachmentDefinitionCreated",
					Message: "NetworkAttachmentDefinition has been created in following namespaces: [evpn-no-vtep-informer-test]",
				}}), "should succeed when VTEP informer is not available")

				// NAD should be created
				Eventually(func() error {
					_, err := cs.NetworkAttchDefClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(testNs.Name).Get(context.Background(), cudn.Name, metav1.GetOptions{})
					return err
				}).Should(Succeed(), "NAD should be created when VTEP validation is disabled")
			})

			It("should skip VTEP validation for EVPN CUDN with nil EVPNConfiguration", func() {
				...
				// The VTEP validation in validateEVPNVTEP should return nil (skip validation),
				// but subsequent NAD rendering will correctly reject this malformed CR.
				...
				// CUDN should fail with NetworkAttachmentDefinitionSyncError because EVPN transport
				// requires EVPNConfiguration. This is different from VTEPNotFound - proving that
				// validateEVPNVTEP correctly returns nil when EVPNConfiguration is nil.
				Eventually(func() []metav1.Condition {
					cudn, err := cs.UserDefinedNetworkClient.K8sV1().ClusterUserDefinedNetworks().Get(context.Background(), cudn.Name, metav1.GetOptions{})
					Expect(err).NotTo(HaveOccurred())
					return normalizeConditions(cudn.Status.Conditions)
					...
					Status:  "False",
					Reason:  "NetworkAttachmentDefinitionSyncError",
					Message: "failed to generate NetworkAttachmentDefinition: failed to render CNI network config: EVPN transport requires evpnConfiguration to be set",
				}}), "should fail with sync error (not VTEPNotFound) when EVPNConfiguration is nil")
