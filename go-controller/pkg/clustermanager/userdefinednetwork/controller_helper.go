...
//
// Thread-safety: This function is safe to call concurrently from multiple reconcile
// goroutines. The underlying ID allocator is thread-safe and uses
// atomic operations to prevent duplicate VID allocation and is idempotent for the same key.
func (c *Controller) allocateEVPNVIDsIfNeeded(obj client.Object) ([]template.RenderOption, error) {
	...
	if evpnCfg.MACVRF != nil {
		...
		vid, err := c.vidAllocator.AllocateID(key)
		if err != nil {
