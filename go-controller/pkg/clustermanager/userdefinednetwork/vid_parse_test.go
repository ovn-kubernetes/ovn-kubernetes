...
var _ = Describe("parseEVPNVIDs", func() {
	...
	DescribeTable("should handle various configs",
		...
		Entry("EVPN with both VRFs", `{"transport":"evpn","evpn":{"macVRF":{"vid":10},"ipVRF":{"vid":20}}}`, 10, 20, false),
		Entry("EVPN with MAC-VRF only", `{"transport":"evpn","evpn":{"macVRF":{"vid":15}}}`, 15, 0, false),
		Entry("EVPN with IP-VRF only", `{"transport":"evpn","evpn":{"ipVRF":{"vid":25}}}`, 0, 25, false),
		Entry("EVPN with spaced JSON", `{"transport": "evpn", "evpn": {"macVRF": {"vid": 30}}}`, 30, 0, false),
		Entry("EVPN with no VIDs", `{"transport":"evpn","evpn":{"vtep":"test"}}`, 0, 0, false),
		...
		// Unicode escape edge cases - verifies JSON parsing handles unicode escapes correctly
		Entry("EVPN with unicode-escaped transport (e as \\u0065)", `{"transport":"\u0065vpn","evpn":{"macVRF":{"vid":50}}}`, 50, 0, false),
		Entry("EVPN with fully unicode-escaped transport", `{"transport":"\u0065\u0076\u0070\u006e","evpn":{"ipVRF":{"vid":60}}}`, 0, 60, false),
		Entry("non-EVPN with unicode escape in other field", `{"name":"\u0041lice","topology":"layer3"}`, 0, 0, false),
	)
})
