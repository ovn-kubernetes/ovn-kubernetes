...
// recoverEVPNVIDs recovers VID allocations from existing EVPN CUDNs.
// Returns an error if VID recovery fails for any EVPN CUDN, which will prevent controller startup.
...
func (c *Controller) recoverEVPNVIDs(cudnNADs cudnToNADs) error {
	for cudnName, entry := range cudnNADs {
		...
		// Skip EVPN CUDNs with no NADs - nothing to recover from.
		if len(entry.nads) == 0 {
			klog.V(4).Infof("EVPN CUDN %s has no NADs, skipping VID recovery", cudnName)
			continue
		}
		...
		for _, nad := range entry.nads {
			...
			// Check if this NAD has EVPN VIDs allocated
			if macVRFVID == 0 && ipVRFVID == 0 {
				continue // Parsed successfully, but no VIDs allocated yet
			}
		...
		// For EVPN CUDNs, we must recover VIDs to prevent route leakage
		if !recovered {
			return fmt.Errorf("VID recovery failed for EVPN CUDN %s: %w. "+
...
//
// Returns an error if:
//   - The network is EVPN but EVPN is nil
...
func (c *Controller) validateEVPNVTEP(cudn *userdefinednetworkv1.ClusterUserDefinedNetwork) error {
	...
	evpnCfg := cudn.Spec.Network.EVPN
	if evpnCfg == nil {
		return fmt.Errorf("EVPN transport requires EVPN to be set")
	}
