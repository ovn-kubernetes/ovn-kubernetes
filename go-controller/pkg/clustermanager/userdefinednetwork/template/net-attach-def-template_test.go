...
var _ = Describe("NetAttachDefTemplate", func() {
	...
	Context("EVPN VID injection", func() {
		It("should inject VIDs into EVPN config when provided via WithEVPNVIDs", func() {
			...
			Expect(netConf.EVPNConfig).NotTo(BeNil(), "evpnConfig should be present")
			Expect(netConf.EVPNConfig.MACVRF).NotTo(BeNil(), "macVRF should be present")
			Expect(netConf.EVPNConfig.MACVRF.VID).To(Equal(12), "macVRF VID should be 12")
			Expect(netConf.EVPNConfig.IPVRF).NotTo(BeNil(), "ipVRF should be present")
			Expect(netConf.EVPNConfig.IPVRF.VID).To(Equal(13), "ipVRF VID should be 13")
		})
		...
		It("should omit VID when zero (VID=0 not injected)", func() {
			...
			Expect(netConf.EVPNConfig).NotTo(BeNil(), "evpnConfig should be present")
			Expect(netConf.EVPNConfig.MACVRF).NotTo(BeNil(), "macVRF should be present")
			Expect(netConf.EVPNConfig.MACVRF.VID).To(Equal(0), "VID should be zero when not injected")
		...
		It("should omit empty RouteTarget in EVPN config", func() {
			...
			Expect(netConf.EVPNConfig.MACVRF.RouteTarget).To(BeEmpty(), "empty routeTarget should unmarshal as empty string")
			...
			Expect(netConf.EVPNConfig.MACVRF.VID).To(Equal(5), "macVRF VID should be 5")
		})
