...
var _ = Describe("NetAttachDefTemplate", func() {
	...
	It("should succeed for secondary Layer2 network when cluster join subnet matches UDN default", func() {
		...
		nad, err := RenderNetAttachDefManifest(testUdn, "test-ns")
		Expect(err).NotTo(HaveOccurred())
		Expect(nad).NotTo(BeNil())

		// Verify the NAD config does not contain a joinSubnet field
		Expect(nad.Spec.Config).NotTo(ContainSubstring("joinSubnet"))
	})
