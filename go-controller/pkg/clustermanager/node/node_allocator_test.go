...
// which is determined by checking if all nodes have the transit router annotation.
func TestSyncNodeNetworkAnnotations_TunnelID(t *testing.T) {
	tests := []struct {
		name                     string
		allNodesUseTransitRouter bool
		nodeUsesTransitRouter    bool
		wantAnnotation           bool
		wantAllocatorID          bool
	}{
		{
			name:                     "skip tunnel ID when all nodes use transit router",
			allNodesUseTransitRouter: true,
			nodeUsesTransitRouter:    true,
			wantAnnotation:           false,
			wantAllocatorID:          false,
		},
		{
			name:                     "allocate tunnel ID when no nodes use transit router",
			allNodesUseTransitRouter: false,
			nodeUsesTransitRouter:    false,
			wantAnnotation:           true,
			wantAllocatorID:          true,
		},
		{
			name:                     "allocate tunnel ID when at least one node doesn't use transit router",
			allNodesUseTransitRouter: false,
			nodeUsesTransitRouter:    true,
			wantAnnotation:           true,
			wantAllocatorID:          true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config.OVNKubernetesFeature.EnableNetworkSegmentation = true
			config.OVNKubernetesFeature.EnableMultiNetwork = true
			// Start with false to allow setTopologyType to determine the mode
			config.Layer2UsesTransitRouter = false
			...
			// Set transit router annotation based on test case
			if tt.nodeUsesTransitRouter {
				node1.Annotations[util.Layer2TopologyVersion] = util.TransitRouterTopoVersion
			}

			// Create a second node to simulate cluster state
			node2 := &corev1.Node{
				ObjectMeta: metav1.ObjectMeta{
					Name:        "node2",
					Annotations: map[string]string{},
			...
			if tt.allNodesUseTransitRouter {
				node2.Annotations[util.Layer2TopologyVersion] = util.TransitRouterTopoVersion
			}
			...
			na := &NodeAllocator{
				nodeLister:             newFakeNodeLister([]*corev1.Node{node1, node2}),
				kube:                   kube,
				netInfo:                netInfo,
				networkID:              1,
				clusterSubnetAllocator: NewSubnetAllocator(),
			...
			// Call setTopologyType to determine the cluster topology mode
			if err := na.setTopologyType(); err != nil {
				t.Fatalf("setTopologyType failed: %v", err)
			}

			// Verify that config.Layer2UsesTransitRouter matches expected state
			if config.Layer2UsesTransitRouter != tt.allNodesUseTransitRouter {
				t.Fatalf("expected Layer2UsesTransitRouter=%v, got %v", tt.allNodesUseTransitRouter, config.Layer2UsesTransitRouter)
			}

			// Verify HasNodeTunnelIDAllocation returns the expected value
			expectedHasTunnelID := !tt.allNodesUseTransitRouter
