...
func NewController(
	...
	c.nodeEventHandler, err = wf.NodeCoreInformer().Informer().AddEventHandler(
		cache.ResourceEventHandlerFuncs{
			...
			DeleteFunc: func(_ interface{}) { c.configReconciler.Reconcile("") },
		})
	if err != nil {
...
func (c *Controller) Stop() {
	...
	if c.nodeEventHandler != nil {
		_ = c.wf.NodeCoreInformer().Informer().RemoveEventHandler(c.nodeEventHandler)
	}
	if c.raEventHandler != nil {
		_ = c.wf.RouteAdvertisementsInformer().Informer().RemoveEventHandler(c.raEventHandler)
	}
	if c.frrEventHandler != nil {
		_ = c.wf.FRRConfigurationsInformer().Informer().RemoveEventHandler(c.frrEventHandler)
	}
...
func (c *Controller) raNeedsUpdate(oldRA, newRA *ratypes.RouteAdvertisements) bool {
	if !isManagedRA(newRA) {
		return false
	}
	// Ignore events from our own writes.
	// The nil check is a safety guard; newRA should never be nil here as delete events bypass raNeedsUpdate.
...
func (c *Controller) frrNeedsUpdate(oldFRR, newFRR *frrtypes.FRRConfiguration) bool {
	...
	// Ignore events from our own writes.
	// The nil check is a safety guard; newFRR should never be nil here as delete events bypass frrNeedsUpdate.
