...
func UpdateNetworkConnectSubnetAnnotation(cnc *networkconnectv1.ClusterNetworkConnect, cncClient networkconnectclientset.Interface, allocatedSubnets map[string][]*net.IPNet) error {
	...
	if klog.V(5).Enabled() {
		applyJSON, _ := json.Marshal(applyObj)
		klog.V(5).Infof("DEBUG: applyObj JSON=%s", string(applyJSON))
	}
	result, err := cncClient.K8sV1().ClusterNetworkConnects().Apply(context.TODO(), applyObj,
	...
	klog.V(5).Infof("DEBUG: subnetresult.Annotations=%v", result.Annotations)
...
func UpdateNetworkConnectRouterTunnelKeyAnnotation(cncName string, cncClient networkconnectclientset.Interface, tunnelID int) error {
	...
	if klog.V(5).Enabled() {
		applyJSON, _ := json.Marshal(applyObj)
		klog.V(5).Infof("DEBUG: tunnel applyObj JSON=%s", string(applyJSON))
	}
	result, err := cncClient.K8sV1().ClusterNetworkConnects().Apply(
		context.TODO(),
		applyObj,
	...
	klog.V(5).Infof("DEBUG: tunnel result.Annotations=%v", result.Annotations)
