...
func TestRouterAdvertisementWithMultiplePIOs(t *testing.T) {
	...
	// Test that PrefixInfos are correctly stored
	assert.Len(t, ra.PrefixInfos, 2, "Should have 2 PrefixInfos")
	assert.Equal(t, uint32(65535), ra.PrefixInfos[0].ValidLifetime)
	assert.Equal(t, uint32(0), ra.PrefixInfos[0].PreferredLifetime)
	assert.True(t, ra.PrefixInfos[0].OnLink)
	assert.True(t, ra.PrefixInfos[0].Autonomous)

	assert.Equal(t, uint32(3600), ra.PrefixInfos[1].ValidLifetime)
	assert.Equal(t, uint32(1800), ra.PrefixInfos[1].PreferredLifetime)
	assert.True(t, ra.PrefixInfos[1].OnLink)
	assert.False(t, ra.PrefixInfos[1].Autonomous)
}
...
func TestRouterAdvertisementWithNoPrefixInfos(t *testing.T) {
	sourceMAC := net.HardwareAddr{0x00, 0x11, 0x22, 0x33, 0x44, 0x55}
	destinationMAC := net.HardwareAddr{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff}
	sourceIP := net.ParseIP("fe80::211:22ff:fe33:4455")
	destinationIP := net.ParseIP("fe80::aabb:ccff:fedd:eeff")
	...
	ra := RouterAdvertisement{
		...
		DestinationIP:  destinationIP,
		Lifetime:       300,
		PrefixInfos:    nil, // No prefix infos
	}

	// Test that empty PrefixInfos slice doesn't cause issues
	assert.Empty(t, ra.PrefixInfos, "PrefixInfos should be empty")
}
