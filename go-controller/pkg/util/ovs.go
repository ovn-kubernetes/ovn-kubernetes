...
//	        bucket=bucket_id:1,actions=ct(commit,table=6,zone=64003,nat(dst=10.244.0.1:8888))]
func GetOFGroups(bridgeName string) (map[int]string, error) {
	stdout, stderr, err := RunOVSOfctl("dump-groups", bridgeName)
	if err != nil {
	...
	for _, line := range strings.Split(stdout, "\n") {
		if strings.Contains(line, "group_id=") {
			matches := re.FindStringSubmatch(line)
			if len(matches) != 2 {
				return nil, fmt.Errorf("failed to get groups on bridge %q, could not parse line %q", bridgeName, line)
			}
			groupID := matches[1]
			...
			groups[groupIDInt] = strings.TrimSpace(line)
		}
	}
...
func DelOFGroup(bridgeName string, groupID int) error {
	...
	stdout, stderr, err := RunOVSOfctl("del-groups", bridgeName, fmt.Sprintf("group_id=%d", groupID))
	if err != nil {
...
func FlushOFGroup(bridgeName string) error {
	stdout, stderr, err := RunOVSOfctl("del-groups", bridgeName)
	if err != nil {
...
func ReplaceOFGroups(bridgeName string, groups map[int]string) error {
	...
	for existingGroupID := range existingGroups {
		if _, shouldKeep := groups[existingGroupID]; !shouldKeep {
			if err := DelOFGroup(bridgeName, existingGroupID); err != nil {
				return fmt.Errorf("could not delete group %d on bridge %q: %w", existingGroupID, bridgeName, err)
			}
		}
	}
	...
	for groupID, groupString := range groups {
		...
		stdout, stderr, err := runCmd(cmd, runner.ofctlPath, args...)
		if err != nil {
			return fmt.Errorf("could not %s for group %d on bridge %q: stdout: %q, stderr: %q, err: %w",
