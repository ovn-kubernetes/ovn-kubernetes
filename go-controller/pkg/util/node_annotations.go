...
func NodeChassisHostnameAnnotationChanged(oldNode, newNode *corev1.Node) bool {
	oldChassisHostname, ok := oldNode.Annotations[OvnNodeChassisHostname]
	if ok && oldChassisHostname != "" {
		newChassisHostname, ok := newNode.Annotations[OvnNodeChassisHostname]
		if ok && newChassisHostname != "" {
			return oldChassisHostname != newChassisHostname
		}
	}
	return false
}
