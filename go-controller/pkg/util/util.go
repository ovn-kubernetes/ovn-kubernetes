...
// GetEndpointsForService extracts load balancer endpoints from EndpointSlices for a given Service.
// It returns two maps.
...
func GetEndpointsForService(endpointSlices []*discoveryv1.EndpointSlice, service *corev1.Service,
	...
	for portName, protocolMap := range targetEndpoints {
		for protocol, portNumberMap := range protocolMap {
			...
			if service != nil {
				if !validServicePortKeys[slicePortKey] {
					continue
				}
			}
	...
	if needsGlobalEndpoints {
		klog.V(5).Infof("Global endpoints for %s: %v", serviceString, globalEndpoints)
	}
	if needsLocalEndpoints {
		klog.V(5).Infof("Local endpoints for %s: %v", serviceString, localEndpoints)
	}
...
func newTargetEndpoints(slices []*discoveryv1.EndpointSlice) targetEndpoints {
	...
	for _, slice := range slices {
		...
		for _, slicePort := range slice.Ports {
			// Protocol and Port may be nil per API; ignore invalid entries.
