...
// IP addresses will we converted into net.IP representation before being compared. This means that invalid IP addresses
...
func (le LBEndpoints) Equals(other LBEndpoints) bool {
	...
	for _, ip := range le.V4IPs {
		...
		for _, ipOther := range other.V4IPs {
			if ipParsed.Equal(net.ParseIP(ipOther)) {
				found = true
				break
			}
		}
