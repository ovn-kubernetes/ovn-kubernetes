...
func GetNadFromIndexedNADKey(nadNameWithIndex string) (string, int, error) {
	parts := strings.Split(nadNameWithIndex, "/")

	if len(parts) == 3 {
		// Attempt to parse the integer part
		num, err := strconv.Atoi(parts[2])
		if err != nil {
			return "", 0, fmt.Errorf("malformed index for NAD key %s: %v", nadNameWithIndex, err)
		}
	...
	return "", 0, fmt.Errorf("malformed NAD key %s, expect in the form of namespace/name{/index}", nadNameWithIndex)
}
...
//	        for the first attachment of a NAD, and "namespace/name/<idx>" for additional attachments
//	        of the same NAD on the same pod. Note multiple NADs of the same network are allowed
//		    on one pod. They can be of different NAD Name or the same NAD Name.
