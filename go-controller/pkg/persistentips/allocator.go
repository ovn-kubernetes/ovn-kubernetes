...
type PersistentAllocations interface {
	...
	UpdateIPAMClaimStatus(ipamClaim *ipamclaimsapi.IPAMClaim, podAnnotation *util.PodAnnotation, podName string, ipReleaser IPReleaser, allocationErr error)
}
...
// UpdateIPAMClaimStatus updates the IPAM claim status and reconciles it.
...
func (icr *IPAMClaimReconciler) UpdateIPAMClaimStatus(
	...
	if reconcileErr := icr.Reconcile(ipamClaim, updatedClaim, ipReleaser); reconcileErr != nil {
		klog.Errorf("Failed to reconcile IPAM claim %s/%s: %v", ipamClaim.Namespace, ipamClaim.Name, reconcileErr)
	}
}
