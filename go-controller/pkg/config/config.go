...
// These are used for parsing INI files only. For all internal comparisons,
// use types.NetworkTransport* constants which are in canonical lowercase format.
const (
...
type DefaultConfig struct {
	...
	// INI config accepts: "geneve" or "no-overlay" (converted to canonical lowercase internally).
	// Internally stored in canonical format: "geneve" or "nooverlay". Defaults to "geneve".
...
func buildDefaultConfig(cli, file *config) error {
	...
	if Default.Transport == "" {
		...
		// Translate from INI format to canonical format
		Default.Transport = types.TransportFromINI(Default.Transport)
	}
