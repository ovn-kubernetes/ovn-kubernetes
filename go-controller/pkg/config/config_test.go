...
var _ = Describe("Config Operations", func() {
	...
	It("uses expected defaults", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			gomega.Expect(Tracing.Endpoint).To(gomega.Equal(""))
			gomega.Expect(Tracing.Insecure).To(gomega.BeTrue())
...
`), 0o644)
	...
	It("rejects enabled tracing without endpoint", func() {
		err := os.WriteFile(cfgFile.Name(), []byte(`[ovnkubernetesfeature]
enable-tracing=true
`), 0o644)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		app.Action = func(ctx *cli.Context) error {
			...
			gomega.Expect(err).To(gomega.MatchError(
				gomega.ContainSubstring("tracing endpoint must be configured when enable-tracing is true")),
			)
			return nil
		}
		cliArgs := []string{app.Name, "-config-file=" + cfgFile.Name()}
