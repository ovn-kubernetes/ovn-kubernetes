...
vswitchd-pid=/custom/ovs/vswitchd.pid
ovsdb-server-pid=/custom/ovs/ovsdb-server.pid
...
var _ = Describe("Config Operations", func() {
	...
	It("overrides defaults with config file options", func() {
		...
		app.Action = func(ctx *cli.Context) error {
			...
			gomega.Expect(OvsPaths.OvsDbServerPid).To(gomega.Equal("/custom/ovs/ovsdb-server.pid"))
