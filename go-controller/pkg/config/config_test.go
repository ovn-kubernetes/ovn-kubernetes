...
enable-custom-network-configuration=false
...
var _ = Describe("Config Operations", func() {
	...
	It("uses expected defaults", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			gomega.Expect(OVNKubernetesFeature.EnableCustomNetworkConfig).To(gomega.BeFalse())
	...
	It("overrides defaults with config file options", func() {
		...
		err = writeTestConfigFile(cfgFile.Name(), "kubeconfig="+kubeconfigFile, "cacert="+kubeCAFile,
			...
			"enable-custom-network-configuration=true",
		...
		app.Action = func(ctx *cli.Context) error {
			...
			gomega.Expect(OVNKubernetesFeature.EnableCustomNetworkConfig).To(gomega.BeTrue())
