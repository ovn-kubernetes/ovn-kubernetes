...
func (pr *PodRequest) updateDHCPAndDPUAnnotations(clientset *ClientSet, kubecli kube.Interface, pod *corev1.Pod,
	...
	// The retry loop below re-fetches the pod by namespace/name on every
	// attempt. If the pod is deleted and recreated with the same name while
	// this request is in flight, that lookup returns the new pod instance and
	// the annotations staged for the old instance would land on it: the new
	// pod's own ADD would then reuse the stale MAC instead of minting its
	// own. Verify identity on every attempt; a mismatch fails the ADD without
	// writing (the error is not retryable).
	expectedUID := pod.UID
	updateFn := func(pod *corev1.Pod) (*corev1.Pod, func(), error) {
		if pod.UID != expectedUID {
			return nil, nil, fmt.Errorf("pod %s/%s was replaced while staging CNI annotations: "+
				"expected UID %q, found UID %q", pr.PodNamespace, pr.PodName, expectedUID, pod.UID)
		}
		if dhcpEntry != nil {
