...
// VFIO driver handoff), lease markers for cmdDel and result/annotation
// reporting.
...
// nodes, which have no host OVS to record them in.
...
func healInterruptedVFIOHandoff(deviceID string) error {
	...
	if currentDriver != vfioPCIDriver {
		...
		// bindPCIDeviceDriver ends with the override cleared so restore it
		if err := writePCIDeviceDriverOverride(deviceID, vfioPCIDriver); err != nil {
			klog.Warningf("Failed to restore the %s pin on device %s after the repair: %v",
				vfioPCIDriver, deviceID, err)
		}
	}
	// device on vfio-pci with a journal left over means the crash hit after
	// the rebind but before the journal removal
...
func (pr *PodRequest) execDHCPAdd() (*current.Result, error) {
	...
	if dropped := filterOutDefaultRoutes(dhcpResult); len(dropped) > 0 {
	...
	// netns. No lease release on failure: the lease marker recorded before
	// this exchange guarantees the DEL the runtime issues for the failed
	// sandbox releases the lease.
...
// recordDHCPLeaseMarker records that the daemon holds a lease for this
// sandbox, letting cmdDel decide the release from node-local state alone.
// Full mode stores it in the host OVS interface external_ids and DPU-host has
// no host OVS and uses a local file instead.
...
// lease for this sandbox. The node-local marker decides this even when the
// pod is already gone from the apiserver (e.g. force delete). (false, nil)
// means there is provably nothing to release: a repeat DEL, or a KubeVirt VM
// pod, which never records one. A lookup failure is an error, never false:
// treating "couldn't check" as "no lease" would skip the RELEASE while the
// teardown deletes the marker, leaking the lease with no retry path.
...
// no-op there and on DPU-host it unlinks the file instead.
...
// The per-IP Gateway field is not turned into a default route, because the
// not turned into a default route, because the dhcp daemon has already translated
...
func (pr *PodRequest) runLocalnetVFIODHCP() (dhcpResult *current.Result, retErr error) {
	...
	defer func() {
		if err := dhcpOps.BindPCIDeviceDriver(deviceID, vfioPCIDriver); err != nil {
			...
			// vfio devices bound across reprobes
			if err := writePCIDeviceDriverOverride(deviceID, vfioPCIDriver); err != nil {
				klog.Warningf("Failed to restore the %s pin on device %s: %v",
					vfioPCIDriver, deviceID, err)
			}
			// the device is back on vfio-pci and the handoff is concluded
			removeVFIOHandoffJournal(deviceID)
		}
	}()
...
func acquireDHCPLease(ctx context.Context, ifName string, hwAddr net.HardwareAddr) (*current.Result, *nclient4.Lease, error) {
