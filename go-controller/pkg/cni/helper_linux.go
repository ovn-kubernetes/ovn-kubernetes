		...
		if len(netdevice) != 0 {
			...
			err = netns.Do(func(_ ns.NetNS) error {
				...
				if ifInfo.IsDPUHostMode && ifInfo.IsSimulatedDPU {
					...
					// annotations are already gone (see rescueSimulatedNetdevs).
...
// rescueSimulatedNetdevs returns any simulated netdevices still present in the
...
// setup time, or from the link name itself; a link whose original name cannot
// be determined is left in place, since the simulated allocator resolves
// devices strictly by name and could never find it again anyway. Renaming
// before the move keeps retries idempotent: a link already renamed by an
// interrupted rescue is recognized again through its name or alias.
func rescueSimulatedNetdevs(netns, hostNS ns.NetNS) error {
	return netns.Do(func(_ ns.NetNS) error {
		...
		for _, link := range links {
			...
			if name != primaryUDNIfName && !util.IsSimulatedNetdevName(name) && !util.IsSimulatedNetdevName(alias) {
				continue
			}
			var oldName string
			switch {
...
func (*defaultPodRequestInterfaceOps) UnconfigureInterface(pr *PodRequest, ifInfo *PodInterfaceInfo) error {
	...
	if !pr.IsVFIO {
		...
		if ifInfo.IsDPUHostMode && ifInfo.IsSimulatedDPU && !isSecondary {
			...
			// namespace teardown.
			if err := rescueSimulatedNetdevs(netns, hostNS); err != nil {
