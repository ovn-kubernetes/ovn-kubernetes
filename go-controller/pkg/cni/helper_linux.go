...
type CNIPluginLibOps interface {
	AddRoute(ipn *net.IPNet, gw net.IP, dev netlink.Link, mtu int, priority int) error
...
func (defaultCNIPluginLibOps) AddRoute(ipn *net.IPNet, gw net.IP, dev netlink.Link, mtu int, priority int) error {
	route := &netlink.Route{
...
func setupNetwork(link netlink.Link, ifInfo *PodInterfaceInfo) error {
	...
	for _, route := range ifInfo.Routes {
		if err := cniPluginLibOps.AddRoute(route.Dest, route.NextHop, link, ifInfo.RoutableMTU, route.Priority); err != nil {
