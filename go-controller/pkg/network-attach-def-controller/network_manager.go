...
func (nm *networkManagerImpl) syncAll() error {
	...
	for _, network := range validNetworks {
		err := nm.syncNetwork(network.GetNetworkName())
		if errors.Is(err, ErrNetworkControllerTopologyNotManaged) {
			klog.V(5).Infof("Ignoring network %q since %q does not manage it", network.GetNetworkName(), nm.name)
			continue
		}
		if err != nil {
	...
	for raName := range raNames {
		...
		for _, node := range nodes {
			if node.Name == nm.node || util.GetNodeZone(node) == nm.zone {
				vrfs[node.Name] = append(vrfs[node.Name], vrf)
			}
		}
	}
...
func (nm *networkManagerImpl) hasRouteAdvertisements() bool {
	...
	return util.IsRouteAdvertisementsEnabled() && !isClusterManager
}
