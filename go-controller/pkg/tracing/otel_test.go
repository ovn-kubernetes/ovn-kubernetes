...
func TestStartLinkedSpanCreatesRootWithLink(t *testing.T) {
	...
	ctx, span := StartLinkedSpan(ctx, "add", map[string]string{
...
func TestStartLinkedSpanWithoutAnnotationCreatesRootWithoutLinks(t *testing.T) {
	sr := &endedSpanRecorder{}
	tp := sdktrace.NewTracerProvider(sdktrace.WithSpanProcessor(sr))
	oldTP := otel.GetTracerProvider()
	otel.SetTracerProvider(tp)
	...
	ctx := ContextWithSpanNamePrefix(context.Background(), "test.component")
	_, span := StartLinkedSpan(ctx, "add", nil)
	span.End()
	...
	got := spans[0]
	if got.Parent().IsValid() {
		t.Fatalf("expected root span without parent")
	}
	if len(got.Links()) != 0 {
		t.Fatalf("expected no span links, got %d", len(got.Links()))
	}
}