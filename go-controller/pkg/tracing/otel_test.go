...
func TestSpanContextFromPodAnnotationsAcceptsFutureVersionExtensions(t *testing.T) {
	const traceID = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	const upstreamSpanID = "bbbbbbbbbbbbbbbb"
	sc, ok := SpanContextFromPodAnnotations(map[string]string{
		TraceparentAnnotation: "01-" + traceID + "-" + upstreamSpanID + "-01-extension",
	})
	if !ok {
		t.Fatalf("expected future-version traceparent with extensions to be parsed")
	}
	if got := sc.TraceID().String(); got != traceID {
		t.Fatalf("expected trace ID %q, got %q", traceID, got)
	}
...
func TestSpanContextFromPodAnnotationsIgnoresMissingOrInvalidTraceparent(t *testing.T) {
	tests := []struct {
		...
		{
			name: "forbidden version ff",
			annotations: map[string]string{
				TraceparentAnnotation: "ff-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-bbbbbbbbbbbbbbbb-01",
			},
		},
		{
			name: "version 00 rejects trailing extension fields",
			annotations: map[string]string{
				TraceparentAnnotation: "00-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-bbbbbbbbbbbbbbbb-01-extra",
			},
		},
		{
			name: "uppercase hex",
			annotations: map[string]string{
				TraceparentAnnotation: "00-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA-bbbbbbbbbbbbbbbb-01",
			},
		},
	}
...
// TestSpanContextFromPodAnnotationsAcceptsForwardCompatibleTraceparent guards the
// forward compatibility the W3C spec requires: a higher version must still be
// parsed using the fields version 00 defines, and unknown trailing fields must be
// ignored rather than rejected. A hand-rolled parser that splits on "-" and
// demands exactly four fields silently drops such headers.
func TestSpanContextFromPodAnnotationsAcceptsForwardCompatibleTraceparent(t *testing.T) {
	const traceID = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	const upstreamSpanID = "bbbbbbbbbbbbbbbb"
	...
	tests := []struct {
		...
		{
			name:        "future version",
			traceparent: "01-" + traceID + "-" + upstreamSpanID + "-01",
		},
		{
			name:        "future version with extension field",
			traceparent: "01-" + traceID + "-" + upstreamSpanID + "-01-extension",
		},
		{
			name:        "max supported version",
			traceparent: "fe-" + traceID + "-" + upstreamSpanID + "-01",
		},
		{
			// annotations written by hand or templated through YAML routinely
			// arrive with surrounding whitespace
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sc, ok := SpanContextFromPodAnnotations(map[string]string{
				TraceparentAnnotation: tt.traceparent,
			})
			if !ok {
				t.Fatalf("expected traceparent %q to be parsed", tt.traceparent)
			}
			if !sc.IsRemote() {
				t.Fatalf("expected span context to be marked remote")
			}
...
func TestSpanContextFromPodAnnotationsUsesConfiguredAnnotationKey(t *testing.T) {
	oldAnnotationKey := traceparentAnnotationKey
	traceparentAnnotationKey = "example.com/traceparent"
	defer func() { traceparentAnnotationKey = oldAnnotationKey }()
...
func TestStartTraceCreatesRootWithLink(t *testing.T) {
	...
	oldEnableTracing := config.OVNKubernetesFeature.EnableTracing
	config.OVNKubernetesFeature.EnableTracing = true
	defer func() { config.OVNKubernetesFeature.EnableTracing = oldEnableTracing }()
