...
type VTEPStatus struct {
	...
	// Allocations contains the per-node VTEP IP allocations.
	// +optional
	Allocations []VTEPAllocation `json:"allocations,omitempty"`
}

// VTEPAllocation represents a VTEP IP allocation for a specific node.
type VTEPAllocation struct {
	// Node is the name of the node.
	// +kubebuilder:validation:Required
	// +required
	Node string `json:"node"`

	// IP is the VTEP IP allocated to the node.
	// +kubebuilder:validation:Required
