...
type ClusterUserDefinedNetworkSpec struct {
	...
	// +kubebuilder:validation:XValidation:rule="!has(self.noOverlayOptions) || (has(self.transport) && self.transport == 'NoOverlay')", message="noOverlayOptions is only allowed when transport is NoOverlay"
