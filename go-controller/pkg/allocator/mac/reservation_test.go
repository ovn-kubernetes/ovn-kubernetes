...
var _ = Describe("ReservationManager", func() {
	...
	Context("reserve", func() {
		It("should not fail on repeated reservation for the same owner", func() {
			Expect(testMgr.Reserve(owner1, mac1)).To(Succeed())
			Expect(testMgr.Reserve(owner1, mac1)).To(Succeed(), "same owner should not raise a conflict")
		})

		It("should fail reserve existing MAC for different owner", func() {
			Expect(testMgr.Reserve(owner1, mac1)).To(Succeed())
			Expect(testMgr.Reserve(owner2, mac1)).To(MatchError(mac.ErrMACConflict),
