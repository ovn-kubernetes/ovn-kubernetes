...
func Test_allocatePodAnnotationWithRollback(t *testing.T) {
	...
	tests := []struct {
		...
		enableCustomNetworkConfig bool
		role                      string
...

		{
			...
			ipam:                      true,
			enableCustomNetworkConfig: true,
			role:                      types.NetworkRolePrimary, // has to be primary network for default routes to be set
			netConf: &ovncnitypes.NetConf{
		...
		{
			// on networks with IPAM and layer2 topology, expect error when EnableCustomNetworkConfig is false
			name: "expect error, static IP with IPAM on layer2 when EnableCustomNetworkConfig is false",
			ipam: true,
			// enableCustomNetworkConfig defaults to false
			netConf: &ovncnitypes.NetConf{
				Topology: types.Layer2Topology,
				NetConf: cnitypes.NetConf{
					Name: "network",
		...
		{
			// on secondary networks with IPAM and layer2 topology, expect error when EnableCustomNetworkConfig is enabled
			name:                      "expect error, static IP with IPAM on secondary network when EnableCustomNetworkConfig is enabled",
			ipam:                      true,
			enableCustomNetworkConfig: true,
			netConf: &ovncnitypes.NetConf{
				Topology: types.Layer2Topology,
				...
				NADName: util.GetNADName("namespace", "network"),
				Subnets: "192.168.0.0/24,2001:db8::/64",
				Role:    types.NetworkRoleSecondary, // This makes it a secondary network
			},
			args: args{
				network: &nadapi.NetworkSelectionElement{
					IPRequest: []string{"192.168.0.101/24"},
		...
		{
			// on primary networks with IPAM and layer2 topology, expect success when EnableCustomNetworkConfig is enabled
			name:                      "expect success, static IP with IPAM on primary network when EnableCustomNetworkConfig is enabled",
			ipam:                      true,
			enableCustomNetworkConfig: true,
			netConf: &ovncnitypes.NetConf{
				Topology: types.Layer2Topology,
				...
				NADName: util.GetNADName("namespace", "network"),
				Subnets: "192.168.0.0/24,2001:db8::/64",
				Role:    types.NetworkRolePrimary, // This makes it a primary network
			},
			args: args{
				network: &nadapi.NetworkSelectionElement{
					IPRequest: []string{"192.168.0.101/24"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config.OVNKubernetesFeature.EnableCustomNetworkConfig = tt.enableCustomNetworkConfig
