...
func NewClusterManagerWatchFactory(ovnClientset *util.OVNClusterManagerClientset) (*WatchFactory, error) {
	...
	if util.IsNetworkSegmentationSupportEnabled() {
		...
		// Initialize VTEP factory for EVPN support if VTEPClient is available.
		// NOTE: This assumes the VTEP CRD is deployed when Network Segmentation is enabled.
		if ovnClientset.VTEPClient != nil {
