...
// A burst of updates for one UID should leave a single slot with the newest snapshot.
...
// Reusing a key for a new UID must keep both resource incarnations in order.
...
// Merging consecutive replacements must retain the original and final UID boundary.
...
// A replacement update and a later deletion must both survive shutdown.
...
// An enqueue already inside the map must not be stranded by concurrent shutdown.
...
// Accepted callbacks are drained before the workers exit.
...
// While a callback is blocked, later updates share one slot and replace its snapshot.
