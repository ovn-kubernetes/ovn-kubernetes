	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			g.Eventually(func() []callbackEvent {
				...
				defer gotMu.Unlock()
				return got
			}, 3*time.Second, 100*time.Millisecond).Should(gomega.Equal(tt.expectAdds))
			...
			g.Eventually(func() []callbackEvent {
				...
				defer gotMu.Unlock()
				return got
			}, 3*time.Second, 100*time.Millisecond).Should(gomega.Equal(expectedFinal))

			g.Consistently(func() []callbackEvent {
				gotMu.Lock()
				defer gotMu.Unlock()
				return got
			}, 500*time.Millisecond, 100*time.Millisecond).Should(gomega.Equal(expectedFinal))
