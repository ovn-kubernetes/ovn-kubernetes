...
type podEntry struct {
	...
	// queueing fails afterward: the handler may already have released IPAM.
...
func (c *Controller) teardown(handler NetworkHandler, podKey, netName string, fallbackPod *corev1.Pod, observationSequence uint64, lifecycleComplete bool) error {
	...
	if deletePod != nil {
		c.stateMu.RLock()
		complete := c.entries[netName][podKey].teardownComplete
		c.stateMu.RUnlock()
		if !complete {
			...
			c.stateMu.Lock()
			c.ensureEntryLocked(netName, podKey).teardownComplete = true
			c.stateMu.Unlock()
		}
