...
set_common_default_params() {
  ...
  if [[ $ENABLE_UPLINK == true && ($ENABLE_MULTI_NET != true || $ENABLE_NETWORK_SEGMENTATION != true) ]]; then
    echo "Uplink requires multi-network (-mne) and network-segmentation (-nse) to be enabled"
    exit 1
  fi
