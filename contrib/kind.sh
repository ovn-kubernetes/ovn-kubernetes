...
usage() {
    ...
    echo "                 [-uae | --pre-conf-udn-addr-enable]"
    ...
    echo "-uae | --pre-conf-udn-addr-enable     Enable connecting workloads with custom network configuration to user-defined networks"
        ...
        case $1 in
           ...
           -cm  | --compact-mode )              OVN_COMPACT_MODE=true
            ...
            -nse | --network-segmentation-enable) ENABLE_NETWORK_SEGMENTATION=true
                                                  ;;
            -uae | --pre-conf-udn-addr-enable) ENABLE_PRE_CONF_UDN_ADDR=true
                                                  ;;
...
create_ovn_kube_manifests() {
  ...
  ./daemonset.sh \
    ...
    --pre-conf-udn-addr-enable="${ENABLE_PRE_CONF_UDN_ADDR}" \
