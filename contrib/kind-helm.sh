...
# run_script_in_container should be used when kind-helm.sh is run nested in a container
...
create_kind_cluster() {
  # Use the same templating mechanism as kind.sh
  KIND_CONFIG=${KIND_CONFIG:-${DIR}/kind.yaml.j2}
  KIND_CONFIG_LCL=${DIR}/kind-${KIND_CLUSTER_NAME}.yaml

  # Generate the KIND configuration using Jinja templating
...
helm install ovn-kubernetes . -f "${value_file}" \
          ...
          --set global.disablePktMTUCheck=$(if [ "${OVN_DISABLE_PKT_MTU_CHECK}" == "true" ]; then echo "true"; else echo "false"; fi) \
          ...
          --set global.enableInterconnect=$(if [ "${OVN_ENABLE_INTERCONNECT}" == "true" ]; then echo "true"; else echo "false"; fi) \
