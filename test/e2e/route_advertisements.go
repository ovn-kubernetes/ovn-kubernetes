...
var _ = ginkgo.Describe("BGP: When default podNetwork is advertised", feature.RouteAdvertisements, func() {
	...
	ginkgo.When("a client ovnk pod is created", func() {
		...
		ginkgo.It("can connect to an external server and another cluster node after toggling default network advertisement off and back on", ginkgo.Serial, func() {
			...
			// Check if outbound-snat is enabled in ovnkube-config
			isOutboundSNATEnabled := isOutboundSNATEnabled(f)
			...
			gomega.Eventually(func() error {
				for _, serverIP := range serverContainerIPs {
					...
					if isOutboundSNATEnabled {
						// If outbound-snat is enabled, expect node IP
						for _, nodeIP := range clientPodNodeIPs {
							if nodeIP != "" && utilnet.IsIPv6String(nodeIP) == isV6 {
								expectedIP = nodeIP
								break
							}
						}
					} else {
						// If outbound-snat is disabled (default for this test scenario previously), expect pod IP
...
func isOutboundSNATEnabled(f *framework.Framework) bool {
	if !isNoOverlayEnabled() {
		return false
	}
	configMap, err := f.ClientSet.CoreV1().ConfigMaps(deploymentconfig.Get().OVNKubernetesNamespace()).Get(context.TODO(), "ovnkube-config", metav1.GetOptions{})
	if err != nil {
		framework.Logf("Failed to get ovnkube-config: %v", err)
		return false
	}
	if conf, ok := configMap.Data["ovnkube.conf"]; ok {
