...
var _ = ginkgo.DescribeTableSubtree("BGP: isolation between advertised networks",
	func(cudnATemplate, cudnBTemplate *udnv1.ClusterUserDefinedNetwork) {
		...
		ginkgo.DescribeTable("connectivity between networks",
			...
			ginkgo.Entry("UDN pod to the same node nodeport service in default network should work",
				func(ipFamilyIndex int) (clientName string, clientNamespace string, dst string, expectedOutput string, expectErr bool) {
					clientPod := podsNetA[0]
			...
			ginkgo.Entry("UDN pod to the same node nodeport service in same UDN should work",
				func(ipFamilyIndex int) (clientName string, clientNamespace string, dst string, expectedOutput string, expectErr bool) {
					clientPod := podsNetA[0]
			...
			ginkgo.Entry("UDN pod to a different node nodeport service in same UDN should work",
				func(ipFamilyIndex int) (clientName string, clientNamespace string, dst string, expectedOutput string, expectErr bool) {
					clientPod := podsNetA[0]
					// The service is backed by pods in podsNetA.
			...
			ginkgo.Entry("UDN pod to the same node nodeport service in different UDN should work",
				func(ipFamilyIndex int) (clientName string, clientNamespace string, dst string, expectedOutput string, expectErr bool) {
					clientPod := podsNetA[0]
			...
			ginkgo.Entry("UDN pod to a different node nodeport service in different UDN should work",
				func(ipFamilyIndex int) (clientName string, clientNamespace string, dst string, expectedOutput string, expectErr bool) {
					clientPod := podsNetA[0]
					// The service is backed by podNetB.
