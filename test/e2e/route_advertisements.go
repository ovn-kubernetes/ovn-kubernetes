...
var _ = ginkgo.Describe("BGP: For a VRF-Lite configured network", func() {
	...
	testPodToHostnameAndExpect := func(src *corev1.Pod, dstIP, expect string) {
		...
		hostname, err := e2epodoutput.RunHostCmdWithRetries(
			src.Namespace,
			src.Name,
			fmt.Sprintf("curl --max-time 2 -g -q -s http://%s/hostname", net.JoinHostPort(dstIP, "8080")),
			framework.Poll,
			timeout,
		)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	testPodToClientIP := func(src *corev1.Pod, dstIP string) {
		...
		_, err := e2epodoutput.RunHostCmdWithRetries(
			src.Namespace,
			src.Name,
			fmt.Sprintf("curl --max-time 2 -g -q -s http://%s/clientip", net.JoinHostPort(dstIP, "8080")),
			framework.Poll,
			timeout,
		)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	testContainerToClientIPAndExpect := func(src, dstIP, expect string) {
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			ip, err := infraprovider.Get().ExecK8NodeCommand(
				src,
				[]string{"curl", "--max-time", "2", "-g", "-q", "-s", fmt.Sprintf("http://%s/clientip", net.JoinHostPort(dstIP, "8080"))},
			)
	...
	var (
		layer3NetworkSpec = &udnv1.NetworkSpec{
			...
			Layer3: &udnv1.Layer3Config{
				Role: "Primary",
				Subnets: generateL3Subnets(
					udnv1.Layer3Subnet{CIDR: cudnCIDRv4, HostSubnet: 24},
					udnv1.Layer3Subnet{CIDR: cudnCIDRv6, HostSubnet: 64}),
			},
		}
	)
					...
					var (
						otherLayer3NetworkSpec = &udnv1.NetworkSpec{
							...
							Layer3: &udnv1.Layer3Config{
								Role: "Primary",
								Subnets: generateL3Subnets(
									udnv1.Layer3Subnet{CIDR: otherUDNCIDRv4, HostSubnet: 24},
									udnv1.Layer3Subnet{CIDR: otherUDNCIDRv6, HostSubnet: 64}),
							},
						}
						otherLayer2NetworkSpec = &udnv1.NetworkSpec{
							...
							Layer2: &udnv1.Layer2Config{
								Role:    "Primary",
								Subnets: generateL2Subnets(otherUDNCIDRv4, otherUDNCIDRv6),
							},
						}
					)
											...
											func(family utilnet.IPFamily) {
												...
												gomega.Consistently(func(g gomega.Gomega) {
													_, err := e2epodoutput.RunHostCmd(
														testPod.Namespace,
														testPod.Name,
														fmt.Sprintf("curl --max-time 2 -g -q -s http://%s/clientip", net.JoinHostPort(clusterIP, "8080")),
													)
													g.Expect(err).To(gomega.HaveOccurred())
												}).WithTimeout(timeoutNOK).WithPolling(framework.Poll).Should(gomega.Succeed())
											},
...
func runBGPNetworkAndServer(
	...
	families := getSupportedIPFamilies()
