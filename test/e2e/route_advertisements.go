...
var _ = ginkgo.Describe("BGP: Pod to external server when UDN Layer3 Network is advertised", func() {
	const (
		...
		primaryNetworkName     = "kind"
		bgpExternalNetworkName = "bgpnet"
		testUdnName            = "bgp-udn-layer3-network"
	...
	ginkgo.When("a client ovnk pod targeting an external server is created", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			ginkgo.By("create layer3 UserDefinedNetwork")
			udnManifest := `
...
`
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			gomega.Eventually(userDefinedNetworkReadyFunc(f.DynamicClient, f.Namespace.Name, testUdnName), 5*time.Second, time.Second).Should(gomega.Succeed())
		...
		ginkgo.It("tests are run towards the external agnhost echo server", func() {
			...
			ginkgo.By("routes to the UDN network are advertised to external frr router")
			// Get the first element in the advertisements array (assuming you want to check the first one)
			ginkgo.By("create route advertisement matching UDN Network")
			raManifest := generateRAManifest(testRAName)
			cleanup, err := createManifest(f.Namespace.Name, raManifest)
			ginkgo.DeferCleanup(cleanup)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())

			ginkgo.By("ensure route advertisement matching UDN was created successfully")
			gomega.Eventually(func() string {
			...
			podIP, err := podIPsForUserDefinedPrimaryNetwork(cs, f.Namespace.Name, clientPod.Name, namespacedName(f.Namespace.Name, testUdnName), 0)
			framework.ExpectNoError(err, fmt.Sprintf("Getting podIPs for pod %s failed: %v", clientPod.Name, err))
			...
			for _, serverContainerIP := range serverContainerIPs {
				...
				framework.ExpectNoError(err, fmt.Sprintf("Testing pod to external traffic failed: %v", err))
				if isIPv6Supported() && utilnet.IsIPv6String(serverContainerIP) {
					podIP, err := podIPsForUserDefinedPrimaryNetwork(cs, f.Namespace.Name, clientPod.Name, namespacedName(f.Namespace.Name, testUdnName), 1)
					framework.ExpectNoError(err, fmt.Sprintf("Getting podIPs for pod %s failed: %v", clientPod.Name, err))
					// For IPv6 addresses, need to handle the brackets in the output
					outputIP := strings.TrimPrefix(strings.Split(stdout, "]:")[0], "[")
