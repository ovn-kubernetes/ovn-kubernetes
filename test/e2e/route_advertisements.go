...
var _ = ginkgo.Describe("BGP: Pod to external server when default podNetwork is advertised", feature.RouteAdvertisements, func() {
	...
	ginkgo.When("a client ovnk pod targeting an external server is created", func() {
		...
		var clientPod, internalServerPod *corev1.Pod
		var clientPodNodeName, internalServerPodNodeName string
		var err error
		...
		ginkgo.BeforeEach(func() {
			...
			internalServerPodNodeName = nodes.Items[2].Name
			...
			ginkgo.By("Creating an internal server pod on a second node")
			ports := []corev1.ContainerPort{
				{ContainerPort: 8080},
			}
			internalServerPod = e2epod.NewAgnhostPod(f.Namespace.Name, "internal-server-pod", nil, nil, ports, "netexec", "--http-port=8080")
			internalServerPod.Spec.NodeName = internalServerPodNodeName
			e2epod.NewPodClient(f).CreateSync(context.TODO(), internalServerPod)
		})
		...
		ginkgo.It("tests are run towards the external agnhost echo server and toward another pod on another cluster node", func() {
			...
			// Get internal server pod IPs
			var internalServerPodIPs []string
			internalServerPodv4IP, internalServerPodv6IP, err := podIPsForDefaultNetwork(f.ClientSet, f.Namespace.Name, internalServerPod.Name)
			internalServerPodIPs = append(internalServerPodIPs, internalServerPodv4IP)
			internalServerPodIPs = append(internalServerPodIPs, internalServerPodv6IP)
			framework.ExpectNoError(err, fmt.Sprintf("Getting podIPsv4: %s, podIPv6:%s for pod %s failed: %v", internalServerPodv4IP, internalServerPodv6IP, internalServerPod.Name, err))
			framework.Logf("Internal server pod IP address v4=%s, v6=%s", internalServerPodv4IP, podv6IP)

			ginkgo.By("With default network being advertised, queries to the internal server on another node are SNATed (uses nodeIP)")
			// testClientPodToServerConnectivity(internalServerPodIPs, clientNodeIPs, "internal server pod")
			...
			ginkgo.By("After default network is toggled to unadvertised, run test towards the internal server on another node again, egressing packets are SNATed (uses nodeIP)")
			// testClientPodToServerConnectivity(internalServerPodIPs, clientNodeIPs, "internal server pod")
