...
var _ = ginkgo.Describe("BGP: Pod to external server when default podNetwork is advertised", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		framework.ExpectNoError(err, "external container %s network %s information must be available", routerContainerName, providerPrimaryNetwork.GetName())
	...
	ginkgo.When("a client ovnk pod targeting an external server is created", func() {
		...
		ginkgo.It("tests are run towards the external agnhost echo server", func() {
			...
			externalServerV4CIDR, externalServerV6CIDR, err := bgpNetwork.GetIPv4IPv6Subnets()
...
var _ = ginkgo.Describe("BGP: Pod to external server when CUDN network is advertised", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		framework.ExpectNoError(err, "container %s attached to network %s must contain network info", routerContainerName, providerPrimaryNetwork.GetName())
