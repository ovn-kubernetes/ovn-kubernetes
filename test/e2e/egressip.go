...
`
	...
	ginkgo.It("Should fail if egressip-mark annotation is present during EgressIP creation", func() {
		...
		gomega.Expect(err).To(gomega.MatchError(fmt.Sprintf("The egressips \"%s\" is invalid: : ValidatingAdmissionPolicy 'egressip-create-validation' with binding 'egressip-create-validation-binding' denied request: EgressIP resources cannot be created with the \"k8s.ovn.org/egressip-mark\" annotation. This annotation is managed by the system.", egressIPName)), "Should fail if k8s.ovn.org/egressip-mark annotation is present during creation")
	})
