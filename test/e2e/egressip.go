...
`)
		...
		ginkgo.By("7. Check connectivity from pod to another node primary IP and verify that the srcIP is the expected nodeIP or pod IP if advertised")
		expectedIP := egressNodeIP.String()
		if isClusterDefaultNetwork(netConfigParams) && isDefaultNetworkAdvertised() {
			expectedIP = pod1IP.String()
		}
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(hostNetPod, pod1Name, podNamespace.Name,
			true, []string{expectedIP}))
