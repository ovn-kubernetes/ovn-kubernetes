...
`)
		...
		raName := fmt.Sprintf("e2e-eip-expose-ns-%s-eip", f.Namespace.Name)
		raManifest := `
...
spec:
  ...
  advertisements:
    - "EgressIP"`
		...
		ginkgo.DeferCleanup(func() error {
			_, err := e2ekubectl.RunKubectl(f.Namespace.GetName(), "delete", "ra", raName)
			if err != nil {
				return err
			}
			cleanup()
		...
		gomega.Eventually(func() string {
			eipValue, err := e2ekubectl.RunKubectl(f.Namespace.GetName(), "get", "ra", raName, "--template={{index .spec.advertisements 0}}")
			if err != nil {
				return ""
			}
			return eipValue
