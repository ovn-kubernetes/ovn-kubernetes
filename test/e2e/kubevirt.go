...
`
	...
	waitForVMIPodEventFailure := func(vmName, expectedMessageSubstring, errorDescription string) {
		Eventually(func() []corev1.Event {
			...
			)), fmt.Sprintf("VM %s should fail with %s", vmName, errorDescription))
	}
  ...
  eth0:
    ...
    ipv6-address-generation: eui64`,
		...
		It("should fail when creating second VM with duplicate static IP", func() {
			...
			waitForVMIPodEventFailure(vm2.Name, "provided IP is already allocated", "IP allocation error")
...
ethernets:
  eth0:
    ...
    ipv6-address-generation: eui64`,
			...
			waitForVMIPodEventFailure(exhaustedVMName, "subnet address pool exhausted", "subnet exhaustion error")
