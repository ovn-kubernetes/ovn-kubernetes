...
- /tmp/iperf-server.sh`, base64.StdEncoding.EncodeToString([]byte(iperfServerScript)))
		...
		DescribeTable("should keep ip", func(td testData) {
			...
			if td.role == "primary" && td.test.description == liveMigrate.description && isInterconnectEnabled() {
				...
				if isIPv6Supported() {
					step = by(vm.Name, fmt.Sprintf("Checking IPv6 gateway after %s %s", td.resource.description, td.test.description))
					obtainedIPv6Gateways, err := kubevirt.RetrieveIPv6Gateways(vmi)
					Expect(err).NotTo(HaveOccurred())

					sourceNode, err := fr.ClientSet.CoreV1().Nodes().Get(context.Background(), vmi.Status.MigrationState.SourceNode, metav1.GetOptions{})
					Expect(err).NotTo(HaveOccurred(), step)

					sourceNodeIPv6GatewayPath, err := kubevirt.GenerateGatewayIPv6RouterLLA(sourceNode, netConfig.networkName)
					Expect(err).NotTo(HaveOccurred())
					...
					Expect(obtainedIPv6Gateways).To(SatisfyAll(
