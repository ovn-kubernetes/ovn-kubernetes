...
var _ = Describe("Network Segmentation", func() {
	...
	const (
		...
		podClusterNetPort            = 9000
		podClusterNetDefaultPort     = 8080
	...
	Context("a user defined primary network", func() {
			...
			func(createNetworkFn func(c *networkAttachmentConfigParams) error) {
				...
				DescribeTable(
					...
					func(
						...
						nodes, err := e2enode.GetBoundedReadySchedulableNodes(context.Background(), f.ClientSet, 1)
						framework.ExpectNoError(err, "two scheduable nodes is a prerequisite for test")
						node1Name := nodes.Items[0].GetName()
						udnPodConfig.nodeSelector = map[string]string{nodeHostnameKey: node1Name}
							...
							pod.Spec.Containers[0].ReadinessProbe = &v1.Probe{
								ProbeHandler: v1.ProbeHandler{
									HTTPGet: &v1.HTTPGetAction{
										Path: "/healthz",
										Port: intstr.FromInt32(podClusterNetPort),
									},
								},
						...
						_, err = provider.Get().ExecK8NodeCommand(node1Name, []string{"systemctl", "restart", "kubelet"})
						Expect(err).NotTo(HaveOccurred())
						...
						if !isUDNHostIsolationDisabled() {
							...
							hostNetPod, err := createPod(f, "host-net-pod", node1Name,
							...
							for _, destIP := range []string{defaultIPv4, defaultIPv6} {
								...
								Eventually(func() bool {
									_, err := provider.Get().ExecK8NodeCommand(node1Name, []string{
		...
		It("doesn't cause network name conflict", func() {
			...
			By("ensuring 2 schedulable Nodes")
			nodes, err := e2enode.GetBoundedReadySchedulableNodes(context.TODO(), cs, 2)
			framework.ExpectNoError(err, "two scheduable Nodes are required")
			node1Name, node2Name := nodes.Items[0].GetName(), nodes.Items[1].GetName()
			...
			func(createNetworkFn func(c *networkAttachmentConfigParams) error) {
				...
				DescribeTable(
					...
					func(netConfigParams *networkAttachmentConfigParams, clientPodConfig podConfiguration) {
						...
						assertClientExternalConnectivity(clientPodConfig, externalContainer.GetIPv4(), externalContainer.GetIPv6(), podClusterNetPort)
					},
		...
		DescribeTable(
			...
			func(
				...
				for _, testPod := range []*v1.Pod{clientPod, serverPod} {
					...
					Expect(restartOVNKubeNodePod(cs, deployment.Get().OVNKubernetesNamespace(), testPod.Spec.NodeName)).ShouldNot(HaveOccurred(), "restart of OVNKube node pod must succeed")
