...
func changeNodeZone(node *v1.Node, zone string, cs clientset.Interface) error {
	...
	err = restartOVNKubeNodePod(cs, deployment.Get().OVNKubernetesNamespace(), node.Name)
	framework.ExpectNoError(err, "must get OVN-Kubernetes deployment config for Node %s and namespace %s", node.Name, deployment.Get().OVNKubernetesNamespace())
