...
func checkNumberOfETPRules(backendNodeName string, value int, pattern string) wait.ConditionFunc {
	return func() (bool, error) {
		...
		if !isExpected {
			framework.Logf("numberOfETPRules got: numberOfNFTElements: %d, expected: %d",
				numberOfETPRules, value)
		}
...
func checkNumberOfNFTElements(backendNodeName string, value int, name string) wait.ConditionFunc {
	return func() (bool, error) {
		...
		if !isExpected {
			framework.Logf("numberOfNFTElements got: numberOfNFTElements: %d, expected: %d",
				numberOfNFTElements, value)
		}
		return isExpected, nil
	}
}
