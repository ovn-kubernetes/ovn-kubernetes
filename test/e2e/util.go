...
func wgetInExternalContainer(externalContainer provider.ExternalContainer, targetHost string, targetPort int32, endPoint string, maxTime int) (string, error) {
	...
	return provider.Get().ExecExternalContainerCommand(externalContainer, []string{
		"wget", fmt.Sprintf("http://%s:%d/%s", targetHost, targetPort, endPoint),
	})
}
...
func countNFTablesElements(nodeName, name string) int {
	...
	nftElements, err := provider.Get().ExecK8NodeCommand(nodeName, nftCmd)
	...
	ginkgo.JustAfterEach(func() {
		...
		for _, node := range nodes.Items {
			...
			for _, ovsdbLocation := range ovsdbLocations {
				_, err = provider.Get().ExecK8NodeCommand(node.Name, []string{"stat", fmt.Sprintf("%s/%s", ovsdbLocation, ovsdb)})
				if err == nil {
...
func updateIPTablesRulesForNode(op, nodeName string, ipTablesArgs []string, ipv6 bool) {
	...
	_, err := provider.Get().ExecK8NodeCommand(nodeName, append([]string{iptables, "-v", "--check"}, ipTablesArgs...))
	...
	_, err = provider.Get().ExecK8NodeCommand(nodeName, append(args, ipTablesArgs...))
	if err != nil {
...
func GetNodeIPv6LinkLocalAddressForEth0(nodeName string) (string, error) {
	...
	output, err := provider.Get().ExecK8NodeCommand(nodeName, ipCmd)
	if err != nil {
...
func isKernelModuleLoaded(nodeName, kernelModuleName string) bool {
	out, err := provider.Get().ExecK8NodeCommand(nodeName, []string{"lsmod"})
	if err != nil {
