...
func checkConnectionOrNoConnectionToLoadBalancers(f *framework.Framework, clientPod *v1.Pod, service *v1.Service, expectedOutput string, shouldConnect bool) {
	...
	for _, lbIngress := range filterLoadBalancerIngressByIPFamily(service) {
...
func filterLoadBalancerIngressByIPFamily(service *v1.Service) []v1.LoadBalancerIngress {
	...
	// allocate dual stack addresses to the service. However we need to filter
	// out in accordance with the configuration we are running the cluster with.
