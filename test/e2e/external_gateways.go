...
var _ = ginkgo.Describe("External Gateway", func() {
	...
	var _ = ginkgo.Describe("e2e non-vxlan external gateway and update validation", func() {
		...
		ginkgo.It("Should validate connectivity without vxlan before and after updating the namespace annotation to a new external gateway", func() {
			...
			externalContainer := provider.ExternalContainer{Name: gwContainerNameAlt1, Image: images.AgnHost(), Network: primaryProviderNetwork}
			...
			externalContainer2 := provider.ExternalContainer{Name: gwContainerNameAlt2, Image: images.AgnHost(), Network: primaryProviderNetwork}
			externalContainer2, err = providerCtx.CreateExternalContainer(externalContainer2)
			framework.ExpectNoError(err, "failed to start external gateway test container %s", gwContainerNameAlt2)
			...
			if exGWIpAlt2 == "" {
				framework.Failf("failed to retrieve container %s IP address", gwContainerNameAlt2)
			}
			...
			framework.ExpectNoError(err, "failed to add the loopback ip to dev lo on the test container %s", gwContainerNameAlt2)
			providerCtx.AddCleanUpFn(func() error {
				_, err = provider.Get().ExecExternalContainerCommand(externalContainer2, []string{"ip", "address", "del", exGWRemoteCidrAlt2, "dev", "lo"})
				if err != nil {
					return fmt.Errorf("failed to cleanup loopback ip on test container %s: %v", gwContainerNameAlt2, err)
				}
				return nil
			})
			...
			framework.ExpectNoError(err, "failed to add the pod route on the test container %s", gwContainerNameAlt2)
			providerCtx.AddCleanUpFn(func() error {
				_, err = provider.Get().ExecExternalContainerCommand(externalContainer2, []string{"ip", "route", "del", pingSrc, "via", nodeIP})
				if err != nil {
					return fmt.Errorf("failed to cleanup route on test container %s: %v", gwContainerNameAlt2, err)
				}
				return nil
			})
	...
	var _ = ginkgo.Describe("e2e ingress gateway traffic validation", func() {
		...
		ginkgo.It("Should validate ingress connectivity from an external gateway", func() {
			...
			externalContainer := provider.ExternalContainer{Name: gwContainer, Image: images.AgnHost(), Network: primaryProviderNetwork,
				Args: getAgnHostHTTPPortBindCMDArgs(externalContainerPort), ExtPort: externalContainerPort}
			externalContainer, err = providerCtx.CreateExternalContainer(externalContainer)
			framework.ExpectNoError(err, "failed to start external gateway test container %s", gwContainer)
			...
			providerCtx.AddCleanUpFn(func() error {
				...
				if err != nil {
					return fmt.Errorf("failed to cleanup route in external container %s: %v", gwContainer, err)
				}
				return nil
			})
			...
			providerCtx.AddCleanUpFn(func() error {
				...
				if err != nil {
					return fmt.Errorf("failed to cleanup loopback ip on dev lo within test container %s: %v", gwContainer, err)
				}
				return nil
			})
			...
			framework.ExpectNoError(err, "failed to ping the pod address %s from mock container %s", pingDstPod, gwContainer)
		})
	})
	...
	var _ = ginkgo.Context("With annotations", func() {
		...
		var _ = ginkgo.Describe("e2e non-vxlan external gateway through a gateway pod", func() {
			...
			ginkgo.BeforeEach(func() {
				...
				gwContainers, addressesv4, addressesv6 = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork, gwContainer1, gwContainer2,
		...
		var _ = ginkgo.Describe("e2e multiple external gateway stale conntrack entry deletion validation", func() {
			...
			ginkgo.BeforeEach(func() {
				...
				externalContainers, addressesv4, addressesv6 = setupGatewayContainersForConntrackTest(f, providerCtx, nodes, primaryProviderNetwork, gwContainer1, gwContainer2, srcPodName)
		...
		var _ = ginkgo.Context("BFD", func() {
			var _ = ginkgo.Describe("e2e non-vxlan external gateway through an annotated gateway pod", func() {
				...
				ginkgo.BeforeEach(func() {
					...
					gwContainers, addressesv4, addressesv6 = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork, gwContainer1,
						gwContainer2, srcPingPodName, externalUDPPort, externalTCPPort, ecmpRetry, false)
			...
			var _ = ginkgo.Describe("e2e multiple external gateway validation", func() {
				...
				ginkgo.BeforeEach(func() {
					...
					gwContainers, addressesv4, addressesv6 = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork, gwContainer1, gwContainer2, srcPodName, externalUDPPort, externalTCPPort, ecmpRetry, true)
		...
		var _ = ginkgo.Context("BFD", func() {
			...
			var _ = ginkgo.Describe("e2e non-vxlan external gateway through a dynamic hop", func() {
				...
				ginkgo.BeforeEach(func() {
					...
					gwContainers, addressesv4, addressesv6 = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork,
						gwContainer1, gwContainer2, srcPingPodName, externalUDPPort, externalTCPPort, ecmpRetry, true)
			...
			var _ = ginkgo.Describe("e2e multiple external gateway validation", func() {
				...
				ginkgo.BeforeEach(func() {
					...
					gwContainers, addressesv4, addressesv6 = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork, gwContainer1,
						gwContainer2, srcPodName, externalUDPPort, externalTCPPort, ecmpRetry, true)
				})
	...
	var _ = ginkgo.Context("When migrating from Annotations to Admin Policy Based External Route CRs", func() {
		...
		var _ = ginkgo.Describe("e2e non-vxlan external gateway through a gateway pod", func() {
			...
			ginkgo.BeforeEach(func() {
				...
				gwContainers, addressesv4, addressesv6 = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork,
					gwContainer1, gwContainer2, srcPingPodName, externalUDPPort, externalTCPPort, ecmpRetry, false)
				setupAnnotatedGatewayPods(f, nodes, primaryProviderNetwork, gatewayPodName1, gatewayPodName2, servingNamespace, sleepCommand, addressesv4, addressesv6, false)
			})
	...
	var _ = ginkgo.Context("When validating the Admin Policy Based External Route status", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			_, addressesv4, _ = setupGatewayContainers(f, providerCtx, nodes, primaryProviderNetwork, gwContainer1, gwContainer2, srcPodName,
...
func setupGatewayContainers(f *framework.Framework, providerCtx provider.Context, nodes *corev1.NodeList, network provider.Network, container1, container2,
	...
	externalContainer1 := provider.ExternalContainer{Name: container1, Image: externalContainerImage, Network: network, Args: []string{}, ExtPort: externalContainer1Port}
	externalContainer2Port := provider.Get().GetExternalContainerPort()
	externalContainer2 := provider.ExternalContainer{Name: container2, Image: externalContainerImage, Network: network, Args: []string{}, ExtPort: externalContainer2Port}
...
func setupGatewayContainersForConntrackTest(f *framework.Framework, providerCtx provider.Context, nodes *corev1.NodeList, network provider.Network,
	gwContainer1, gwContainer2 string, srcPodName string) ([]provider.ExternalContainer, gatewayTestIPs, gatewayTestIPs) {
	...
	gwExternalContainer1 := provider.ExternalContainer{Name: gwContainer1, Image: images.IPerf3(), Network: network, Args: []string{}}
	...
	gwExternalContainer2 := provider.ExternalContainer{Name: gwContainer2, Image: images.IPerf3(), Network: network, Args: []string{}}
	...
	for _, gwExternalContainer := range gwExternalContainers {
		...
		if testIPv4 {
			...
			_, err = provider.Get().ExecExternalContainerCommand(gwExternalContainer, []string{"ip", "route", "add", addressesv4.srcPodIP,
			...
			providerCtx.AddCleanUpFn(func() error {
				_, err = provider.Get().ExecExternalContainerCommand(gwExternalContainer, []string{"ip", "route", "del", addressesv4.srcPodIP,
					"via", addressesv4.nodeIP, "dev", provider.Get().PrimaryInterfaceName()})
				if err != nil {
