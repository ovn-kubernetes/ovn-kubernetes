	...
	var _ = ginkgo.Context("With annotations", func() {
		...
		var _ = ginkgo.Describe("e2e multiple external gateway stale conntrack entry deletion validation", func() {
			...
			ginkgo.DescribeTable("Namespace annotation: Should validate conntrack entry deletion for TCP/UDP traffic via multiple external gateways a.k.a ECMP routes", func(addresses *gatewayTestIPs, protocol string, isPodServer bool) {
				...
				startIperfServerAndClientForConntrackTest(f, iperfPodName, isPodServer, externalContainers, addresses.srcPodIP, addresses.gatewayIPs)
	...
	var _ = ginkgo.Context("With Admin Policy Based External Route CRs", func() {
		...
		var _ = ginkgo.Describe("e2e multiple external gateway stale conntrack entry deletion validation", func() {
			...
			ginkgo.DescribeTable("Static Hop: Should validate conntrack entry deletion for TCP/UDP traffic via multiple external gateways a.k.a ECMP routes", func(addresses *gatewayTestIPs, protocol string, isPodServer bool) {
				...
				startIperfServerAndClientForConntrackTest(f, iperfPodName, isPodServer, gwContainers, addresses.srcPodIP, addresses.gatewayIPs)
...
func startIperfServerAndClientForConntrackTest(f *framework.Framework, iperfPodName string, isPodServer bool, externalContainers []infraapi.ExternalContainer, srcPodIP string, gatewayIPs []string) {
	var err error
	if isPodServer {
		...
		for i, externalContainer := range externalContainers {
			...
			_, err = infraprovider.Get().ExecExternalContainerCommand(externalContainer, []string{"iperf3", "-u", "-c", srcPodIP,
		...
		for i, externalContainer := range externalContainers {
			...
			_, err = infraprovider.Get().ExecExternalContainerCommand(externalContainer, []string{"iperf3", "-s", "--daemon", "-V", "-p", fmt.Sprintf("%d", 5201+i)})
