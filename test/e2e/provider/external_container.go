...
func (ec ExternalContainer) IsAvailable() bool {
	if provider.Name() != "kind" {
		panic("IsAvailable is only supported for kind provider")
	}
	return doesContainerExist(ec.Name)
}

// IsConnected verifies the container is connected to its network
func (ec ExternalContainer) IsConnected() bool {
	if provider.Name() != "kind" {
		panic("IsConnected is only supported for kind provider")
	}
	if !doesNetworkExist(ec.Network.Name) {
		return false
	}
	if !doesContainerExist(ec.Name) {
		return false
	}
	return isConnectedToNetwork(ec.Name, ec.Network.Name)
}

func isConnectedToNetwork(containerName, networkName string) bool {
