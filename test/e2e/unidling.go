...
var _ = ginkgo.Describe("Unidling", feature.Unidle, func() {
	...
	ginkgo.It("Should generate a NeedPods event for traffic destined to idled services", func() {
		...
		if pollErr := wait.PollUntilContextTimeout(context.Background(), framework.Poll, e2eservice.KubeProxyEndpointLagTimeout, true, func(_ context.Context) (bool, error) {
...
func createBackend(f *framework.Framework, serviceName, namespace, node string, labels map[string]string, port int32) *corev1.Pod {
	...
	serverPod := e2epod.NewAgnhostPod(
		namespace, "pod-backend", nil, nil,
		[]corev1.ContainerPort{{ContainerPort: port}},
		"netexec", fmt.Sprintf("--http-port=%d", port),
	)
...
func hittingGeneratesNewEvents(service *corev1.Service, cs clientset.Interface, clientPod *corev1.Pod, cmd string) bool {
	...
	err := wait.PollUntilContextTimeout(context.Background(), framework.Poll, 3*time.Second, true, func(_ context.Context) (bool, error) {
