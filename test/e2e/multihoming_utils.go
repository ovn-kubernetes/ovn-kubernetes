...
// getTestCIDRsForTopology returns the appropriate CIDRs for the given topology based on cluster IP family support.
// This function is IP family-aware and dynamically selects CIDRs that match the cluster's IP family capabilities.
func getTestCIDRsForTopology(cs clientset.Interface, topology string, ipv4CIDR, ipv6CIDR string) string {
	var cidrs []string
	if topology == "layer3" {
		// For layer3, use the netCIDR with prefix length
		cidrs = filterCIDRs(cs,
			netCIDR(ipv4CIDR, netPrefixLengthPerNode),
			netCIDR(ipv6CIDR, netPrefixLengthIPv6PerNode))
	} else {
		// For layer2 and localnet, use the CIDR directly
		cidrs = filterCIDRs(cs, ipv4CIDR, ipv6CIDR)
	}
	return joinStrings(cidrs...)
}

// generateStaticIPsForTest generates static IP addresses for testing based on cluster IP family support.
// It takes base CIDRs for IPv4 and IPv6 and an offset, returning IP addresses with CIDR notation.
func generateStaticIPsForTest(cs clientset.Interface, ipv4CIDR, ipv6CIDR string, offset int) ([]string, error) {
	supportedCIDRs := filterCIDRs(cs, ipv4CIDR, ipv6CIDR)
	if len(supportedCIDRs) == 0 {
		return nil, fmt.Errorf("no supported IP families in cluster for CIDRs: %s, %s", ipv4CIDR, ipv6CIDR)
	}

	var staticIPs []string
	for _, cidr := range supportedCIDRs {
		ipGen, err := ip.NewIPGenerator(cidr)
		if err != nil {
			return nil, fmt.Errorf("failed to create IP generator for CIDR %s: %v", cidr, err)
		}
		generatedIP, err := ipGen.GenerateIP(offset)
		if err != nil {
			return nil, fmt.Errorf("failed to generate IP from CIDR %s at offset %d: %v", cidr, offset, err)
		}

		// Get the prefix length from the original CIDR
		...
		// Return IP with CIDR notation (e.g., "192.168.200.10/24")
		staticIPs = append(staticIPs, fmt.Sprintf("%s/%d", generatedIP.String(), prefixLen))
	}
	return staticIPs, nil
}

// getIPBlockCIDRForTest returns an IPBlock CIDR string based on cluster IP family support.
// This is used for multi-network policy tests that use IPBlock specifications.
func getIPBlockCIDRForTest(cs clientset.Interface, ipv4CIDR, ipv6CIDR string) (string, error) {
	supportedCIDRs := filterCIDRs(cs, ipv4CIDR, ipv6CIDR)
	if len(supportedCIDRs) == 0 {
		return "", fmt.Errorf("no supported IP families in cluster for CIDRs: %s, %s", ipv4CIDR, ipv6CIDR)
	}
	// Return the first supported CIDR (IPv4 if available, otherwise IPv6)
	return supportedCIDRs[0], nil
}
