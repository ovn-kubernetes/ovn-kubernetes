...
var _ = Describe("Network Segmentation: Custom L2 UDN", feature.NetworkSegmentation, func() {
	...
	DescribeTable("should respect network configuration",
		func(netConfig *networkAttachmentConfigParams, expectedGatewayIPs []string) {
			...
			if isPreconfigureUDNAddressesEnabled() {
				var podGatewayIPs []string
				for _, route := range podAnno.Routes {
					if route.NextHop != nil {
						podGatewayIPs = append(podGatewayIPs, route.NextHop.String())
					}
				}
				expectedGatewayIPs = filterCIDRs(f.ClientSet, expectedGatewayIPs...)
			...
			if isPreconfigureUDNAddressesEnabled() && len(udn.Spec.Layer2.ReservedSubnets) > 0 {
		...
		Entry("Layer2 basic configuration", &networkAttachmentConfigParams{
			...
			role:     "primary",
		}, []string{"10.128.0.1", "2014:100:200::1"}),
		Entry("Layer2 with custom subnets", &networkAttachmentConfigParams{
			...
			infraCIDRs:        joinStrings("10.128.0.10/30", "2014:100:200::100/122"),
		}, []string{"10.128.0.10", "2014:100:200::100"}),
		...
		}, []string{"10.128.0.2", "2014:100:200::2"}),
	)
})
