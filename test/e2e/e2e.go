...
func setupHostRedirectPod(f *framework.Framework, externalContainer provider.ExternalContainer, nodeName, nodeIP string, isIPv6 bool) error {
	mask := 32
	ipCmd := []string{"ip"}
	if isIPv6 {
		mask = 128
	...
	_, err := provider.Get().ExecExternalContainerCommand(externalContainer, cmd) // cleanup not needed because container is'nt reused between test specs
	if err != nil {
		return err
	}
...
func checkContinuousConnectivity(f *framework.Framework, nodeName, podName, host string, port, tries, timeout int, podChan chan *v1.Pod, errChan chan error) {
	...
	_, err := podClient.Create(context.Background(), pod, metav1.CreateOptions{})
	if err != nil {
		framework.Logf("check contineous connection thread: failed to create pod: %v", err)
		errChan <- err
		return
	}
	...
	if err != nil {
		framework.Logf("check contineous connection thread: pod failed to come up: %v", err)
		errChan <- err
		return
	}
	...
	if err != nil {
		framework.Logf("check contineous connection thread: pod stayed pending: %v", err)
		errChan <- err
		return
	}
	...
	if err != nil {
		framework.Logf("check contineous connection thread: failed to get pod %v", err)
		errChan <- err
		return
	}
...
func isolateKinDIPv6Networks(networkA, networkB string) error {
	if provider.Get().Name() != "kind" {
		// nothing to do
		return nil
	}
	if containerruntime.Get() != containerruntime.Docker {
...
var _ = ginkgo.Describe("e2e control plane", func() {
	...
	var (
		...
		secondaryProviderNetwork   provider.Network
		secondaryExternalContainer provider.ExternalContainer
	)

	ginkgo.BeforeEach(func() {
		var err error
		providerCtx = provider.Get().NewTestContext()
		cmdArgs := []string{"network", "create", "--driver", "bridge", "--subnet", secondaryIPV4Subnet, secondaryNetworkName}
		_, err = osexec.Command(containerruntime.Get().String(), cmdArgs...).CombinedOutput()
		framework.ExpectNoError(err, "must create secondary network")
		secondaryProviderNetwork = provider.Network{Name: secondaryNetworkName}
		...
		secondaryExternalContainerPort := provider.Get().GetExternalContainerPort()
		secondaryExternalContainerSpec := provider.ExternalContainer{Name: "e2e-ovn-k", Image: images.AgnHost(),
		...
		if isInterconnectEnabled() {
			controlPlanePodName = "ovnkube-control-plane" //TODO: CHECK
		...
		controlPlanePods, err := f.ClientSet.CoreV1().Pods(deployment.Get().OVNKubernetesNamespace()).List(context.Background(), metav1.ListOptions{
	...
	ginkgo.It("should provide Internet connection continuously when ovnkube-node pod is killed", func() {
		...
		targetNodeInterface, err := provider.Get().GetK8NodeNetworkInterface(targetNodeName, secondaryProviderNetwork)
		...
		go func() {
			defer ginkgo.GinkgoRecover()
			out, err := provider.Get().ExecExternalContainerCommand(secondaryExternalContainer, []string{"nc", redirectIP, redirectPort})
			if err != nil {
		...
		err = restartOVNKubeNodePod(f.ClientSet, deployment.Get().OVNKubernetesNamespace(), targetNodeName)
		framework.ExpectNoError(err)
	...
	ginkgo.It("should provide Internet connection continuously when pod running master instance of ovnkube-control-plane is killed", func() {
		...
		ovnKubeControlPlaneNode, err := findOvnKubeControlPlaneNode(deployment.Get().OVNKubernetesNamespace(), controlPlanePodName, controlPlaneLeaseName)
	...
	ginkgo.It("should provide Internet connection continuously when all ovnkube-control-plane pods are killed", func() {
		...
		for _, pod := range podList.Items {
			if strings.HasPrefix(pod.Name, controlPlanePodName) && !strings.HasPrefix(pod.Name, "ovs-node") {
				framework.Logf("%q", pod.Namespace)
				err = deletePodWithWaitByName(context.TODO(), f.ClientSet, pod.Name, deployment.Get().OVNKubernetesNamespace())
	...
	ginkgo.Describe("test node readiness according to its defaults interface MTU size", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			res, err := provider.Get().ExecK8NodeCommand(testNodeName, []string{"cat", fmt.Sprintf("/sys/class/net/%s/mtu", deployment.Get().ExternalBridgeName())})
			if err != nil {
		...
		ginkgo.AfterEach(func() {
			...
			_, err := provider.Get().ExecK8NodeCommand(testNodeName, []string{"ip", "link", "set", deployment.Get().ExternalBridgeName(), "mtu", fmt.Sprintf("%d", originalMTU)})
			if err != nil {
				framework.Failf("could not reset MTU of interface: %s", err)
			}

			// restart ovnkube-node pod
			if err := restartOVNKubeNodePod(f.ClientSet, deployment.Get().OVNKubernetesNamespace(), testNodeName); err != nil {
		...
		ginkgo.It("should get node not ready with a too small MTU", func() {
			...
			_, err := provider.Get().ExecK8NodeCommand(testNodeName, []string{"ip", "link", "set", deployment.Get().ExternalBridgeName(), "mtu", "1000"})
			if err != nil {
				framework.Failf("could not set MTU of interface: %s", err)
			}

			// restart ovnkube-node pod to trigger mtu validation
			if err := restartOVNKubeNodePod(f.ClientSet, deployment.Get().OVNKubernetesNamespace(), testNodeName); err == nil || err != wait.ErrWaitTimeout {
				if err == nil {
		...
		ginkgo.It("should get node ready with a big enough MTU", func() {
			...
			_, err := provider.Get().ExecK8NodeCommand(testNodeName, []string{"ip", "link", "set", deployment.Get().ExternalBridgeName(), "mtu", "2000"})
			if err != nil {
				framework.Failf("could not set MTU of interface: %s", err)
			}
...
var _ = ginkgo.Describe("test e2e pod connectivity to host addresses", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		_, err = provider.Get().ExecK8NodeCommand(workerNodeName, []string{"ip", "a", "add",
			fmt.Sprintf("%s/%s", targetIP, singleIPMask), "dev", deployment.Get().ExternalBridgeName()})
		framework.ExpectNoError(err, "failed to add IP to %s", workerNodeName)
	})

	ginkgo.AfterEach(func() {
		_, err := provider.Get().ExecK8NodeCommand(workerNodeName, []string{"ip", "a", "del",
			fmt.Sprintf("%s/%s", targetIP, singleIPMask), "dev", deployment.Get().ExternalBridgeName()})
...
var _ = ginkgo.Describe("e2e ingress traffic validation", func() {
	...
	ginkgo.Context("Validating ingress traffic", func() {
		var externalContainer provider.ExternalContainer
		...
		ginkgo.BeforeEach(func() {
			...
			externalContainer = provider.ExternalContainer{Name: "e2e-ingress", Image: images.AgnHost(), Network: primaryProviderNetwork,
	...
	ginkgo.Context("Validating ingress traffic to manually added node IPs", func() {
		...
		ginkgo.BeforeEach(func() {
			...
			framework.ExpectNoError(err, "failed to get primary network")
			externalContainerPort := provider.Get().GetExternalContainerPort()
			externalContainer = provider.ExternalContainer{Name: "e2e-ingress-add-more", Image: images.AgnHost(), Network: primaryProviderNetwork,
				Args: getAgnHostHTTPPortBindCMDArgs(externalContainerPort), ExtPort: externalContainerPort}
			externalContainer, err = providerCtx.CreateExternalContainer(externalContainer)
			...
			_, _ = providerCtx.AttachNetwork(provider.Network{Name: "kindexgw"}, externalContainer.Name)
			...
			for i, node := range nodes.Items {
				...
				_, err := provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "addr", "add", newIP, "dev", deployment.Get().ExternalBridgeName()})
				if err != nil {
					framework.Failf("failed to add new Addresses to node %s: %v", node.Name, err)
				}
				providerCtx.AddCleanUpFn(func() error {
					_, err := provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "addr", "del", newIP, "dev", deployment.Get().ExternalBridgeName()})
					if err != nil {
...
var _ = ginkgo.Describe("e2e br-int flow monitoring export validation", func() {
	...
	ginkgo.DescribeTable("Should validate flow data of br-int is sent to an external gateway",
		func(protocol flowMonitoringProtocol, collectorPort uint16) {
			...
			primaryProviderNetwork, err := provider.Get().PrimaryNetwork()
			framework.ExpectNoError(err, "failed to get primary network")
			collectorExternalContainer := provider.ExternalContainer{Name: getContainerName(collectorPort), Image: "cloudflare/goflow",
			...
			collectorContainerLogsTest := func() wait.ConditionFunc {
				return func() (bool, error) {
					collectorContainerLogs, err := provider.Get().GetExternalContainerLogs(collectorExternalContainer)
					if err != nil {
