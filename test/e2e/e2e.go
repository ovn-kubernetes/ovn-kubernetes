...
func IsGatewayModeLocal(cs kubernetes.Interface) bool {
	...
	if !ok {
		framework.Failf("Failed to find L3 gateway config for Node %q", node.Name)
	}
	var l3Config util.L3GatewayConfig
	if err = l3Config.UnmarshalJSON([]byte(l3ConfigRaw)); err != nil {
		framework.Failf("Failed unmarshall Node %q L3 gateway config: %v", node.Name, err)
	}
