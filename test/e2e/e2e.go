...
func setupHostRedirectPod(f *framework.Framework, externalContainer provider.ExternalContainer, node *v1.Node, isIPv6 bool) error {
	nodeV4, nodeV6 := externalContainer.GetIPv4(), externalContainer.GetIPv6()
	mask := 32
	ipCmd := []string{"ip"}
	nodeIP := nodeV4
	if isIPv6 {
...
var _ = ginkgo.Describe("e2e control plane", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		secondaryProviderNetwork, err = providerCtx.CreateNetwork(secondaryNetworkName, "99.10.0.0/16")
	...
	ginkgo.It("should provide Internet connection continuously when ovnkube-node pod is killed", func() {
		...
		err = setupHostRedirectPod(f, secondaryExternalContainer, targetNode, IsIPv6Cluster(f.ClientSet))
		framework.ExpectNoError(err)

		// start TCP client
		go func() {
			defer ginkgo.GinkgoRecover()
			_, _ = provider.Get().ExecExternalContainerCommand(secondaryExternalContainer, []string{"nc", "--idle-timeout", "120s", redirectIP, redirectPort})
		}()
		...
		err = restartOVNKubeNodePod(f.ClientSet, deployment.Get().OVNKubernetesNamespace(), nodeName)
		framework.ExpectNoError(err)
...
var _ = ginkgo.Describe("e2e network policy hairpinning validation", func() {
	...
	ginkgo.It("Should validate the hairpinned traffic is always allowed", func() {
		...
		hostname := pokeEndpointViaPod(namespaceName, pod1.Name, "http", svcIP, endpointHTTPPort, serviceHTTPPort, "hostname")
				...
				gomega.Eventually(func() (r bool) {
					...
					for _, ipAddresses := range ipv4Addresses {
						for _, targetHost := range ipAddresses {
							...
							framework.Logf("Running command %v", curlCommand)
...
var _ = ginkgo.Describe("e2e br-int flow monitoring export validation", func() {
	...
	ginkgo.DescribeTable("Should validate flow data of br-int is sent to an external gateway",
		func(protocol flowMonitoringProtocol, collectorPort uint16) {
			...
			primaryProviderNetwork, err := provider.Get().PrimaryNetwork()
			framework.ExpectNoError(err, "failed to get primary network")
			collectorExternalContainer := provider.ExternalContainer{Name: collectorContainer, Image: "cloudflare/goflow",
