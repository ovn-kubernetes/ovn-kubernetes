...
func setupIPVRFAgnhost(ictx infraapi.Context, vid int, vrfName string, ipFamilies []utilnet.IPFamily, subnets ...string) ([]string, error) {
	...
	// Re-add IPv6 address that was removed when interface was moved to VRF
	if frrNetInf.IPv6 != "" && frrNetInf.IPv6Prefix != "" {
		ipv6WithPrefix := fmt.Sprintf("%s/%s", frrNetInf.IPv6, frrNetInf.IPv6Prefix)
		_, err = infraprovider.Get().ExecExternalContainerCommand(frr,
			[]string{"ip", "-6", "addr", "add", ipv6WithPrefix, "dev", frrInterface})
		if err != nil {
			return nil, fmt.Errorf("failed to re-add IPv6 address %s to interface %s after VRF assignment: %w",
				ipv6WithPrefix, frrInterface, err)
		}
		framework.Logf("Re-added IPv6 address %s to %s after VRF assignment", ipv6WithPrefix, frrInterface)
	}
	...
	ginkgo.DescribeTable("EVPN connectivity", feature.RouteAdvertisements, feature.EVPN,
		func(networkSpec *udnv1.NetworkSpec) {
			...
			err = runClusterEVPNSetupScript(ictx,
				...
				hasMACVRF, macVRFVNI, macVRFVID,
				hasIPVRF, ipVRFVNI, ipVRFVID,
				cudnSubnetsFromSpec)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
