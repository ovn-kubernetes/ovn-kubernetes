...
func setupEVPNBGPOnExternalFRR(ictx infraapi.Context, asn int, neighborIPs []string) error {
	...
	// Activate each neighbor in the l2vpn evpn address-family and configure
	// route-reflector-client so that EVPN routes are reflected.
	for _, ip := range neighborIPs {
		cmd = append(cmd, "-c", fmt.Sprintf("neighbor %s activate", ip))
		cmd = append(cmd, "-c", fmt.Sprintf("neighbor %s route-reflector-client", ip))
	}
...
//
// With unmanaged VTEP mode using node IPs, no IPv4/IPv6 unicast prefix advertisement
// is needed — the VTEP IPs (node IPs) are already directly reachable from the external FRR
// on the same L2 network. Only the BGP neighbor definition and label for RA selector are needed.
//
// Parameters:
//   - ictx: Infrastructure context for cleanup registration
...
func createFRRConfiguration(ictx infraapi.Context,
	...
	// No prefixes/toAdvertise needed - with unmanaged VTEP using node IPs,
	// the VTEP IPs are directly reachable on the same L2 network.
...
var _ = ginkgo.Describe("EVPN", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		// Derive VTEP subnets from the kind network CIDRs.
		// With unmanaged VTEP mode, we use node IPs as VTEP IPs, so the VTEP
		// CIDRs match the node network (kind bridge) — no separate underlay needed.
		kindV4Subnet, kindV6Subnet, err := kindNetwork.IPv4IPv6Subnets()
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		if isIPv4Supported(f.ClientSet) && kindV4Subnet != "" {
			vtepSubnets = append(vtepSubnets, kindV4Subnet)
		}
		if isIPv6Supported(f.ClientSet) && kindV6Subnet != "" {
			vtepSubnets = append(vtepSubnets, kindV6Subnet)
		}
		gomega.Expect(vtepSubnets).NotTo(gomega.BeEmpty(), "Kind network must have subnets for VTEP CIDRs")
		framework.Logf("VTEP subnets (from kind network): %v", vtepSubnets)
