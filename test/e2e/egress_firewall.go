...
var _ = ginkgo.Describe("e2e egress firewall policy validation", func() {
	...
	ginkgo.Context("with external containers", func() {
		...
		var (
			...
			externalContainer1       provider.ExternalContainer
			externalContainer2       provider.ExternalContainer
		)

		getExternalContainerIP := func(externalContainer provider.ExternalContainer) string {
			if IsIPv6Cluster(f.ClientSet) {
		...
		checkExternalContainerConnectivity := func(externalContainer provider.ExternalContainer, dstIP string, dstPort int) {
			_, err := provider.Get().ExecExternalContainerCommand(externalContainer, []string{
		...
		ginkgo.BeforeEach(func() {
			...
			externalContainer1Spec := provider.ExternalContainer{
			...
			externalContainer2Spec := provider.ExternalContainer{
			...
			gomega.Eventually(func() bool {
				_, err := provider.Get().ExecExternalContainerCommand(externalContainer1, []string{
					"curl", "-s", "--connect-timeout", fmt.Sprint(testTimeout), net.JoinHostPort(getExternalContainerIP(externalContainer2), fmt.Sprint(externalContainer2.GetPort())),
				})
				if err != nil {
					framework.Logf("Failed to connect to container 2 from container 1: %v", err)
					return false
				}
				_, err = provider.Get().ExecExternalContainerCommand(externalContainer2, []string{
					"curl", "-s", "--connect-timeout", fmt.Sprint(testTimeout), net.JoinHostPort(getExternalContainerIP(externalContainer1), fmt.Sprint(externalContainer1.GetPort())),
				})
				if err != nil {
		...
		ginkgo.It("Should validate the egress firewall policy functionality for allowed IP", func() {
			...
			createSrcPodWithRetry(3, getExternalContainerIP(externalContainer1), externalContainer1.GetPort(),
...
`, f.Namespace.Name, getExternalContainerIP(externalContainer1), singleIPMask, denyAllCIDR)
			...
			checkConnectivity(srcPodName, getExternalContainerIP(externalContainer1), externalContainer1.GetPort(), true)
			...
			checkConnectivity(srcPodName, getExternalContainerIP(externalContainer2), externalContainer2.GetPort(), false)
		})
...
spec:
  ...
  - type: Deny
    to:
      cidrSelector: %s
`, f.Namespace.Name, getExternalContainerIP(externalContainer1), subnetMask, externalContainer1.GetPort(), denyAllCIDR)
			applyEF(egressFirewallConfig, f.Namespace.Name)
		...
		ginkgo.It("Should validate the egress firewall allows inbound connections", func() {
			...
			ginkgo.By(fmt.Sprintf("Verifying connectivity from pod %s to external container [%s]:%s",
				efPodName, getExternalContainerIP(externalContainer1), externalContainer1.GetPort()))
			checkConnectivity(efPodName, getExternalContainerIP(externalContainer1), externalContainer1.GetPort(), true)
...
`, f.Namespace.Name, denyAllCIDR)
			...
			ginkgo.By(fmt.Sprintf("Verifying connectivity from pod %s to external container [%s]:%s is blocked",
				efPodName, getExternalContainerIP(externalContainer1), externalContainer1.GetPort()))
			checkConnectivity(efPodName, getExternalContainerIP(externalContainer1), externalContainer1.GetPort(), false)
...
`, f.Namespace.Name, denyAllCIDR)
			...
			ginkgo.By(fmt.Sprintf("Verifying connectivity to an external host %s is not permitted as defined "+
				"by the external firewall policy", getExternalContainerIP(externalContainer1)))
			checkConnectivity(srcPodName, getExternalContainerIP(externalContainer1), externalContainer1.GetPort(), false)
		})
...
`, f.Namespace.Name, f.Namespace.Name, labelMatch, denyAllCIDR)
			...
			for nodeName, ipFamilies := range node2ndaryIPs {
				for _, ip := range ipFamilies {
					...
					_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{
						"ip", "addr", "add", ip, "dev", provider.Get().PrimaryInterfaceName(),
					})
					...
					ginkgo.DeferCleanup(func() error {
						_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "addr", "delete", ip, "dev", provider.Get().PrimaryInterfaceName()})
						return err
					})
