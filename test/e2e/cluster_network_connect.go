...
var _ = Describe("ClusterNetworkConnect OVN-Kubernetes Controller", feature.NetworkConnect, func() {
	...
	*/
	...
	It("should report events on failure", func() {
		...
		blueNs := fmt.Sprintf("blue-ns-%s", testID)
		// Labels for CNC selection
		blueLabel := map[string]string{"network-color": "blue", "test-id": testID}
		...
		Eventually(func() error {
			...
			for _, event := range events.Items {
				if event.InvolvedObject.Kind == "ClusterNetworkConnect" && event.InvolvedObject.Name == cncName &&
					event.Reason == "ErrorReconcilingCNC" && strings.Contains(event.Message, "failed to parse subnet annotation") {
					return nil
				}
			}
