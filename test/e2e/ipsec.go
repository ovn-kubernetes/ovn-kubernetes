	...
	ginkgo.Context("Test IPsec metrics ", func() {
		...
		ginkgo.It("Get IPsec metrics which reflect the down and up state of the IPsec tunnel", func() {
			...
			for _, ovnPod := range ovnPods.Items {
				...
				ipsecMetricValue, err := getMetricValue(f, ovnPod.Name, ovnKubernetesNamespace, ipsecMetricName)
				...
				ipsecTunnelMetricValue, err := getMetricValue(f, ovnPod.Name, ovnKubernetesNamespace, ipsecTunnelMetricName)
				framework.ExpectNoError(err, "Retrieving metrics failed")
			...
			for _, ovnPod := range ovnPods.Items {
				...
				ipsecTunnelMetricValue, err := getMetricValue(f, ovnPod.Name, ovnKubernetesNamespace, ipsecTunnelMetricName)
				framework.ExpectNoError(err, "Retrieving metrics failed")
				gomega.Expect(ipsecTunnelMetricValue).Should(gomega.Equal("0"))
			}
...
// Get metric value by prom2json tool
func getMetricValue(f *framework.Framework, podName, podNamespace, metricName string) (string, error) {
	ginkgo.GinkgoHelper()
	...
	if err != nil {
		return "", fmt.Errorf("prom2json failed: %v", err)
	}
	...
	if len(values) == 0 || values[0] == "" {
		return "", fmt.Errorf("no metric value found for %s", metricName)
	}
