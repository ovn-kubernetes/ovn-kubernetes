	...
	for _, hostNetwork := range []bool{true, false} {
		...
		ginkgo.When(fmt.Sprintf("a nodePort service targeting a pod with hostNetwork:%t is created", hostNetwork), func() {
			...
			ginkgo.BeforeEach(func() {
				...
				serverPodPort := provider.Get().GetK8HostPort() // maybe a host net or cluster net pod but select host port anyway
							...
							Ports: []v1.ServicePort{
								{
									...
									TargetPort: intstr.FromInt(int(serverPodPort)),
									Protocol:   v1.ProtocolTCP,
								},
								{
									Name:       udpPortName,
		...
		ginkgo.It("should listen on each host addresses", func() {
			...
			for nodeName, ipFamilies := range nodeIPs {
				for _, ip := range ipFamilies {
					...
					_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "addr", "add", ip, "dev", deployment.Get().ExternalBridgeName()})
					if err != nil {
...
`)
			...
			primaryProviderNetwork, err := provider.Get().PrimaryNetwork()
			framework.ExpectNoError(err, "primary network must be available to attach containers")
			clientExternalContainerPort := provider.Get().GetExternalContainerPort()
			clientExternalContainerSpec := provider.ExternalContainer{
				Name:    clientContainerName,
				Image:   images.AgnHost(),
				Network: primaryProviderNetwork,
				Args:    []string{"netexec", fmt.Sprintf("--http-port=%d", clientExternalContainerPort)},
				ExtPort: clientExternalContainerPort,
			}
			clientExternalContainer, err := providerCtx.CreateExternalContainer(clientExternalContainerSpec)
			framework.ExpectNoError(err, "external container %s must be created", clientExternalContainerSpec.Name)
			...
			// If `kindexgw` exists, connect client container to it
			_, _ = providerCtx.AttachNetwork(provider.Network{Name: "kindexgw"}, clientExternalContainer.GetName())
			...
			for nodeName, ipFamilies := range nodeIPs {
				for _, ip := range ipFamilies {
					...
					providerCtx.AddCleanUpFn(func() error {
						_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "addr", "del", ip, "dev", deployment.Get().ExternalBridgeName()})
						if err != nil {
			...
			err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(serverExternalContainer, egressPod.Name,
...
var _ = ginkgo.Describe("Service Hairpin SNAT", func() {
	...
	ginkgo.It("Should ensure service hairpin traffic is SNATed to hairpin masquerade IP; Switch LB", func() {
		...
		clientIP := pokeEndpointViaPod(namespaceName, backendName, "http", svcIP, endpointHTTPPort, serviceHTTPPort, "clientip")
...
`)
	...
	ginkgo.AfterEach(func() {
		...
		defer func() {
			...
			externalContainer := provider.ExternalContainer{Name: externalRouterContainerName}
...
`)
	...
	ginkgo.It("Should ensure load balancer service works with pmtud", func() {
		...
		externalContainer := provider.ExternalContainer{Name: externalRouterContainerName} // pre-created
		_, err = curlInExternalContainer(externalContainer, svcLoadBalancerIP, endpointHTTPPort, "big.iso -o big.iso", 120)
...
`)
	...
	ginkgo.It("Should ensure load balancer service works when ETP=local and backend pods are also egressIP served pods", func() {
		...
		bgpRouteCommand := strings.Split(fmt.Sprintf("ip%s route show %s", ipVer, svcLoadBalancerIP), " ")
		gomega.Eventually(func() bool {
			routes, err := provider.Get().ExecExternalContainerCommand(provider.ExternalContainer{Name: externalRouterContainerName}, bgpRouteCommand)
			framework.ExpectNoError(err, "failed to get BGP routes from intermediary router")
			framework.Logf("Routes in FRR %s", routes)
			routeCount := 0
			matchedRoute := ""
