...
var _ = ginkgo.Describe("Services", func() {
	...
	ginkgo.It("Creates a host-network service, and ensures that host-network pods can connect to it", func() {
		...
		targetPort := provider.Get().GetK8HostPort()
	...
	for _, hostNetwork := range []bool{true, false} {
		...
		ginkgo.When(fmt.Sprintf("a nodePort service targeting a pod with hostNetwork:%t is created", hostNetwork), func() {
			...
			ginkgo.BeforeEach(func() {
				...
				serverPodPortTCP := provider.Get().GetK8HostPort() // maybe a host net or cluster net pod but select host port anyway
				serverPodPortUDP := provider.Get().GetK8HostPort() // maybe a host net or cluster net pod but select host port anyway
				...
				// random port.
				gomega.Eventually(func() error {
					servicePort := rand.Intn(32767-30000) + 30000
	...
	ginkgo.It("does not use host masquerade address as source IP address when communicating externally", func() {
		...
		getIPRouteGetOutput := func(dst string) string {
			...
			output, err := provider.Get().ExecK8NodeCommand(nodeName, cmd)
	...
	ginkgo.It("All service features work when manually listening on a non-default address", func() {
		...
		udpHostNsPort := provider.Get().GetK8HostPort()
		service, err := jig.CreateUDPService(context.TODO(), func(s *v1.Service) {
			s.Spec.Ports = []v1.ServicePort{
				{
					Name:       "udp",
	...
	ginkgo.Context("of type NodePort", func() {
		...
		ginkgo.AfterEach(func() {
			...
			for nodeName, ipFamilies := range nodeIPs {
				for _, ip := range ipFamilies {
					...
					_, err := provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "addr", "delete",
						fmt.Sprintf("%s/%s", ip, subnetMask), "dev", deployment.Get().ExternalBridgeName()})
		...
		ginkgo.It("should listen on each host addresses", func() {
			...
			externalContainer := provider.ExternalContainer{Name: clientContainerName, Image: images.AgnHost(), Network: primaryProviderNetwork,
			...
			exGWNetwork := provider.Network{Name: "kindexgw"}
			...
			primaryNetworkSubnet := primaryProviderNetwork.Configs[0].Subnet
			...
			for nodeName, ipFamilies := range nodeIPs {
				for _, ip := range ipFamilies {
					...
					_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", iproute2Proto, "addr", "add", ip, "dev", deployment.Get().ExternalBridgeName()})
					if err != nil {
						framework.Failf("failed to add new IP address %s to node %s: %v", ip, nodeName, err)
					}
					providerCtx.AddCleanUpFn(func() error {
						// manually add the a secondary IP to each node
						_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", iproute2Proto, "addr", "del", ip, "dev", deployment.Get().ExternalBridgeName()})
						if err != nil {
		...
		ginkgo.It("should work on secondary node interfaces for ETP=local and ETP=cluster when backend pods are also served by EgressIP", func() {
			...
			serverExternalContainerPort := provider.Get().GetExternalContainerPort()
			serverExternalContainerSpec := provider.ExternalContainer{
...
`)
			...
			// If `kindexgw` exists, connect client container to it
			_, _ = providerCtx.AttachNetwork(provider.Network{Name: "kindexgw"}, serverExternalContainer.GetName())

			ginkgo.By("Selecting additional IP addresses for each node")
			// add new secondary IP from node subnet to all nodes, if the cluster is v6 add an ipv6 address
			secondaryNetworkV4Subnet, secondaryNetworkV6Subnet := secondaryProviderNetwork.GetIPV4V6Subnets()
			...
			ginkgo.By("Adding additional IP addresses to each node")
			for nodeName, ipFamilies := range nodeIPs {
				secondaryNetworkInterface, err := provider.Get().GetK8NodeNetworkInterface(nodeName, secondaryProviderNetwork)
				...
				for _, ip := range ipFamilies {
					// manually add the a secondary IP to each node
					_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "addr", "add", ip, "dev", secondaryNetworkInterface.InfName})
					if err != nil {
						framework.Failf("failed to add new IP address %s to node %s: %v", ip, nodeName, err)
					}
					providerCtx.AddCleanUpFn(func() error {
						_, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "addr", "del", ip, "dev", secondaryNetworkInterface.InfName})
						if err != nil {
			...
			for _, serviceSpec := range []*v1.Service{etpLocalSvc, etpClusterSvc} {
				...
				for _, protocol := range []string{"http", "udp"} {
					...
					secondaryNetworkInterface, err := provider.Get().GetK8NodeNetworkInterface(egressNode, secondaryProviderNetwork)
		...
		ginkgo.It("should handle IP fragments", func() {
			...
			primaryProviderNetwork, err := provider.Get().PrimaryNetwork()
			framework.ExpectNoError(err, "failed to get primary network")
			primaryInf, err := provider.Get().GetK8NodeNetworkInterface(nodeName, primaryProviderNetwork)
			...
			externalContainer := provider.ExternalContainer{Name: clientContainerName, Image: images.AgnHost(), Network: primaryProviderNetwork,
				Args: []string{"pause"}, ExtPort: provider.Get().GetExternalContainerPort()}
			externalContainer, err = providerCtx.CreateExternalContainer(externalContainer)
			...
			ipContainerCmd := append(ipContainerCmdPrefix, "route", "add", serverNodeIP, "dev", provider.Get().PrimaryInterfaceName(), "src", clientIP, "mtu", pmtu)
			_, err = provider.Get().ExecExternalContainerCommand(externalContainer, ipContainerCmd)
			framework.ExpectNoError(err, "lowering MTU in the external kind container failed: %v", err)
			providerCtx.AddCleanUpFn(func() error {
				ipContainerCmd := append(ipContainerCmdPrefix, "route", "del", serverNodeIP, "dev", provider.Get().PrimaryInterfaceName(), "src", clientIP, "mtu", pmtu)
				_, err = provider.Get().ExecExternalContainerCommand(externalContainer, ipContainerCmd)
				return err
			})
			var udpPort int32
			...
			for _, payload := range []string{"non-fragmented", "fragmented"} {
				...
				stdout, err := provider.Get().ExecExternalContainerCommand(externalContainer, []string{"/bin/sh", "-c", containerCmd})
...
var _ = ginkgo.Describe("Load Balancer Service Tests with MetalLB", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		frrContainer := provider.ExternalContainer{Name: externalRouterContainerName, Network: primaryProviderNetwork}
		if !frrContainer.IsAvailable() {
			ginkgo.Skip("FRR container is not available")
		}
		if !frrContainer.IsConnected() {
			ginkgo.Fail("external container frr is not connected to the primary provider network")
		}
		nodes, err := e2enode.GetBoundedReadySchedulableNodes(context.TODO(), f.ClientSet, 2)
		framework.ExpectNoError(err)
		if len(nodes.Items) < 2 {
...
`)
	...
	ginkgo.AfterEach(func() {
		...
		defer func() {
			...
			externalContainer := provider.ExternalContainer{Name: externalRouterContainerName, Network: primaryProviderNetwork}
			if !externalContainer.IsAvailable() {
				ginkgo.Skip("FRR container is not available")
			}
			if !externalContainer.IsConnected() {
				ginkgo.Skip("FRR container is not connected to primary provider network")
			}
			_, err = provider.Get().ExecExternalContainerCommand(externalContainer, []string{"ip", "link", "set", "mtu", "1500", "dev", "eth1"})
...
`)
	...
	ginkgo.It("Should ensure load balancer service works with pmtud", func() {
		...
		primaryProviderNetwork, err := provider.Get().PrimaryNetwork()
		framework.ExpectNoError(err, "must fetch primary provider network")
		externalContainer := provider.ExternalContainer{Name: externalRouterContainerName, Network: primaryProviderNetwork} // pre-created
		if !externalContainer.IsAvailable() {
			ginkgo.Skip("FRR container is not available")
		}
		if !externalContainer.IsConnected() {
			framework.Failf("FRR container is not connected to primary provider network")
		}
		...
		framework.ExpectNoError(err, fmt.Sprintf("failed to get node's %s node ip address", backendNodeName))
		gomega.Eventually(func() bool {
			routes, err := provider.Get().ExecExternalContainerCommand(externalContainer, cmd)
...
`)
			...
			_, err = provider.Get().ExecExternalContainerCommand(externalContainer, cmd)
	...
	ginkgo.It("Should ensure load balancer service works with 0 node ports when ETP=local", func() {
		...
		ginkgo.By("by sending a TCP packet to service " + svcName + " with type=LoadBalancer in namespace " + namespaceName + " with backend pod " + backendName)
		externalContainer := provider.ExternalContainer{Name: externalClientContainerName}
		_, err = wgetInExternalContainer(externalContainer, svcLoadBalancerIP, endpointHTTPPort, "big.iso", 120)
		framework.ExpectNoError(err, "failed to curl load balancer service")
	...
	ginkgo.It("Should ensure load balancer service works when ETP=local and session affinity is set", func() {
		...
		output, err = provider.Get().ExecExternalContainerCommand(externalContainer, cmd)
		...
		clientNetNetwork := provider.Network{Name: "clientnet"}
		lbClientInf, err := provider.Get().GetExternalContainerNetworkInterface(externalContainer, clientNetNetwork)
		framework.ExpectNoError(err, "failed to get network interface info for network %s within external container %s", clientNetNetwork.Name, externalContainer.GetName())
	...
	ginkgo.It("Should ensure load balancer service works when ETP=local and backend pods are also egressIP served pods", func() {
		...
		gomega.Eventually(func() bool {
			routes, err := provider.Get().ExecExternalContainerCommand(externalContainer, bgpRouteCommand)
			framework.ExpectNoError(err, "failed to get BGP routes from intermediary router")
			framework.Logf("Routes in FRR %s", routes)
			routeCount := 0
			matchedRoute := ""
		...
		framework.ExpectNoError(err, "failed to inspect logs in backend pods")
		framework.Logf("%v", targetPodLogs)
		clientnetNetwork, err := provider.Get().GetNetwork("clientnet")
		framework.ExpectNoError(err, "network 'clientnet' must be available")
		lbClientNetworkInterface, err := provider.Get().GetExternalContainerNetworkInterface(externalContainer, clientnetNetwork)
		framework.ExpectNoError(err, "failed to get network interface info for external container %s connected to network %s", externalContainer.Name, clientnetNetwork.Name)
