...
func GenerateGatwayIPv6RouterLLA(node *corev1.Node, networkName string) (string, error) {
	lrpAddresses, err := util.ParseNodeGatewayRouterJoinAddrs(node, networkName)
	if err != nil {
		return "", err
	}
	return util.HWAddrToIPv6LLA(util.IPAddrToHWAddr(lrpAddresses[0].IP)).String(), nil
}
