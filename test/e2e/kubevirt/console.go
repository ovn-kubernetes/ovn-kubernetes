...
func loginToFedoraWithHostname(virtctlPath string, vmi *kubevirtv1.VirtualMachineInstance, user, password, hostname string) error {
	err := retry.OnError(retry.DefaultBackoff,
		...
		func() error {
			expecter, _, err := newExpecter(virtctlPath, vmi, consoleConnectionTimeout, expect.Verbose(true), expect.VerboseWriter(GinkgoWriter), expect.DebugCheck(log.New(GinkgoWriter, "expect: virtctl console check", 0 /* no flags */)))
			if err != nil {
				return err
			}
			defer expecter.Close()
