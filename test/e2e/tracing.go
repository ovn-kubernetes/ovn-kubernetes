...
var _ = ginkgo.Describe("Tracing", feature.Tracing, func() {
	...
	ginkgo.It("creates and deletes a pod and emits linked spans", func() {
		...
		gomega.Eventually(func() error {
			...
			rows, err := collectSpanRows(queryPod, rootSpans, queryTimeRange)
			if err != nil {
				return err
			}
			if len(rows) == 0 {
...
func collectSpanRows(queryPod *v1.Pod, rootSpans []traceSpan, lookback time.Duration) ([][]string, error) {
	now := time.Now().Unix()
	start := now - int64(lookback.Seconds())
