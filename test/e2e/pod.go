				...
				for _, size := range []string{"small", "large"} {
					for _, externalContainerIP := range externalContainerIPs {
						if size == "large" {
							...
							var stdout string
							output, err := infraprovider.Get().ExecExternalContainerCommand(externalContainer, []string{"ip", "route", "flush", "cache"})
							framework.ExpectNoError(err, "Flushing the ip route cache failed: %s", output)
						...
						for _, ovnKubeNodePod := range ovnKubeNodePods.Items {
							...
							output, err := e2ekubectl.RunKubectl(deploymentconfig.Get().OVNKubernetesNamespace(), "exec", ovnKubeNodePod.Name, "--container", containerName, "--",
