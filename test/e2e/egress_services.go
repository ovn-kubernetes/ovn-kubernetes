...
var _ = ginkgo.Describe("EgressService", func() {
	...
	var (
		...
		nodes                        []v1.Node
		externalContainerKinDNetwork infraapi.ExternalContainer
	...
	ginkgo.BeforeEach(func() {
		...
		externalContainerKinDNetwork = infraapi.ExternalContainer{Name: externalContainerName, Image: images.AgnHost(),
			Network: primaryProviderNetwork, ExtPort: 8080,
			Args: getAgnHostHTTPPortBindCMDArgs(8080)}
		externalContainerKinDNetwork, err = providerCtx.CreateExternalContainer(externalContainerKinDNetwork)
	...
	ginkgo.DescribeTable("Should validate pods' egress is SNATed to the LB's ingress ip without selectors",
		func(protocol v1.IPFamily) {
			skipIfProtoNotAvailableFn(protocol, externalContainerKinDNetwork)
			dstIP := externalContainerKinDNetwork.GetIPv4()
			if protocol == v1.IPv6Protocol {
				dstIP = externalContainerKinDNetwork.GetIPv6()
			}
...
`)
			...
			setSVCRouteOnExternalContainer(externalContainerKinDNetwork, svcIP, egressHostV4IP, egressHostV6IP)

			ginkgo.By("Verifying the pods reach the external container with the service's ingress ip")
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerKinDNetwork.GetPortStr())
			...
			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerKinDNetwork.GetPortStr()); err != nil {
						return err
					}
				}
				return nil
			...
			reachAllServiceBackendsFromExternalContainer(externalContainerKinDNetwork, svcIP, podHTTPPort, pods)

			ginkgo.By("Creating the custom network")
			setBlackholeRoutingTableOnNodes(providerCtx, nodes, externalContainerKinDNetwork, blackholeRoutingTable, protocol == v1.IPv4Protocol)
...
`)
			...
			gomega.Consistently(func() error {
				for _, pod := range pods {
					err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerKinDNetwork.GetPortStr())
			...
			delExternalClientBlackholeFromNodes(nodes, blackholeRoutingTable, externalContainerKinDNetwork.GetIPv4(), externalContainerKinDNetwork.GetIPv6(), protocol == v1.IPv4Protocol)
			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainerKinDNetwork.GetPortStr()); err != nil {
						return err
			...
			for i, pod := range pods {
				...
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, dstIP, externalContainerKinDNetwork.GetPortStr())
...
`)
	...
	ginkgo.DescribeTable("Should validate pods' egress is SNATed to the LB's ingress ip with selectors",
		func(protocol v1.IPFamily) {
			skipIfProtoNotAvailableFn(protocol, externalContainerKinDNetwork)
			externalContainerIP := externalContainerKinDNetwork.GetIPv4()
			if protocol == v1.IPv6Protocol {
				externalContainerIP = externalContainerKinDNetwork.GetIPv6()
			}
			ginkgo.By("Creating the backend pods")
			podsCreateSync := errgroup.Group{}
			index := 0
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, externalContainerIP, externalContainerKinDNetwork.GetPortStr())
				}, 5*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with loadbalancer's ingress ip")
			}

			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, externalContainerIP, externalContainerKinDNetwork.GetPortStr()); err != nil {
						return err
					}
				}
				return nil
...
`)
			...
			for i, pod := range pods {
				...
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, externalContainerIP, externalContainerKinDNetwork.GetPortStr())
				}, 3*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with node's ip")
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, egressIP.String(), dstIP, externalContainerKinDNetwork.GetPortStr())
...
`)
				...
				setSVCRouteOnContainer(externalContainerKinDNetwork, svcIP, v4Via, v6Via)

				ginkgo.By("Verifying the external client can reach the service")
				gomega.Eventually(func() error {
					_, err := curlServiceAgnHostHostnameFromExternalContainer(externalContainerKinDNetwork, svcIP, podHTTPPort)
					return err
				...
				gomega.Consistently(func() error {
					out, err := curlServiceAgnHostHostnameFromExternalContainer(externalContainerKinDNetwork, svcIP, podHTTPPort)
...
`)
				...
				gomega.Consistently(func() error {
					for _, pod := range append(net1.createdPods, net2.createdPods...) {
						err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, "", dst, externalContainerKinDNetwork.GetPortStr())
