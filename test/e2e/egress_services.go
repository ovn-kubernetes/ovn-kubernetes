...
var _ = ginkgo.Describe("EgressService", func() {
	...
	var (
		...
		nodes             []v1.Node
		externalContainer provider.ExternalContainer
		f                 = wrappedTestFramework("egress-services")
		providerCtx       provider.Context
	)

	skipIfProtoNotAvailableFn := func(protocol v1.IPFamily, container provider.ExternalContainer) {
	...
	ginkgo.BeforeEach(func() {
		...
		externalContainer = provider.ExternalContainer{Name: externalContainerName, Image: images.AgnHost(),
			Network: primaryProviderNetwork, ExtPort: externalContainerPort,
			Args: getAgnHostHTTPPortBindCMDArgs(externalContainerPort)}
...
`)
			...
			setSVCRouteOnExternalContainer(providerCtx, externalContainer, svcIP, egressHostV4IP, egressHostV6IP)

			ginkgo.By("Verifying the pods reach the external container with the service's ingress ip")
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainer.GetPort())
			...
			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainer.GetPort()); err != nil {
						return err
					}
				}
				return nil
...
`)
			...
			gomega.Consistently(func() error {
				for _, pod := range pods {
					err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, dstIP, externalContainer.GetPort())
			...
			for i, pod := range pods {
				...
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, dstIP, externalContainer.GetPort())
	...
	ginkgo.XDescribeTable("[LGW] Should validate pods' egress uses node's IP when setting Network without SNAT",
		func(protocol v1.IPFamily) {
			skipIfProtoNotAvailableFn(protocol, externalContainer)
			dstIP := externalContainer.GetIPv4()
			if protocol == v1.IPv6Protocol {
...
`)
			...
			_, err = provider.Get().ExecK8NodeCommand(dstNode.Name, []string{"ip", "addr", "add", otherDst, "dev", extBridgeName})
			if err != nil {
				framework.Failf("failed to add address to node %s: %v", dstNode.Name, err)
			}
			providerCtx.AddCleanUpFn(func() error {
				_, err = provider.Get().ExecK8NodeCommand(dstNode.Name, []string{"ip", "addr", "delete", otherDst, "dev", extBridgeName})
				return err
			})
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, externalContainerIP, externalContainer.GetPort())
				}, 5*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with loadbalancer's ingress ip")
			}

			gomega.Consistently(func() error {
				for _, pod := range pods {
					if err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, svcIP, externalContainerIP, externalContainer.GetPort()); err != nil {
						return err
					}
				}
				return nil
...
`)
			...
			for i, pod := range pods {
				...
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, externalContainerIP, externalContainer.GetPort())
				}, 3*time.Second, 500*time.Millisecond).ShouldNot(gomega.HaveOccurred(), "failed to reach external container with node's ip")
...
`)
			...
			for _, pod := range pods {
				gomega.Eventually(func() error {
					return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, egressIP.String(), dstIP, externalContainer.GetPort())
...
`)
				...
				providerPrimaryNetwork, err := provider.Get().PrimaryNetwork()
				framework.ExpectNoError(err, "provider primary network must be available")
				nodeNetworkInterface, err := provider.Get().GetK8NodeNetworkInterface(createdPods[0].Spec.NodeName, providerPrimaryNetwork)
		...
		*/
		...
		ginkgo.BeforeEach(func() {
			...
			for _, net := range []*netSettings{net1, net2} {
				...
				extContainerSecondaryNetPort := provider.Get().GetExternalContainerPort()
				extContainerSecondaryNet := provider.ExternalContainer{Name: net.containerName, Image: images.AgnHost(), Network: network,
					Args: []string{"netexec", fmt.Sprintf("--http-port=%d", extContainerSecondaryNetPort)}, ExtPort: extContainerSecondaryNetPort}
				...
				out, err := provider.Get().ExecExternalContainerCommand(extContainerSecondaryNet, []string{"ip", "address", "add", sharedIPv4 + "/32", "dev", "lo"})
				...
				out, err = provider.Get().ExecExternalContainerCommand(extContainerSecondaryNet, []string{"ip", "address", "add", sharedIPv6 + "/128", "dev", "lo"})
				...
				for _, node := range nodes {
					...
					_, err = provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "route", "add", sharedIPv4, "via", extContainerSecondaryNet.GetIPv4(), "table", net.routingTable})
					...
					_, err = provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "-6", "route", "add", sharedIPv6, "via", extContainerSecondaryNet.GetIPv6(), "table", net.routingTable})
					...
					providerCtx.AddCleanUpFn(func() error {
						out, err := provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "route", "flush", "table", net.routingTable})
						...
						out, err = provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "-6", "route", "flush", "table", net.routingTable})
						if err != nil && !strings.Contains(err.Error(), "FIB table does not exist") {
							return fmt.Errorf("unable to flush table %s on node %s: out: %s err: %v", net.routingTable, node.Name, out, err)
						}
						return nil
					})
					netNetworkInf, err := provider.Get().GetK8NodeNetworkInterface(node.Name, network)
		...
		ginkgo.XDescribeTable("[LGW] Should validate pods on different networks can reach different clients with same ip without SNAT",
			func(protocol v1.IPFamily) {
...
`)
					...
					setSVCRouteOnExternalContainer(providerCtx, provider.ExternalContainer{Name: net.containerName},
				...
				for _, net := range []*netSettings{net1, net2} {
					for i, pod := range net.createdPods {
						...
						gomega.Eventually(func() error {
							return curlAgnHostClientIPFromPod(f.Namespace.Name, pod, expected, dst, externalContainer.GetPort())
						...
						gomega.Consistently(func() error {
							return curlAgnHostHostnameFromPod(f.Namespace.Name, pod, net.containerName, dst, externalContainer.GetPort())
					...
					reachAllServiceBackendsFromExternalContainer(provider.ExternalContainer{Name: net.containerName}, net.serviceIP, podHTTPPort, net.createdPods)
				}
				...
				gomega.Consistently(func() error {
					for _, pod := range append(net1.createdPods, net2.createdPods...) {
						err := curlAgnHostClientIPFromPod(f.Namespace.Name, pod, "", dst, externalContainer.GetPort())
				...
				reachAllServiceBackendsFromExternalContainer(provider.ExternalContainer{Name: net1.containerName}, net1.serviceIP, podHTTPPort, net1.createdPods)
				reachAllServiceBackendsFromExternalContainer(provider.ExternalContainer{Name: net2.containerName}, net2.serviceIP, podHTTPPort, net2.createdPods)
			},
			ginkgo.Entry("ipv4 pods", v1.IPv4Protocol),
			ginkgo.Entry("ipv6 pods", v1.IPv6Protocol),
		)
...
func setSVCRouteOnExternalContainer(providerCtx provider.Context, container provider.ExternalContainer, svcIP, v4Via, v6Via string) {
	if utilnet.IsIPv4String(svcIP) {
		out, err := provider.Get().ExecExternalContainerCommand(container, []string{"ip", "route", "replace", svcIP, "via", v4Via})
		framework.ExpectNoError(err, "failed to add the service host route on the external container %s, out: %s", container, out)
		providerCtx.AddCleanUpFn(func() error {
			out, err = provider.Get().ExecExternalContainerCommand(container, []string{"ip", "route", "del", svcIP, "via", v4Via})
			if err != nil {
				return fmt.Errorf("failed to remove static service route on external container: stdout %q, err: %q", out, err)
			}
			return nil
		})
		return
	}
	out, err := provider.Get().ExecExternalContainerCommand(container, []string{"ip", "-6", "route", "replace", svcIP, "via", v6Via})
	providerCtx.AddCleanUpFn(func() error {
		out, err = provider.Get().ExecExternalContainerCommand(container, []string{"ip", "route", "del", svcIP, "via", v6Via})
		if err != nil {
			return fmt.Errorf("failed to remove static service route on external container: stdout %q, err: %q", out, err)
		}
		return nil
...
func setSVCRouteOnContainer(container provider.ExternalContainer, svcIP, v4Via, v6Via string) {
	var out string
	var err error
	if utilnet.IsIPv4String(svcIP) {
		out, err = provider.Get().ExecExternalContainerCommand(container, []string{
...
func curlServiceAgnHostHostnameFromExternalContainer(container provider.ExternalContainer, svcIP, svcPort string) (string, error) {
	dst := net.JoinHostPort(svcIP, svcPort)
	out, err := provider.Get().ExecExternalContainerCommand(container, []string{"curl", "-s", "--retry-connrefused", "--retry", "2", "--max-time", "0.5",
...
func reachAllServiceBackendsFromExternalContainer(container provider.ExternalContainer, svcIP, svcPort string, svcPods []string) {
...
func setBlackholeRoutingTableOnNodes(providerCtx provider.Context, nodes []v1.Node, extContainer provider.ExternalContainer, routingTable string, useV4 bool) {
	for _, node := range nodes {
		if useV4 {
...
func setBlackholeRoutesOnRoutingTable(providerCtx provider.Context, nodeName, ip, table string) {
	type route struct {
		Dst string `json:"dst"`
		Dev string `json:"dev"`
	}
	out, err := provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "--json", "route", "get", ip})
	...
	routeTo := routes[0]
	out, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "route", "replace", ip, "dev", routeTo.Dev, "table", table, "prio", "100"})
	...
	providerCtx.AddCleanUpFn(func() error {
		out, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "route", "del", "blackhole", ip, "table", table})
	...
	out, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "route", "replace", "blackhole", ip, "table", table, "prio", "50"})
	providerCtx.AddCleanUpFn(func() error {
		out, err = provider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "route", "del", "blackhole", ip, "table", table})
		if err != nil && !isAlreadyDeletedFn(err.Error()) {
...
func delExternalClientBlackholeFromNodes(nodes []v1.Node, routingTable, externalV4, externalV6 string, useV4 bool) {
	for _, node := range nodes {
		if useV4 {
			out, err := provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "route", "del", "blackhole", externalV4, "table", routingTable})
		...
		out, err := provider.Get().ExecK8NodeCommand(node.Name, []string{"ip", "route", "del", "blackhole", externalV6, "table", routingTable})
