		...
		t.Run(fmt.Sprintf("%d:%s", i, tc.desc), func(t *testing.T) {
			...
			for _, existingPrimaryNodeIP := range tc.existingPrimaryNodeIPs {
				if existingPrimaryNodeIP.v4.ip != "" {
					...
					g.Expect(err).ToNot(gomega.HaveOccurred(), "should succeed in allocating next IPv4 address")
				...
				if existingPrimaryNodeIP.v6.ip != "" {
					...
					g.Expect(err).ToNot(gomega.HaveOccurred(), "should succeed in allocating next IPv6 address")
