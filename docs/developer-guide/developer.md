...
intermediate update. A pending add followed by a delete remains ordered, and both callbacks
are delivered. Handler registration may already have delivered the add from its initial store
snapshot, so dropping the queued pair could leave stale state.
