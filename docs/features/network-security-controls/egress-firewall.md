...
## DNS-backed address set recovery

When the legacy DNS resolver refreshes an EgressFirewall rule, it recreates
the corresponding OVN address set if its database row was removed. The next
refresh then applies the current DNS results to the recreated address set, so
the rule can recover without requiring the EgressFirewall resource to be
recreated. This recovery does not change the rule's matching semantics.
