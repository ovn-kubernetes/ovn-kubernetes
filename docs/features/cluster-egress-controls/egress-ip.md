...
In case of an egress IP assigned to a secondary host interface, a in each node an nftable chain is added in postrouting hook with priority `srcnat+1` 
with a set of rules to drop traffic from any pod CIDR subnet:
```shell
table inet ovn-kubernetes {
	chain ovn-kube-egress-ip {
		...
		ip saddr 10.244.0.0/16 drop comment "Drop egress IP pod traffic from 10.244.0.0/16"
	}
}
```
