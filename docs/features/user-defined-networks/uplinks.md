...
If multiple `nodeConfigs` select the same node, the `Uplink` is degraded and a
...
* `Degraded=True` means one or more selected nodes do not have usable
...
```bash
kubectl get uplinkstate -l k8s.ovn.org/uplink=tenant-blue -o wide
...
* `Uplink` with `Degraded` condition status `True` and reason
