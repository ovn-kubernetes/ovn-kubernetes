...
not tracked: the node reports a `UDNKubeletRestartTrackingUnavailable`
event, and probes to primary UDN pods stop working once kubelet's cgroup
is recreated, until ovnkube-node is restarted. Such a host should also set
`global.hostHasSystemd: false` in the helm chart, so the daemonset does not
mount the systemd socket it cannot use.

A stale rule can be recognised from the ruleset, because nft prints the
cgroup ID it can no longer resolve instead of the path:

```text
socket cgroupv2 level 2 669 ip daddr @udn-pod-default-ips-v4 accept
```

A cgroup that outlives kubelet avoids this, at the cost of the wider
