...
The introduced controller duplicates source EndpointSlices, creating
new copies that include IP addresses suitable for the primary user-defined
network. It bypasses EndpointSlices in namespaces that do not have a
user-defined primary network. For Pod TargetRefs from selector-based Services,
it replaces default-network addresses with UDN addresses from OVN-Kubernetes
annotations. For host-networked pods, the controller retains the same IP
addresses as the source EndpointSlice. For selector-less Services with manual
Endpoints (EndpointSlices managed by `endpointslicemirroring-controller.k8s.io`),
non-Pod endpoint addresses are mirrored as-is, including node or external IPs
outside the UDN CIDR.

Source EndpointSlices that are mirrored include those managed by:
...
- `endpointslice.kubernetes.io/managed-by:endpointslicemirroring-controller.k8s.io` - Indicates
  that the EndpointSlice is managed by the Endpoints mirroring controller (selector-less
  Services with manual Endpoints).

Those source EndpointSlices also use:
...
- `k8s.ovn.org/source-endpointslice-version:<source-endpointslice-resourceversion>` - The
  last reconciled resource version from the source EndpointSlice.
...
- `k8s.ovn.org/source-endpointslice:<source-endpointslice>` -  The name of the
  source EndpointSlice that was mirrored (default controller or Endpoints
