...
A local kind cluster can enable route advertisements with `contrib/kind.sh -rae`.
...
```shell
$ hack/demo/demo.sh <second-session-interface> <second-session-vrf> [<second-session-vlan> [<ipv4-sed> [<ipv6-sed>]]]
```
