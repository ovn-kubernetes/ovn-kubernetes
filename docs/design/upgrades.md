...
traffic is allowed to depend on the upgrade being quick or atomic. The order in
which the components are replaced is fixed, see below, and the rest of this
document assumes it.
...
### Roll-out order: nodes first, then the control plane

The supported order is fixed: `ovnkube-identity`, then the `ovnkube-node`
DaemonSet on every node, then `ovnkube-control-plane`. A new cluster-manager
starts only once every node runs the new release. Nodes still roll one at a
time and, where the platform also updates the host OS, a node is drained and
rebooted some time after its ovnkube pod was replaced, so upgraded and
non-upgraded nodes coexist for hours or days.

Three reasons for fixing the order rather than supporting both:

* **One sequence is easier to get right and to test.** With an arbitrary
  order every change has to work for new nodes with an old cluster-manager and
  for old nodes with a new cluster-manager, and the test matrix has to cover
  both. With a fixed order there is one skew to reason about and one lane to
  run. The upstream upgrade job (`ovn-upgrade-e2e`) enforces it: it pauses the
  `ovnkube-control-plane` Deployment, rolls the `ovnkube-node` DaemonSet to
  the new image, runs the e2e suites against that half-upgraded cluster, then
  resumes the control plane and runs them again
  (`test/scripts/staged-upgrade-ovn.sh`, stages `nodes-first` and
  `control-plane-last`).
* **It is the order that has been exercised in practice.** OpenShift has
  always upgraded worker nodes before control-plane nodes, so new
  `ovnkube-node` pods running against an old cluster-manager is the
  combination that production upgrades have been hitting for years.
* **It lets the cluster-manager assume new nodes.** The cluster-manager is
  the component that publishes cluster-wide state: annotations and labels on
  Nodes and NetworkAttachmentDefinitions, CRD status, identifiers. When it
  starts on the new release every consumer of that state already runs the new
  release, so it can publish a new key, a new value shape or a new allocation
  scheme immediately, without a compatibility mode for old nodes and without
  waiting for a later release to switch. Only the opposite direction needs
  care: a new node must keep working with an old cluster-manager for the
  duration of the node roll, see the fallback rule below.

[OKEP-6943](https://github.com/ovn-kubernetes/ovn-kubernetes/pull/6950), which
raises the number of networks a cluster may hold, is the worked example of the
third point. Today the tunnel key of a network's transit switch is derived
arithmetically from its network ID by every `ovnkube-controller`; the OKEP moves
the key into a NAD annotation written by the cluster-manager together with the
ID, so that keys can come from an allocator instead of the ID. If the
cluster-manager could be upgraded first, an old controller would keep deriving
keys for networks the new cluster-manager had already given allocated keys, and
the two would disagree; avoiding that needed either two releases, an
operator restriction on creating networks during the upgrade, or a runtime
gate. With nodes first, every controller reads the annotation before the new
cluster-manager writes its first allocated key, and a new controller facing an
old cluster-manager simply derives the key as before because the NAD carries
an ID and no keys. The whole change fits in one release with a one-line
fallback.

The combinations that must work inside the one-release window are therefore:

* new `ovnkube-node` with old `ovnkube-control-plane`, for as long as the node
  roll takes,
* upgraded and non-upgraded nodes talking to each other across the
  interconnect for hours or days,
* new `ovnkube-identity` with everything else old.

Old `ovnkube-node` with new `ovnkube-control-plane` is not a supported
combination. A change may rely on it never occurring, and platforms that
upgrade the control plane before the nodes are outside this model.

In central mode (not supported anymore) the order was fixed for a different
reason: a single `ovnkube-master` programmed the shared NB database, so a new
master could not start until every `ovnkube-node` could consume what it would
write, and the master's one-shot migrations assumed all nodes were new.
Interconnect removed that constraint by giving every node its own database and
its own migration. The order is kept as policy for the reasons above, not
because the databases require it.
...
`ovnkube-node` of that release starts, which is why it is the first step of
the order above. This is
...
**Adjacent releases interoperate in the supported order.** Release N+1 of
`ovnkube-node` works with release N of the cluster-manager, release N+1 nodes
work alongside release N nodes, and release N+1 of `ovnkube-identity` works
with release N of everything. A release N+1 cluster-manager may assume release
N+1 nodes. Nothing older than N needs to be understood, and nothing needs to
remain readable by N once N+1 has run.
...
**Fall back when the cluster-manager is older.** If ovnkube-node starts
needing data the new cluster-manager publishes, it must keep working with the
old logic for at least one release, because the cluster-manager is upgraded
after the nodes and the node roll can take days. The converse is not required:
the cluster-manager may publish state only new nodes understand from its first
start, since no old node is running by then. The network ID move from Node
annotations to NAD annotations
...
release to the PR image nodes first and runs the e2e suites both with the
control plane still on the previous release and after it is upgraded, plus
unit tests for the mixed-release decision logic.
...
* Does ovnkube-node need something new from the cluster-manager? Is there a
  fallback for nodes that run against the old cluster-manager during the node
  roll?
* Does the cluster-manager publish something only new nodes understand? That
  is allowed, but check that no node needs it before the cluster-manager is
  upgraded, and that the change does not depend on old nodes ever seeing it.
...
* [OKEP-6943: Increase the MaxNetworks limit](https://github.com/ovn-kubernetes/ovn-kubernetes/pull/6950),
  the worked example of a cluster-manager change that relies on the node-first
  order.
* `test/scripts/staged-upgrade-ovn.sh` and the `ovn-upgrade-e2e` job in
