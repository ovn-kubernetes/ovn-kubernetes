...
The examples below use a two-node cluster, each node in its own zone:
...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 30, 'rankSpacing': 50, 'padding': 20}, 'themeVariables': {'fontSize': '16px'}}}%%
graph TD
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
    classDef ts fill:#e0f2f1aa,stroke:#00695C,stroke-width:1.5px,color:#004D40
    classDef host fill:#f3e5f5aa,stroke:#6A1B9A,stroke-width:1.5px,color:#4A148C

    subgraph zone1["ovn-worker zone · 172.19.0.2"]
        eth1["eth0"]:::host --- br1["breth0 · br-int"]:::host
    ...
    subgraph zone2["ovn-worker2 zone · 172.19.0.3"]
        eth2["eth0"]:::host --- br2["breth0 · br-int"]:::host
...
Both pods are on **ovn-worker**. Traffic stays entirely local — no tunnel.

```mermaid
%%{init: {'flowchart': {'nodeSpacing': 30, 'rankSpacing': 60, 'padding': 15}, 'themeVariables': {'fontSize': '20px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C

    A["Pod A · 10.244.0.5"]:::pod -->|"1 · egress"| LS["Logical Switch"]:::sw
    LS -->|"2 · route lookup"| CR["ovn_cluster_router"]:::rtr
    CR -->|"3 · forward"| LS
    LS -->|"4 · deliver"| B["Pod B · 10.244.0.8"]:::pod
```

---

### Pod2Pod – Different Node
...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 60, 'rankSpacing': 50, 'padding': 35}, 'themeVariables': {'fontSize': '60px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 30, 'rankSpacing': 60, 'padding': 15}, 'themeVariables': {'fontSize': '30px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
    classDef ts fill:#e0f2f1aa,stroke:#00695C,stroke-width:1.5px,color:#004D40
    classDef decision fill:#fff8e1aa,stroke:#F57F17,stroke-width:1.5px,color:#E65100
    ...
    DECISION -->|"Different node"| TS["Transit Switch → Remote Pod"]:::ts
```

---

### Pod2LoadBalancerIP/ExternalIP (ovn pod backend)
...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 40, 'rankSpacing': 60, 'padding': 15}, 'themeVariables': {'fontSize': '30px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
...
### Pod2NodePort (ovn pod backend) – Same Node NodePort Access
...
### Pod2NodePort (ovn pod backend) – Different Node NodePort Access
...
### Pod2ClusterIP/LoadBalancerIP/ExternalIP (host networked pod backend) – Same Node

The endpoint is a **host-networked pod on the same node**. After DNAT,
traffic must leave OVN to reach the host via the gateway router.
...
graph LR
    ...
    JS -->|"4"| GR["GR_ovn-worker"]:::rtr
    GR -->|"5"| ES["External Switch"]:::sw
    ES -->|"6"| BR["breth0"]:::sw
    BR -->|"7"| HNP["Host Networked Pod"]:::host
```

---

### Pod2ClusterIP/LoadBalancerIP/ExternalIP (host networked pod backend) – Different Node
...
### Pod2NodePort (host networked pod backend) – Same Node NodePort Access

Similar to the ClusterIP → host-networked case above, but the entry
point is a NodePort VIP.
...
### Pod2NodePort (host networked pod backend) – Different Node NodePort Access
...
### Pod2External: routingViaHost=false
...
graph LR
    ...
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
    classDef ext fill:#ffebeeaa,stroke:#B71C1C,stroke-width:1.5px,color:#7F0000
...
### Pod2External: routingViaHost=true

Local gateway mode. Egress traffic still traverses the GR but the GR
does **not** SNAT. Instead, OpenFlow on br-ex sends the packet to the
host kernel (LOCAL port), where iptables/nftables masquerade handles it.
...
graph LR
    ...
    LS -->|"2"| CR["ovn_cluster_router"]:::rtr
    CR -->|"3"| JS["Join Switch"]:::sw
    JS -->|"4"| GR["GR · no SNAT"]:::rtr
    GR -->|"5"| ES["External Switch"]:::sw
    ES -->|"6"| BR["br-ex · LOCAL port"]:::sw
    BR -->|"7"| HOST["Host · masquerade"]:::host
    HOST -->|"8"| EXT["External"]:::ext
```

---

### Pod2External via EgressIPs – Same Node; EIP on primary br-ex shared network
...
### Pod2External via EgressIPs – Same Node; EIP on secondary network

The GR SNATs to the EgressIP and routes out through the secondary bridge.

```mermaid
%%{init: {'flowchart': {'nodeSpacing': 80, 'rankSpacing': 50, 'padding': 65}, 'themeVariables': {'fontSize': '60px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
    ...
    CR -->|"3"| JS["Join Switch"]:::sw
    JS -->|"4"| GR["GR · SNAT → EgressIP"]:::rtr
    GR -->|"5"| ES2["External Switch · secondary"]:::sw
    ES2 -->|"6"| ETH2["Secondary NIC"]:::sw
    ETH2 -->|"7 · src: EgressIP"| EXT["External"]:::ext
```

---

### Pod2External via EgressIPs – Destination towards other Nodes in cluster
...
graph LR
    ...
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
    classDef mgmt fill:#eceff1aa,stroke:#37474F,stroke-width:1.5px,color:#263238
...
### Pod2External via EgressIPs – Different Node; EIP on primary br-ex shared network
...
### Pod2External via EgressIPs – Different Node; EIP on secondary network

Same as above, but egress happens through a **secondary network** interface.
...
graph LR
    ...
    CR2 -->|"5"| JS2["Join Switch"]:::sw
    JS2 -->|"6"| GR2["GR · SNAT → EgressIP"]:::rtr
    GR2 -->|"7"| ES2["External Switch · secondary"]:::sw
    ES2 -->|"8 · src: EgressIP"| EXT["External"]:::ext
```
...
### External2NodePort/ExternalIP/LoadBalancerIP (ovn pod backend) – Same Node: routingViaHost=false
...
### External2NodePort/ExternalIP/LoadBalancerIP (ovn pod backend) – Different Node: routingViaHost=false
...
### External2NodePort/ExternalIP/LoadBalancerIP (ovn pod backend) – Same Node: routingViaHost=true

Local gateway mode. External traffic enters the **host routing stack**,
then enters OVN via the management port.

```mermaid
%%{init: {'flowchart': {'nodeSpacing': 60, 'rankSpacing': 50, 'padding': 35}, 'themeVariables': {'fontSize': '40px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef host fill:#f3e5f5aa,stroke:#6A1B9A,stroke-width:1.5px,color:#4A148C
...
### External2NodePort/ExternalIP/LoadBalancerIP (ovn pod backend) – Different Node: routingViaHost=true
...
### External2NodePort/ExternalIP/LoadBalancer (ovn pod backend) ETP=local with EgressIPs
...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 60, 'rankSpacing': 50, 'padding': 35}, 'themeVariables': {'fontSize': '50px'}}}%%
graph LR
    classDef pod fill:#e8f5e9aa,stroke:#2E7D32,stroke-width:1.5px,color:#1B5E20
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
...
```mermaid
%%{init: {'flowchart': {'nodeSpacing': 30, 'rankSpacing': 50, 'padding': 15}, 'themeVariables': {'fontSize': '11px'}}}%%
graph TD
    classDef sw fill:#e3f2fdaa,stroke:#1565C0,stroke-width:1.5px,color:#0D47A1
    classDef rtr fill:#fff3e0aa,stroke:#E65100,stroke-width:1.5px,color:#BF360C
    classDef ts fill:#e0f2f1aa,stroke:#00695C,stroke-width:1.5px,color:#004D40
    ...
    ES --> BRINT["br-int ↔ breth0 ↔ eth0"]:::sw
```
