...
failures are retried with exponential backoff. Pods are only processed after
nodes and services are synced. Host-network pods are skipped (they do not
need an OVN logical port).
...
The CNI binary (`CmdAdd`) is invoked by kubelet. It acts as a thin shim
that forwards the request over a Unix domain socket to the CNI server
running inside ovnkube-node.
...
`ConfigureInterface()` and `ConfigureOVS()` perform the following steps:
