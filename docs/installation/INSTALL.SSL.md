...
connections to the control-plane node.

Copy this certificate to the control-plane node and each of the worker nodes. $CENTRAL_IP
is the IP address of the control-plane node.
...
### Generate signed certificates for OVN components running on the control-plane node.

#### Generate signed certificates for OVN NB Database
On the control-plane node, run the following commands.
...
to the control-plane node's /etc/openvswitch. The ovnnb-privkey.pem and ovnnb-cert.pem
...
to the control-plane node's /etc/openvswitch. The ovnsb-privkey.pem and ovnsb-cert.pem
