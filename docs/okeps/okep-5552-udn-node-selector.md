...
for any topology type. Additionally, if a pod 
...
controller manager containers. The breakdown of how these will be modified is outlined below:
 ...
 - Cluster Manager
   - UDN Controller - Modified to do any nodeSelector validation/unit tests
   - Route Advertisements Controller - No change
   - Egress Service Cluster - Doesn't support UDN
   - Endpoint Mirror Controller - No change
   - EgressIP Controller - Modified to not pick egress nodes that do not match nodeSelector
   - Unidling Controller - No change
   - DNS Resolver - No change
   - Network Cluster Controller - Modified to ignore node allocation for networks that do not match nodeSelector
...
that filters based on if the UDN nodeSelector matches this node or not.
