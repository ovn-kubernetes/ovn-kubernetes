...
- As an admin I want to be able to perform day2 changes to the localnet topology spec following env changes.
  - Change VLAN, e.g: after the admin decommissioned the VLAN in use, in favor of new VLAN.
  - Changing MTU, e.g: re-aligning the logical network MTU with physical network MTU.
  - Change the localnet topology bridge-mapping - `physicalNetworkName`, e.g.: following a requirement to connect to different interfaces on the node.
...
>> *Note*: Semantically the annotation indicate on any network configuration change, although not all network setting require taking immediate action.
>> We decided to add the annotation on any settings change as the network-config is outdated, and the latest should be consumed to ensure integrity of the network state. 
>> Being more precise in the future is an option; avoid annotating on any change, because some changes may not require immediate action (such as VLAN change). 
...
The proposed annotation for indicating whether workload configured with outdated network-config, servers as the baseline for 
additional developments for improving how this state is communicated to users. 
Please see the [Alternatives](#indicating-pod-configured-with-outdated-network-config) section for more details.
...
Allowing users to mutate the mentioned settings on day2 enables them to act on environment changes that require OVN network setting change,
saving the hassle of re-creating the whole project (delete pods, delete network CR, create the network CR with new settings, create pods ...).

Naturally, changing network setting while workloads connected to it has implications, may introduce transient network disruption, 
which manifests in packet-drops, latency and disconnections.

Having said that mutating the suggested settings should not break the workloads networking, and they should be able to communicate
with each other.
After the network setting changes, restarting the workloads is needed in order for them to consume the latest network-config.
Once all connected workloads configured with the latest network-config the state should be reconciled and no network-disruption is expected.

Please refer to [Risks Known Limitations and Mitigations](#risks-known-limitations-and-mitigations) section for more details.  
...
add the proposed annotation `k8s.ovn.io/network-out-of-sync` to all connected pods in the selected-namespaces.
In case the annotation is already exists and has items (networks), the controller should append the subject network to the list. 
...
| VLAN                | Discriminated union for VLAN configurations for the network<br>`mode`:<br>When `Access`, OVN-Kubernetes apply the VLAN configuration the network logical switch port in access mode, according to the config.<br>`access` the access VLAN configuration<br>`access.id` The VLAN ID (VID) to be set on the logical network switch.                                                                                                                                                                                                                                                                                            | Yes      |
...
As of today localnet topology allows setting VLAN for the network.
...
Although option (1) being pretty straight forward, in case additional VLAN configurations should be supported requiring 
exposing additional fields, it will enforce having VLAN related filed right next to non VLAN related ones, making the API look occurred.

In addition, in case future VLAN additions introduce mutual exclusive relation between VLAN config related fields,
the flat structure make it harder to maintain the related API validations markers. (not centralized, lives alongside other validations make them harder to follow).

Option (2) enable all related VLAN configuration lives under the same roof enable the API to evolve nicely, introducing less noise comparing to (1).
...
follow the same convention, that is having complex filed for VLAN configurations. 

The proposed options for VLAN configurations is (2)

```go
// VLANID is a VLAN ID (VID), should be higher than 2, and lower than 4094.
...
# Network disruption following network configuration change
As mentioned earlier in the doc, changing one of the proposed network settings may introduce transient network disruption
to the connected workloads.
The workloads networking should not break, and workloads should be able to communicate with each other. 

The proposed solution suggest to allow users mutate the following settings for localnet topology networks: 
`VLAN`, `MTU`, `PhysicalNetworkName` and `ExcludeSubnets`.

See below description on how each setting change may or may not affect the workloads:
- `VLAN`, `PhysicalNetworkName`:
  Changing `VLAN` or `PhysicalNetworkName` should not introduce network disruption to connected workloads.
  In edge cases there could be some packet drops, but no TCP connection breakage is expected.
- `MTU`
  As of today the CNI configure the MTU on the pod interface. The CNI is executed on pod create or deletion.
  In order for MTU change take effect the workloads should be re-created in order to have the CNI apply the new MTU settings.

  In a scenario some pods configured with new MTU settings, and other pods configured with old MTU settings.
  - Pods should be able to communicate with each other.
  - If the external network interface MTU is higher than old MTU settings, there might be network disruption causing packet fragmentation.

- `ExcludeSubnets`
  ExcludeSubnet can be configured when `subnets` is configured, in this case OVN-Kubernetes will assigned pod IPs to pods from the specified
  value in `subnets`.
  When `ExcludeSubnet` is set, OVN-Kubernetes exclude the specified IP address from the pool and wont assign IPs in this range to pods.

  Adding or removing an exclude-subnet should not introduce network disruption whatsoever, as this setting effects OVN-Kubernetes IPAM service
  and not the pods network-stack directly.

  In a scenario the external network introduce new "service" whose IP address now considered reserved, e.g.: 10.0.0.10.
  In case there the address 10.0.0.10 already assigned a pod, both the "service" and the pod may suffer from network disruption  
  due to IP address conflict.
  Once the pod with the assigned IP is re-created, it will get different, non excluded, IP address from the pool, and the IP conflict
  should be resolved.

  Mitigation for this can be providing indication about this state - excludeSubnet cannot be set because IP in its range already
  been assigned to pods.


Once all the connected workloads consume the new network-config, no network disruption should occur as all connected workloads
network settings are aligned, using the latest network-config.

## OVN Kubernetes Version Skew

## Alternatives

### Indicating pod configured with outdated network-config
Following discussions about the proposed solution for indicating pod configured with outdated network-config,
some alternatives and improvements were suggested.
The proposed solution serves as a baseline for most suggested alternatives.
Most alternative solutions are additions serving as an additional matter for providing similar indication, as the proposed one.

#### 1. Emit events on pods 
Similar to the idea of the proposed solution, but with emitting events on the connected pods.
Following CUDN CR localnet spec change, the CUDN controller should emit event on connected pods.

The event should indicate pod network configuration is outdated, and suggest restarting to consume the latest network config.
...
Emitting events is a complementary matter for indicating the subject state, alongside the proposed solution.
It can be done as a follow-up upon necessity and feedback from users.

#### 2. Add condition to pod
Similar to the idea of the proposed solution and option (1), but with adding status condition to the connected pods.

Following CUDN CR localnet spec change, the CUDN controller should add a pod condition on the connected pods,
providing indication about OVN-K network configuration state.
The pod condition should be `true` when the pod network-config is up-to-date.
The pod condition should be `false` when the pod network-config is outdated.

Adding condition to pods is a complementary matter for indicating the subject state, alongside the proposed solution.
It can be done as a follow-up upon necessity and feedback from users.

#### 3. CUDN CRD status condition
...
The proposed solution serve as the baseline for providing the status condition, as the indication for when the 
status condition should be true or false.

Adding status condition is a complementary matter for indicating the subject state, alongside the proposed solution.
It can be done as a follow-up upon necessity and feedback from users.

#### 4. Provide collective data for alerting framework
...
Usually alerting framework enable setting a notification on the platform GUI with a nice message referencing 
to instructions on how to address it.

The proposed pod annotation can serve as the baseline for providing collective data for alerting frameworks,
as the indication for raising an alert, e.g.: when there is at least on pod with outdated network-config.

Alerts are a complementary matter for indicating the subject state, alongside the proposed solution.
It can be done as a follow-up upon necessity and feedback from users.

#### 5. Add support for OVN-K CNI plugin to configure running pods  
...
And a controller to monitor NADs and trigger the CNI on spec changes, to update the pods  network-stack with the latest
...
This alternative may require its own design and consensus among the community.
In case this alternative is introduced it can replace the proposed one, there should be no backward compatability issues
because such CNI won't need to rely on the proposed annotation.

In addition, having OVN-K CNI re-configure running pods has potential to improve the UX for spec
mutation support, as it won't require re-creating the workloads.
For Kubevirt VMs the impact is event greater, as usually VMs should have minimal downtime.
