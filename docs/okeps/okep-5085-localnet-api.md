...
The the ClusterUserDefinedNetwork CRD should be extend to support localnet topology.
...
- `ExcludeSubnets`:
    ...
    - Ensure excluded subnet in range of at least on subnet in `spec.network.localnet.subents`
...
#### YAML examples
Assuming the node has OVS bridge-mapping defined by [Kubenetes-nmstate](https://nmstate.io/kubernetes-nmstate/) 
...
As mentioned in [Workflow Description](#workflow-description) section, its possible to mutate the spec 
...
4. E2e test verifying Kubevirt VMs can communicate over locanet topology network created using CUDN CR.
...
Validating specified subnets exclusions (`spec.netwrok.localnet.excludedSubnets`) are in range of the specified topology subnets 
(`spec.netwrok.localnet.excludedSubnets`) is currently impossible due to a bug in the CEL rule library for validation IPs and CIDRs.
