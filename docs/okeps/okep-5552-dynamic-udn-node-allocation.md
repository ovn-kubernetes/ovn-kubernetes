 ...
 * To dynamically allocate subnets and other per-node+UDN information.
...
Furthermore, there is a desire to dynamically allocate per-node network assignments, such as per node subnet allocation.
