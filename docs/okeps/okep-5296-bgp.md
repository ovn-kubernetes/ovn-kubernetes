...
A networkSelector may select more than just the cluster default network, including user-defined networks. In such a case
the network subnets will be checked by OVN-Kubernetes to determine if there is any overlap of the IP subnets. If so, an
error status will be reported to the CRD and no BGP configuration will be done by OVN-Kubernetes.
...
Changes will be made to the CUDN and UDN CRDs in order to specify the method of transport for east/west traffic. A new
...
routed between each other. Note, this security limitation only applies to pod addresses. If a group of pods are exposed
externally as service, external routing from another UDN will succeed.
...
destination node. In the future, routing optimizations will be implemented which break up contiguous address blocks of the
