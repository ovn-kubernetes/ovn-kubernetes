...
- Deliver a production-ready external IPAM service, **Atlas IPAM**
  (`atlas-ipam`), under the `ovn-kubernetes` GitHub organization. This is
  not a sample or a demo; it should be deployable and operable in
  production out of the box: highly available (see the HA goal below),
  durable, and observable. As the first conforming backend, Atlas also
...
backend can implement it, from the first-party bbolt-based Atlas
...
[Goal](#goals)), not just a sample implementation. Because it is open source
...
(the storage engine underneath etcd), which makes it a proven, low-risk
default for a single-process allocator. It is not part of the contract:
persistence lives in the `store/` package behind a small interface, so
Atlas can swap bbolt for another embedded engine (e.g.
Pebble, SQLite) without changing the HTTP API or OVN-Kubernetes. The
choice is not load-bearing, and alternatives that suit operators better
...
**TBD (to be resolved during reviews):** which of these options Atlas
adopts. Each trades implementation effort
...
| **Bare-metal / VM host** | `https://ipam.example.net:9500` | ✅ | No Kubernetes involved; the off-cluster and cross-platform (Stories 7/8) case. |
...
[Story 7](#story-7-cross-platform-migration) and
[Story 8](#story-8-shared-ipam-as-the-single-source-of-truth-during-migration).
...
spec:
  ...
  - matches:
    ...
    backendRefs:
    - name: atlas-ipam              # Service fronting the StatefulSet
      port: 9500
```
...
flat-pool allocator, not a DDI system — its bbolt backend
...
`/release`, `/status`) is vendor-neutral. Enterprise organizations that
already run a DDI platform (Infoblox, BlueCat, or an open-source DHCP
server like ISC Kea) can deploy a thin **adapter** that translates Atlas
API calls into the vendor's native API, instead of replacing their
existing IP management infrastructure.
...
Atlas IPAM ships with the bbolt backend. Enterprise
...
2. **Pluggable backends in Atlas IPAM**: expand Atlas IPAM itself to
   ...
   The default backend is bbolt; additional backends (Infoblox,
...
Atlas IPAM for the design discussion.
...
(this maps to [HA option 3](#high-availability-and-db-replication)).
...
- [bbolt](https://github.com/etcd-io/bbolt)
  — Embedded key-value store used by Atlas IPAM
