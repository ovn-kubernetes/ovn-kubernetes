...
tenant). Not just redundant, they may potentially expose a rogue node to other
victim nodes that perhaps never meant to communicate with it.

## Goals

Allow cluster admins to dedicate Nodes to particular OVN transport zones,
...
### Story 3: Dedicate gateway nodes to communicate with isolated transport zones
...
in multiple (maybe all) transport zones. These dedicated gateway nodes would
then handle external and internal communications across disjoint transport
zones.

To make it work, some routing mechanism between isolated transport zones and
their respective gateway nodes is needed. (One option may be
`AdminPolicyBasedExternalRoutes`). Defining such mechanism is out of scope for
this OKEP.
...
support for Transport Zones. Transport zones are configured in local OVSDB by setting:

```bash
ovs-vsctl set open . external-ids:ovn-transport-zones=tz1
```

Then OVN controller configures its `Chassis:transport_zones` field in SBDB accordingly.
...
to guard against a rogue `Node` updating its own OVN-K transport zones.
