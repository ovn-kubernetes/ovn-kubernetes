...
In the above example, an EVPN CR is created that is matching on a CUDN label "mynetworkName: "blue", which will be used to
look up a matching CUDN with that label.
The only supported selector type at the time of this enhancement is clusterUserDefinedNetworkSelector type.
Any other type will result in an invalid CR. In the future, this may be expanded to allow the primary UDN type.

Additionally, the "vtep" configuration provides the subnet that will be used as VXLAN VTEP endpoints
...
mode, and will use the first found IP address within the provided "cidr" subnet. Note, that the VTEP CIDR may or may not
overlap between EVPN CRs. If there is overlap, the same VTEP will be used for both EVPN CRs.

The macVRF and ipVRF fields are optional, but at least one must be provided.
...
Note, in order to use EVPN, a Route Advertisement CR must also be created which selects teh same network. Additionally,
an FRRConfiguration CR is required to use BGP and EVPN. When the networkSelector finds a matching CUDN for this EVPN CR,
it will then find what BGP RA CR is selecting it, and then which FRRConfiguration references that RA CR. When all of these
pieces exist, then OVN-Kubernetes will create the proper FRR-K8S config for EVPN.
...
# ip link set br100 addr aa:bb:cc:00:00:6e  <-- SVI configuration for the bridge
# ip addr add 10.0.10.254/24 dev br100      <-- SVI configuration for the bridge
...
Rather than using an SVI interface on the linux bridge, OVN-Kubernetes will leverage the management port to serve as an
SVI port. The management port is already assigned an IP address that is common across all nodes in a layer 2 network. We
can route traffic to the ovn_cluster_router and then to the ovn-k8s-mpx management port the same way we do in local gw mode
today. The packet will then go into the blue VRF and routes learned by BGP will send it via the vni101 VXLAN tunnel.
