...
routes for each network's subnet to steer traffic to the correct destination. Specifically, the connect-router
maintains per node-subnet routes for Layer3 networks (e.g., `103.103.0.0/24 -> 192.168.0.12`) to route traffic
to the appropriate node.

When a new subnet is added to a connected UDN, the `connect-router` needs to be updated with routes for the new
subnet. The `network-connect-controller` in `ovnkube-controller` watches for NAD changes and should reconcile
when the network's subnets are updated.

A unit test should be added to ensure that when a UDN is part of a `ClusterNetworkConnect` and a new subnet is
added, the `connect-router` gets updated with the correct routes for the new subnet.
...
When a NAD changes today, we reconcile all BGP RAs. However, in the reconcilation, we check raNeedsUpdate, which only checks
...
A unit test should be added to make sure that when Egress IP is being used and a new a new subnet is added, then the default
