...
reconcile the default gw mac to the new node where the VM is running [merged PR](https://github.com/ovn-kubernetes/ovn-kubernetes/pull/4964). 

For ipv6 there is a work in progress PRs to do something similar by blocking external gateway
...
As an EIP L2 UDN user, the feature must be functional in disconnected environments with no external gateway. EIP traffic must respect multiple external gateways.
...
be a link local address computed from the subnbet address `2010:100:200::1`, again this makes the default gw __independent__ of the
...
Also, the transit router port connected to the switch needs to be configured as gateway router port for the conditional SNAT to work.
...
Anodher change is that the transit router LRP that connects to the switch has 
