...
When a virtual machine is live migrated it's transfer from the node where is
...
the VM do *not* initiate any type of ARP or Router Solicitation to reconcile
routes since from its point of view nothing has happened, this means it's running
...
so that tcp connections are not broken and downtime is minimum with network configuration 
...
This is a big picture of the topology with the transit subnet used to connect the ovn_cluster_router and gateway routers directly without a switch so we have a LRP to LRP connected
using the Peer field.

Routers cannot have ports at the same subnet, so the trtor (trasit router to router) ports at ovn_cluster_router need to have the minimal possible subnet, to accommodate at least 2 ips one per peer, from the 
...
so this makes it independent of where the VM is running.
...
flowchart TD
    ...
    class node1,node2,node3 nodeStyle;
```
From the IPv6 RFC the gateway address configured from router advertisements is a link local address, in this case it will
be a link local address computed from the subnbet address `2010:100:200::1`, again this makes the default gw independent of the
node where the VM is running on.

#### NAT configuration
As much as possible everything related to conntrack should not be modified since doing so can affect tcp connections, so the 
only SNAT that need to be moved from GR to ovn_cluster_router is the one enmasquerading the traffic that goes to the mgmt port from the network subnet, since
now ovn_custer_router is the router connected to the layer2 switch instead of GR, the rest of snat can continue to be done at gateway router.
```
llowed_ext_ips     : []
exempted_ext_ips    : []
...
Also the fact that join ip at gateway router is kept allow to maintain all the NATing done with it at gateway router, load balancer for services 

#### Routing an policies
Moving around routing and policies do not affect tcp connections on upgrade so we can move some routes and policies from gateway router
to ovn_cluster_router without issue, in general it will be similar to the layer3 routes and policies.

These will be the routes and policies at the new ovn_cluster_router 
```
IPv4 Routes
Route Table <main>:
...
And these the gateway router routes (there is no policies)
```
IPv4 Routes
Route Table <main>:
           169.254.0.0/17               169.254.0.4 dst-ip rtoe-GR_test12_namespace.scoped_ovn-control-plane
           203.203.0.0/16               100.88.0.13 dst-ip rtotr-GR_test12_namespace.scoped_ovn-control-plane <-- inbound egress traffic goes back to the ovn_cluster_router peer ip.
                0.0.0.0/0                172.18.0.1 dst-ip rtoe-GR_test12_namespace.scoped_ovn-control-plane
```

For the routes related to EIP they are moved from gateway router to ovn_cluster_router and the redirect points to the transit node peer ip.
...
Also the LRP connected to the switch need a gateway router configured for NAT to work.

```mermaid
%%{init: {"nodeSpacing": 20, "rankSpacing": 100}}%%
flowchart TD
    classDef nodeStyle fill:orange,stroke:none,rx:10px,ry:10px,font-size:35px;
    classDef vmStyle fill:blue,stroke:none,color:white,rx:10px,ry:10px,font-size:25px;
    classDef portStyle fill:#3CB371,color:black,stroke:none,rx:10px,ry:10px,font-size:25px;
    classDef routerStyle fill:brown,color:white,stroke:none,rx:10px,ry:10px,font-size:35px;
    classDef switchStyle fill:brown,color:white,stroke:none,rx:10px,ry:10px,font-size:25px;
    classDef termStyle font-family:monospace,fill:black,stroke:none,color:white,font-size:25px;
    subgraph node1["node1"]
        subgraph node1-ovn_cluster_router["ovn_cluster_router"]
            node1-tnl-key["requested-tnl-key=<b>16477216<b>"]
            node1-trtor-GR-node1["trtor-GR-node1<br>requested-tnl-key=<b>4</b>"]
            node1-trtor-GR-node2["trtor-GR-node2<br>requested-tnl-key=<b>3</b><br>requested-chassis=<b>node2</b>"]
            node1-trtor-GR-node3["trtor-GR-node3<br>requested-tnl-key=<b>2</b><br>requested-chassis=<b>node3</b>"]
            node1-rtos-layer2-switch["rtos-layer2-switch<br>gateway_chassis=<b>node1</b>"]
        end
    end
    subgraph node2["node2"]
        subgraph node2-ovn_cluster_router["ovn_cluster_router"]
            node2-tnl-key["requested-tnl-key=<b>16477216<b>"]
            node2-trtor-GR-node1["trtor-GR-node1<br>requested-tnl-key=<b>4</b><br>requested-chassis=<b>node1</b>"]
            node2-trtor-GR-node2["trtor-GR-node2<br>requested-tnl-key=<b>3</b>"]
            node2-trtor-GR-node3["trtor-GR-node3<br>requested-tnl-key=<b>2</b><br>requested-chassis=<b>node3</b>"]
            node2-rtos-layer2-switch["rtos-layer2-switch<br>gateway_chassis=<b>node2</b>"]
        end
    end
    subgraph node3["node3"]
        subgraph node3-ovn_cluster_router["ovn_cluster_router"]
            node3-tnl-key["requested-tnl-key=<b>16477216<b>"]
            node3-trtor-GR-node1["trtor-GR-node1<br>requested-tnl-key=<b>4</b><br>requested-chassis=<b>node1</b>"]
            node3-trtor-GR-node2["trtor-GR-node2<br>requested-tnl-key=<b>3</b><br>requested-chassis=<b>node2</b>"]
            node3-trtor-GR-node3["trtor-GR-node3<br>requested-tnl-key=<b>2</b>"]
            node3-rtos-layer2-switch["rtos-layer2-switch<br>gateway_chassis=<b>node3</b>"]
        end
    
    end

    node1-trtor-GR-node1 <---> node2-trtor-GR-node1 <---> node3-trtor-GR-node1
    node1-trtor-GR-node2 <---> node2-trtor-GR-node2 <---> node3-trtor-GR-node2
    node1-trtor-GR-node3 <---> node2-trtor-GR-node3 <---> node3-trtor-GR-node3

    class node1-trtor-GR-node1 portStyle;
    class node1-trtor-GR-node2 portStyle;
    class node1-trtor-GR-node3 portStyle;
    class node2-trtor-GR-node1 portStyle;
    class node2-trtor-GR-node2 portStyle;
    class node2-trtor-GR-node3 portStyle;
    class node3-trtor-GR-node1 portStyle;
    class node3-trtor-GR-node2 portStyle;
    class node3-trtor-GR-node3 portStyle; 
    class node1-rtos-layer2-switch portStyle;
    class node2-rtos-layer2-switch portStyle;
    class node3-rtos-layer2-switch portStyle;
    
    class node1-ovn_cluster_router routerStyle;
    class node2-ovn_cluster_router routerStyle;
    class node3-ovn_cluster_router routerStyle;

    class node1-tnl-key termStyle;
    class node2-tnl-key termStyle;
    class node3-tnl-key termStyle;
...
Since these subnet can be shared between layer2 networks they don't need to be qualified by network name, same as layer3 swith port ifaddr.

### Implementation Details

A [PoC PR](https://github.com/ovn-kubernetes/ovn-kubernetes/pull/5067) exist with basic implementation to make all CI pass and as input
for this document.
...
At cluster manager at the zuneCluster controller calculate the peers node subnet
...
- Derive it from the node-id (same we do for transit switch addrs):
  ...
  - bad:
    ...
    - Make the transit peers node subnet dependant on node-id
- Create a new subnet allocator that will be re-sync on restarts and allocate 
  the subnet with it:
  - good:
...
After this topology is configured ovn-kubernetes need to make sure that the new node agnostic gateway are
configured and that the old gateway is gone.

For IPv4 a garp need to be generated with the gateway from ovn_cluster_router this will overwrite 
the mac address from the .1 address at the neighbors cache.

For IPv6 two maint things need to be done
- Send a unsolicited RA to configure the ovn_cluster_router gateway
- Send one unsolicited RA per node with lifetime 0 to remove the gateways from GRs.

#### Upgrade cleanups

The implementation should handle the ports, routes, policies and NAT that is no longer
at the gateway router or layer2 switch and remove it so there is nothing dangling from previous
layer2 topology.
...
* Scale tests to check if adding the transit router affect it
* Upgrade tests is the most important in this case, it should do an upgrade
  while using all the ovn-kuberntes layer2 topology featues and check that they continue working.
...
This topology move the NAT that masquerades the management port traffic from
gateway router to ovn_cluster_router so it may disrupt the readiness probes (api server access)
or name resolution (dns service access), also local gateway traffic may affected.

This topology is going to be use 99% by virtual machines and kubevirt 
do not support readiness probes with UDN so the readiness probe is not an issue, 
