...
routers RAs [Pull Request 4852](https://github.com/ovn-kubernetes/ovn-kubernetes/pull/4852) and reconciling gateways with unsolicited router advertisments
...
OVN routers cannot have multiple ports in the same subnet, so the trtor (trasit router to router) ports of the ovn_cluster_router need to have the minimal possible subnet, to accommodate at least 2 IPs one per peer, from the
...
be a link local address computed from the subnet address `2010:100:200::1`, again this makes the default gw __independent__ of the
...
Also in the case we still need to suport conditional SNAT, the transit router port connected to the switch needs to be configured as gateway router port.
...
After this topology is configured ovn-kubernetes needs to make sure that the new node agnostic gateway is
...
For IPv6 two main things need to be done
...
During the rolling upgrades the nodes are upgrades one after another so cluster
is transitory with some nodes with previous non transit router topology and
others with the new transit router topology.

To prevent traffic disruption at this scenario the remote connection between
nodes has to be kept, following is a possible transitory topology to fix this
issue:
...
not updated nodes using the original switch remote LSPs, since the old nodes
has not transit router yet and after detecting that the node has the new topology
"move" that LSP to the transit router as a LRP pointing to the chassis and
tunnel key.

Another change is that the transit router LRP that connects to the switch has
to keep the join IP since remote switch LSPs will still trigger L2 traffic
towards it, after all the remote nodes has upgrade to transit router this
join IP should be removed since it's already at the gateway router.

Also the mechanism on how to check if remote node is still not updated can be
implemented by checking the transit router subnet annotation, since it should
pop up only if the node is prepare for it.
...
This topology moves the NAT that masquerades the management port traffic from
gateway router to ovn_cluster_router so it may disrupt the readiness probes (api server access)
or name resolution (dns service access), also local gateway traffic may affected, but
take into account that there are plans to move the SNAT from the logical router
to iptables at nodes, so there were no problem, maybe we should wait for that
to happend before implementing this enhancement.

However, this topology is going to be used mainly by virtual machines and kubevirt
...
At local gateway all the egress traffic goes over the management port at the switch, so this gateway mode is going to be the one affected by it,
since SNAT is going to change from gateway router to ovn_cluster_router,
this has to be documented so customers are prepare for it, again this may be
not a problem if before transit router is implemented the SNAT is moved from OVN
to iptables.
