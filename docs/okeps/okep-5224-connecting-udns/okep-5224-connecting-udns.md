...
spec:
  ...
  - cidr: "192.168.0.0/16"  # 65K total IPs
    layer3NetworkPrefix: 24 # 255 IPs per layer3 network = ~100 possible nodes in your cluster
  - cidr: "fd01::/64"
    layer3NetworkPrefix: 96
  connectivityEnabled:
    - PodNetwork
...
* `connectSubnets` field is used to take a configurable subnet from the end
  ...
  and N_L3 networks you should have N_L3×M IPs for allocation in this slice.
  The `layer3NetworkPrefix` contains the prefix length allocated for each
  connected layer3 network. From this pool each node get's a /31 point to
  point network for actual topology connectivity.
  To connect layer2 networks we don't need per node-network slices, we just
  need one `/31` slice per network. Hence we don't need a `layer2NetworkPrefix`.
  It is always going to be `/31`.
  This value is expected to be configured uniquely across multiple UDNConnect
  ...
  APIs must have a unique subnet. If a user accidentally configures overlapping values,
  we must have a status check to block and report this in the status. See
  status validation section for more details.
  This field cannot be mutated to keep things simple in the first
...
// +kubebuilder:validation:XValidation:rule="!has(self.layer3NetworkPrefix) || !isCIDR(self.cidr) || self.layer3NetworkPrefix > cidr(self.cidr).prefixLength()", message="layer3NetworkPrefix must be smaller than CIDR subnet"
// +kubebuilder:validation:XValidation:rule="!has(self.layer3NetworkPrefix) || !isCIDR(self.cidr) || (cidr(self.cidr).ip().family() != 4 || self.layer3NetworkPrefix < 32)", message="layer3NetworkPrefix must < 32 for ipv4 CIDR"
type ConnectSubnet struct {
  ...
  // Layer3NetworkPrefix specifies the prefix length for every connected network.
  // This prefix length should be smaller than the CIDR prefix length.
  //
  // For example, if the CIDR is 10.0.0.0/8 and the layer3NetworkPrefix is 24,
  ...
  // Example; if you plan to deploy 10 nodes, set the layer3NetworkPrefix to /27 (20+ IPs)
  // if you plan to deploy 100 nodes, set the layer3NetworkPrefix to /24 (200+ IPs)
  // if you plan to deploy 1000 nodes, set the layer3NetworkPrefix to /21 (2000+ IPs)
  // if you plan to deploy 5000 nodes, set the layer3NetworkPrefix to /18 (10000+ IPs)
  ...
  // +required
  Layer3NetworkPrefix int32 `json:"layer3NetworkPrefix"`
}
...
1) It is responsible to allocate a range for each network:
  * It get's the `connectSubnets` value from the CNC and allocates
    a `/layer3NetworkPrefix` subnet for each layer3 connected network.
    and `/31` slice subnet for each layer2 connect network.
  * These values are annotated on the `ClusterNetworkConnect`
    ...
    layer3 connected network based on `layer3NetworkPrefix` and each
    ...
    this `layer3NetworkPrefix` can limit the maximum number of nodes in your
    cluster. On the node side from this `layer3NetworkPrefix`, we will give out
