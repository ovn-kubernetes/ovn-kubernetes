...
1) It is responsible to allocate a /31 range for each node-network pair:
  ...
  * These values are annotated on the `ClusterNetworkConnect`
    ...
    and N×1 subnets for Layer2 for N layer2 networks.
2) It is responsible for allocating the global tunnel key for each
  ...
  * It will use the same instance of global `TunnelKeysAllocator`
    ...
    for Layer2 transit switches. It will then allocate the new ones
    for the connect router dynamically. This ensures we don't reuse
    already allocated networkIDs. For each connect API that will
    create a transit router, we will need 1 tunnel key per router.
...
4) Within each `connect-router`, we would need N*M tunnel keys? TBD clarify
   this with dceara.
