...
* `connectSubnets` field is used to take a configurable subnet from the end
  ...
  ClusterNetworkConnect API, each of those ClusterNetworkConnect APIs must
  have a unique subnet. If a user accidentally configures overlapping values,
  we must have a status check to block and report this in the status. See
  status validation section for more details. The default values for the
  connectSubnets are `192.168.0.0/16` and `fd01::/64`.
...
type ClusterNetworkConnectSpec struct {
    ...
    ConnectivityEnabled []FeatureType `json:"connectvityEnabled"`
}
...
const (
    ...
    // Failed indicates that the ClusterNetworkConnect has failed to be applied.
...
* `connectSubnets` must not conflict with:
  ...
  end user for the conflict. Ideally connectSubnets should also
  not conflict with node subnets for choosen by the platform.
  This is the responsibility of the user and we won't provide
  validation for node subnets since fetching that value could
  vary from platform to platform. We will cover this in the docs.
...
* The `Accept` type condition will be done from the cluster-manager
...
**Accept Condition Reasons (from cluster-manager):**
...
* The port's will have IPs allocated from within the `connectSubnets`
  ...
  networks on M nodes we would need 2×(N×M) IPs from the `connectSubnets`.
...
* The port's will have IPs allocated from within the `connectSubnets`
  value provided on the CR (default=`192.168.0.0/10`). For connecting N
  networks on M nodes we would need 2×N IPs from the `connectSubnets`.
  See the controller design section for more details.
...
* Connect the `connect-router-colored-enterprise` to the
  ...
  networks
* The port's will have IPs allocated from within the `connectSubnets`
  value provided on the CR (default=`192.168.0.0/10`). For connecting N_L3
  and N_L2 networks on M nodes we would need 2×(N_L3×M+N_L2) IPs from
  the `connectSubnets`. See the controller design section for more details
...
Route Table <main>:
           ...
           104.104.2.0/24               192.128.0.8 dst-ip
           105.105.0.0/16               192.128.0.2 dst-ip
```
...
action              : pass ---> this need not be pass? maybe allow is ok?
...
from-lport  5000 (ip4.dst==10.96.0.0/16) pass --> tier0
from-lport  4999 (ip4.src==$connected_udn_subnets && ip4.dst==$connected_udn_subnets && ct.new) drop --> tier0
...
from-lport  5000 (ip4.dst==10.96.0.0/16) pass --> tier0
from-lport  4999 (ip4.src==$connected_udn_subnets && ip4.dst==$connected_udn_subnets && ct.new) drop --> tier 0
...
1) It is responsible to allocate a /31 range for each node-network pair:
  ...
  * These values are annotated on the `ClusterNetworkConnect`
    ...
    ```
    We will need N_L3×M subnets for N_L3 Layer3 networks and M nodes
...
This controller also needs to watch for UDNs, CUDNs and Nodes to
accommodate adds, updates and deletes.
...
WIP: Looking for feedback during reviews specially on perf/scale basis
what's the better approach to take

Idea1: Let Network Connect Controller attach the loadbalancers
to connected networks.

No changes required to services controller. Network Connect
controller will depend on the services controller to create
the necessary network load balancers. The Network controller
will need to watch for services and endpointslices and will
add the created load balancers to the other connected networks.
...
logic. So no changes needed here.

Idea2: Let Services Controller attach the loadbalancers
also for other connected networks.

If we need to allow services controller to make changes
for other programming load balancers for connected networks,
we will need to have the information of the connected network
matrix available in the NetInfo cache of services controller
needs to watch for network connect changes. So network connect
controller will need to make the connection information
availabe on the netInfo cache for service controllers.

Idea3: Asynchronous poller syncer from Network Controller

Instead of making services controller watch for CNC
...
in NBDB created by another controller/owner and just sync it
periodically? The problem is a delay in sync.
