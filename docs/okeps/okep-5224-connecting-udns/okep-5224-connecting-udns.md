...
* Connecting and disconnecting UDNs and CUDNs within the same cluster
  ...
  * Support for only `role:Primary` type CUDN, UDN and NAD connections.
    From henceforth all references to CUDN/UDNs implies NADs as well.
* Connecting and disconnecting UDNs owned by the same tenant in a
  self-serve fashion without any admin intervention. A tenant should be able
  to connnect the networks that belong to them.
...
However there is a future enhancement on [multiple primary UDNs] which if
...
* Tenants being able to request for interconnecting their networks
  with other tenants.
    ...
    * Regardless, the tenant API might need more RBAC considerations if we
      get such use cases
* Admins being able to connect `role:Secondary` UDNs/CUDNs/NADs. The
...
* Connecting two UDNs with overlapping subnets won't be supported.
  This is a valid use case scenario but hard to implement. Instead we
  need to perhaps allow these networks to communicate via the exposed
  clusterIPs.
...
which can be used by tenants to isolate their workloads. They can creating
a namespace, create a UDN within that namespace and then any workloads
created inside that namespace will be attached to that UDN which then
becomes the primary network for those workloads. These workloads cannot
talk to workloads attached to other UDNs or default network. Similarly
cluster-scoped ClusterUserDefinedNetworks (CUDNs) CRD can be used by
admins to connect more than 1 namespace as part of the same primary network
and achieve isolation.

When creating these networks with `role:secondary`, it also provides the
ability to achieve multihoming for workloads where a pod can be attached
to more than 1 network.

However there are scenarios where we might want to allow for partial or
full connectivity between the different microservices that are segmented
across the different UDNs. See the use cases section for more details.
This effort tracks the details of implementing connectivity between two
(C)UDNs in a single cluster. The following sections call out some of the
assumptions that will ground the design/implementation.
...
`--enable-connecting-networks` which has to be set to true to get this
feature.
...
to `VirtualPrivateNetworkConnect` and
`AdminVirtualPrivateNetworkConnect`? Or should we keep it as
`NetworkConnect` and `AdminNetworkConnect`?

Dear Reviewers, please vote your choices:

1) VirtualPrivateConnect (VPC) and AdminVirtualPrivateConnect (AVPC)
2) VirtualPrivateConnect (VPC) and ClusterVirtualPrivateConnect (CVPC)
3) VirtualPrivateNetworkConnect (VPNC) and
   AdminVirtualPrivateNetworkConnect (AVPNC)
4) VirtualPrivateNetworkConnect (VPNC) and
   ClusterVirtualPrivateNetworkConnect (CVPNC)
5) NetworkConnect (NC) and AdminNetworkConnect (ANC)
6) NetworkConnect (NC) and ClusterNetworkConnect (CNC)
7) Any other options?
...
`AdminNetworkConnect` CR will be cluster-scoped resource and admin-only
...
Admins can create a `AdminNetworkConnect` CR which signals the
...
spec:
  ...
  featuresEnabled:
    ...
    - ClusterIPServiceNetworkConnectivity
```
...
* If `podNetworkConnectivity` is enabled then admin is asking for full
  pod2pod connectivity across UDNs
* If `ClusterIPServiceNetworkConnectivity` is enabled then admin is asking for
  clusterIPs to be reachable across UDNs.
    * If we don't want to allow ALL clusterIPs to be exposed to other UDNs,
      then on a per-service level in addition to the UDN CR requesting for
      it, we can add a granularity level of "making that service be a
      candidate for UDNConnectivity"
    * If this flag is set but `PodNetworkConnectivity` is not set, then
...
* `connectSubnet` field is used to take a configurable subnet from the end
  ...
  connect API, each of those connect APIs must have a unique subnet. If a
...
Add the CEL validations for the new CRD - waiting for the API fields
to be agreed on
...
* A network could be selected as part of more than one network connnect, so
  ...
  Example, ConnectAPI-A selects blue and green networks and ConnectAPI-B
  selects blue and red networks:
    * In such scenarios the same network's router is going to be connected to
      more than one `vpnc-connect-router`. So blue network will be connected
      to both `vpnc-connect-router-A` and `vpnc-connect-router-B`.
    ...
    * We must add validation in code to ensure if more than 1 connect API
      selects the same network, then they must have non-overlapping
      `connectSubnet` field configured. If there is a conflict, we must emit
      an event on the connectAPI and fail to create the plumbing for the
      subsequent case. [TBD] Ask dceara if OVN will fail hard if I try to
      create two ports with same subnet on a router? If so it might be easy
      to percolate that error back up into an event.
* A connectAPI could be selecting a subset of networks already selected by
  another connectAPI. What is the expectation in this case? Example,
  ConnectAPI-A selects blue, green, red and yellow networks and ConnectAPI-B
  selects blue and green networks.
...
* Select a network can be done as a day0 or day2 operation. So label changes
...
* Create a distributed `colored-enterprise_interconnect-router` (basically
...
* Connect the `colored-enterprise_interconnect-router` to the
  ...
  node using patch ports and tunnelkeys
...
* Create policies on each connected network's `ovn_cluster_router` that
  steers the traffic towards the other networks' subnet to the
  `vpnc-connect-router`.
* On the interconect transit router we will add specific per
...
* Add logical router policies to `blue_blue.network_ovn_cluster_router`
  that steers traffic towards the green UDN's subnet to interconnect
  router on all nodes
* Add logical router policies to `green_green.network_ovn_cluster_router`
  that steers traffic towards the blue UDN's subnet to interconnect router
  on all nodes
* On the `colored-enterprise_interconnect-router` add routes to specific
  node subnet slices of the blue and green ovn_cluster_routers on the
  respective nodes.
...
* Create policies on each connected network's `transit-router` that
  steers the traffic towards the other networks' subnet to the
  `vpnc-connect-router`.
* On the interconect transit router we will add the specific network
  subnet routes that steers the traffic to the ....
...
Case1: `ClusterIPServiceNetworkConnectivity` is not set

no-op. Services connectivity is not supported across connected UDNs.

Case2: `ClusterIPServiceNetworkConnectivity` and `PodNetworkConnectivity` are set

If the `ClusterIPServiceNetworkConnectivity` is explicitly requested, then
...
Case3: `ClusterIPServiceNetworkConnectivity` is set and `PodNetworkConnectivity` is not set
...
Idea2: If `PodNetworkConnectivity` is not set then we don't add
any UDN subnet routes towards `vpnc-connect-router`. Instead, if
`ClusterIPServiceNetworkConnectivity` is set, then we add routes
for each clusterIP that belongs to the other connected networks
to be routed to the `vpnc-connect-router`. We would need to then
either host all load balancers for all connected networks on the
`vpnc-connect-router` OR we would need to route the clusterIP to
...
use for hairpin services that can be leveraged? Can I program the load
balancer such that the masqueradeIP is set and hence in addition to DNAT
we also SNAT to the origin network's masqueradeIP and that takes care of
cross network traffic. Need to ensure the existing masqueradeIP is not
used within/between networks.

### Network Policies and Admin Network Policies

If `PodNetworkConnectivity` is set, then Network Policies
and AdminNetworkPolicies (when they get supported on UDNs)
will that are created in the networks that are part of
connect API will have peers that span across all connect networks.
So the scope of policies will be within all connected networks.

If `PodNetworkConnectivity` is not set, then policies won't
...
* API: https://github.com/ovn-kubernetes/ovn-kubernetes/pull/5540
* Layer3<->Layer3 Connectivity: https://github.com/ovn-kubernetes/ovn-kubernetes/pull/5541
* Layer2<->Layer2 Connectivity:
* Layer3<->Layer2 Connectivity:

## Self Tenant Connect/Disconnect API [WIP]

* Introduct Tenant version of the API `TenantNetworkConnect` that
  allows only setting UDN selector (no CUDNs or NADs)
* [option1] Webhook
* [option2] Use controller logic itself similar to webhook to check
  for `SubjectAccessReview`
* [option3] Well known labels/annotations
* [option4] Owner Reference Validation/Service Accounts

Needs to be backwards compatible to allow for cross-tenant
...
OVN-Kubernetes itself will take care of routing the traffic.
...
* This enhancement depends on the [new Layer2 topology] changes getting
  merged first.
