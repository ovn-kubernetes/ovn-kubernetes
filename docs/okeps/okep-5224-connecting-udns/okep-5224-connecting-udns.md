...
* Connecting and disconnecting UDNs and CUDNs within the same cluster
  ...
  * Support for both `role:Primary` UDN connections.
...
OVN-Kubernetes supports (Cluster)UserDefinedNetworks using which tenants
and admins can isolate their workloads or achieve multi-homing for their
workloads. However there are scenarios where we might want to allow
...
If UDNA is connected to UDNB then that means UDNB is connected to UDNA.
...
**As a cluster admin**, **I want** to connect my newly created UDNNew with an
existing UDNOld **so that** they can communicate with each other as if
...
The admin created UDNOld with a defined subnet CIDR range which gets
exhausted on a 500thday, now the admin cannot mutate UDNOld to change
or expand its subnet CIDR range. Admin creates a new UDNNew
...
**As a cluster admin**, **I want** to allow tenantGreen to be able to access
the clusterIP microservice exposed by tenentBlue.

Example: When separate services owned by different teams need to
communicate via APIs

The admin created UDNBlue and UDNGreen that were isolated initially
...
**As a cluster admin**, **I want** to now connect my existing UDNBlue and UDNGreen
**so that** they can communicate with each other as if being part of the same network.

Example: The admin created UDNBlue and UDNGreen that were isolated
...
TBD collaborate with Girish/Cathy
...
However adding /16 routes will cause assymmetric reply so this can be improved further by
