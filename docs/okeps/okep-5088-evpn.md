...
`<AS Number>:<VNI>`, which is the same format used by FRR itself. `bridge-domain 100` is necessary to indicate to FRR
which netlink vni -> bridge mapping corresponds to this MAC-VRF configuration. The entirety of this configuration section
will enable Type 2 and Type 3 EVPN routes to be advertised.
