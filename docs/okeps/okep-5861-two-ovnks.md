...
* Sharing anything between the instances beyond the local Open vSwitch
  installation: they do not share an OVN stack, a Kubernetes cluster, an
  integration bridge, or a pod network.
...
Both instances share exactly one thing: the local Open vSwitch installation.
Everything else is per instance. Upstream OVN already supports this topology:
...
Story 2: Keep existing deployments untouched

As a cluster admin who runs a single ovn-kubernetes instance per node, I want
this feature to be entirely opt-in, so that my file paths, interface names,
metrics endpoints and OVS resources do not change when I upgrade.

## Proposed Solution

Every instance is given an instance identifier. All state that an instance owns
is derived from that identifier, and each instance runs its own
`ovn-controller` against the shared Open vSwitch, using a unique chassis name
and a dedicated integration bridge.
...
writer of its own databases. Only resources in the shared Open vSwitch, and
names in the shared host namespaces (network interfaces, file paths, TCP ports),
need to be scoped per instance.

### API Details

A single new knob identifies the instance, and everything else is derived from
it:

| Knob | Default | Meaning |
| --- | --- | --- |
| `--instance-id=STRING` | `""` | Short identifier of this instance. Empty selects today's behaviour exactly. |
| `--ovn-chassis-name=STRING` | `NODE-INSTANCE_ID` | Chassis name, and therefore the `external_ids` suffix. Derived from the node name and the instance identifier; settable for deployments that need an explicit chassis name. |
| `--bridge-name=STRING` | `br-int`, or `br-int-INSTANCE_ID` | Integration bridge of this instance. Derived from the instance identifier, so that two instances never share one bridge by accident. |
...
one option that cannot be made chassis-specific.

`--ovn-chassis-name` and `--bridge-name` already exist in the POC. This OKEP
adds `--instance-id` as the primary input: ovn-kubernetes derives the chassis
name and bridge name from it. The existing flags remain optional overrides for
deployments that need explicit values. This lets a deployment configure one
short value instead of separately choosing a chassis name, a bridge name, a set
of directories, an interface name suffix and a set of ports, and enforces the
relationship between those names rather than leaving it to the deployer.
...
This applies to *every* key ovn-kubernetes writes to or reads from the
`Open_vSwitch` table, not only to the five above. The most consequential one is
`ovn-bridge-mappings`, which maps physical network names to local bridges and
therefore decides which bridge each `ovn-controller` patches; it is written
unsuffixed today by ovn-kubernetes, so two instances would overwrite each
other's mappings. OVN
reads it through the chassis-specific lookup, so the suffixed form works.
Beyond that and the encapsulation and bridge configuration, the set currently
includes `ovn-remote-probe-interval`, `ovn-bridge-remote-probe-interval`,
`ovn-is-interconn`, `ovn-monitor-all`, `ovn-ofctrl-wait-before-clear`,
`ovn-enable-lflow-cache`, `ovn-set-local-ip`, and the ovn-kubernetes specific
`ovn-pf-encap-ip-mapping`. Rather than suffixing each call site, the suffix is
applied centrally in the helpers that get and set these keys, so that a key
added later is scoped automatically. Note that `ovn-set-local-ip` was removed in
OVN 24.09, where it became unconditionally true.
...
Each instance is also given a non-overlapping connection tracking zone range,
so that the two controllers cannot dynamically pick the same zone for different
logical ports. This key is chassis-specific as well:
...
Each instance also needs its own gateway bridge, because ovnkube-node owns the
...
at one bridge would erase each other's flows on every synchronization, and no
naming scheme avoids that.

Because a gateway bridge owns a physical uplink, the uplink has to be divided
between the instances rather than shared: separate physical ports, separate
representors or physical functions, or a VLAN per instance on one port. Each
...
* Metrics: `--metrics-bind-address` and `--ovn-metrics-bind-address` must use a
  different port per instance, and exported metrics carry the instance
  identifier as a label so that a scrape can be attributed to an instance.
...
* Unit tests: name generation and validation for every instance-scoped name
  ...
  empty-identifier case that must reproduce today's names exactly.
* E2E tests: the existing `kind-dpu-offload` job already builds the two clusters
  ...
  node, and verify pod-to-pod, pod-to-service and egress traffic in each
  instance independently. This keeps the motivating use case testable without
  DPU hardware, which is a property worth preserving: needing a physical DPU to
  develop this is what makes the existing DPU code paths hard to work on.
* Isolation tests: each instance only sees its own configuration, sockets, PID
  ...
  removed instance is not adopted by the surviving instance.
...
* Scale tests: not applicable beyond confirming that the per-instance resource
  consumption of N instances is N times a single instance, as nothing is shared.
...
* Co-hosting `ovn-controller` instances is documented upstream as highly
  experimental, with the explicit limitation that stateful ACLs are not
  supported. ovn-kubernetes relies on stateful ACLs for network policy, and on
  conntrack for services and egress traffic. Giving each instance its own
  integration bridge and a non-overlapping `ct-zone-range` (OVN 24.09) is
  expected to address the underlying zone conflict, but this must be validated
  by the cross feature tests above before the feature is declared supported,
  and any remaining gap has to be fixed upstream in OVN rather than worked
  around here.
...
* A misconfiguration in which two instances are given the same identifier, or
  in which an instance writes the unsuffixed `external_ids`, would make the two
...
It requires an OVN of at least 24.09 in the node image: co-hosted
`ovn-controller` support arrived in 23.03, and per-chassis conntrack zone
boundaries in 24.09. An older OVN silently ignores the chassis-specific keys and
both instances would fall back to the unsuffixed configuration, so the OVN
version is checked at startup when an instance identifier is set.
