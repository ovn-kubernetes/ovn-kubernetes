...
is set but the required RouterAdvertisements or FrrConfiguration CR is missing,
...
In the future, if there are any routes in the gateway router that are not
imported from BGP and their prefixes overlap with those learned via BGP, we can
add these prefixes to another address set and apply SNAT to traffic routed
...
will be kept. The route import controller will insert a nftables rule for each
...
3. The cluster admin shall create a RouteAdvertisements CR to advertise the
   ...
   FRRConfiguration defined in the last step.
    ...
    spec:
      advertisements:
      - PodNetwork
      # Select the FFRConfiguration defined in step-2 with the custom label.
...
route reflector-based BGP network across the cluster. However, it is not
