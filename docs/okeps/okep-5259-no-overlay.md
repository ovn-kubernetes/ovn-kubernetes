...
* For north-south traffic, we will follow the existing implementation of the BGP
  ...
  pod-to-external traffic, we shall let users to choose whether SNAT shall be
  skipped for the pod egress traffic by adding a new `natOutbound` field to
  the RouteAdvertisements CRD.
...
We introduce a new `layer3.encapsulationConfiguration` field to the Spec of the
...
To give users the control of pod outbound traffic SNAT, we will add a new
`natOutbound` field to the RouteAdvertisements CRD. If it's `true`. The
pod‑to‑external traffic will be SNATed to the node IP before leaving the node.
The field defaults to `false`.

```golang
type RouteAdvertisementsSpec struct {
  ...
  // NatOutbound, when set to true, enables SNAT for pod egress traffic to non-pod subnets. Defaults to false.
  // +optional
  // +kubebuilder:default=false
  NatOutbound bool `json:"natOutbound,omitempty"`
```
...
The routing table of the default network's ovn_cluster_router for node A would
look like:
...
RouteAdvertisements. When enabled, pod egress traffic will be SNATed to the node
IP before leaving the node.

The the `natOutbound` field shall be set to `true` for a deployment where the
user wants a no-overlay network but has not published the pod network to any
external BGP routers. The pod-to-external traffic will be SNATed and forwarded
by a default gateway. However, the pod-to-remote-pod traffic will remain not
SNATed.

##### Shared Gateway Mode

If `natOutbound` is set to `true`, in the gateway router, the default SNAT rule
will be kept for pod-to-external traffic. But pod-to-remote-pod traffic remain
unaffected by SNAT.
...
`10.128.0.0/16` is the CIDR of the pod network. `natOutbound` is set to `true` in
the RA.
...
networks operates in the no-overlay mode either.
