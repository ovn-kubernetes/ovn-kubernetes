...
* **Localnet Type Network** - An OVN-Kubernetes topology rendered into OVN where
  ...
  underlay.
* **BGP-routed Egress Traffic** - The pod egress traffic that is routed via BGP
  routes.
* **Non-BGP-routed Egress Traffic** - The pod egress traffic that is routed via
  non-BGP routes (static, direct, etc.).
...
* This enhancement does not aim to change the existing advertised CUDN/UDN
  isolation mechanism.
...
cluster, but I don't want to advertise the pod network externally and don't
want to deploy an external BGP router for the cluster.
...
* For north-south traffic, we will follow the existing implementation of the BGP
  ...
  externally, this behavior is not ideal. Instead of disabling SNAT for all
  pod-to-external traffic, OVN-Kubernetes shall adjust its SNAT behavior so that
  pod IPs are exposed for traffic routed via BGP, but SNAT is still applied for
  non-BGP-routed egress traffic.

* For east-west traffic:
  * Intra-node traffic remains unchanged.
  * Inter-node traffic will follow the same path as north-south traffic.
    Pod-to-pod, Pod-to-clusterIP traffic will be forwarded by the provider
    network without encapsulation and NAT. Pod-to-node egress traffic will be
    SNATed to the node IP at the source node. This is to allow a pod to access
    nodePort services in a different UDN.
...
We introduce a new `layer3.encapsulationConfiguration` field to be added to the
Spec of the ClusterUserDefinedNetwork (CUDN) CRD. This new field will enable
control over the network encapsulation behavior with the following options:

1. **Encapsulation Configuration**:  
   * `encapsulationConfiguration.protocol`: specifies the encapsulation method
     (default: `Geneve`)  
   * Supported values: `Geneve` (default) or `None` (no-overlay)

```golang
type EncapsulationProtocol string

const (
    EncapsulationProtocolNone   EncapsulationProtocol = "None"
    EncapsulationProtocolGeneve EncapsulationProtocol = "Geneve"
)

// EncapsulationConfiguration defines the encapsulation configuration of a Layer3 network
type EncapsulationConfiguration struct {
    // +kubebuilder:validation:Enum=None;Geneve
    // +kubebuilder:default=Geneve
    Protocol EncapsulationProtocol `json:"protocol"`
}

type Layer3Config struct {
    ...
    // +optional
    EncapConfig EncapsulationConfiguration `json:"encapsulationConfiguration,omitempty"`
}
```
...
The `Layer3Config` is shared between CUDN and UDN. Once we add the new
`encapsulationConfiguration` field, it will be available for both CUDN and UDN.
...
```yaml
    layer3:
      ...
      - message: Setting layer3.encapsulationConfiguration is not supported for UserDefinedNetworks
        rule: '!has(self.encapsulationConfiguration)'
```
...
Here's an example routing table on a UDN's ovn_cluster_router, the pod subnet of
the node is 22.100.1.0/24. The LRSR routes all the pod egress traffic to the
gateway router according to source-based route:

```
IPv4 Routes
Route Table <main>:
                10.96.0.1                22.100.1.2 dst-ip
               10.96.0.10                22.100.1.2 dst-ip
               100.65.0.3                100.65.0.3 dst-ip
            22.100.1.0/24                100.65.0.3 src-ip
            22.100.0.0/16                100.65.0.3 src-ip
```
...
sequenceDiagram
    ...
    participant ovn_cluster_router
    participant OVN Gateway Router (GR)
    participant Underlay Physical Network
    participant Node B

    Pod1 on Node A->>ovn_cluster_router: Packet to remote Pod2 (10.128.2.11)
    Note over ovn_cluster_router: route lookup
    ovn_cluster_router->>ovn_cluster_router: Match the static source-based route
    ovn_cluster_router->>OVN Gateway Router (GR): Forward packet to GR via join switch
    Note over OVN Gateway Router (GR): route lookup
    OVN Gateway Router (GR)->>OVN Gateway Router (GR): Match BGP route: 10.128.2.0/23 via 172.18.0.3 (Node B's IP)
    OVN Gateway Router (GR)->>Underlay Physical Network: Route packet out via breth0
...
sequenceDiagram
    ...
    Underlay Physical Network->>OVN Gateway Router (GR): Route packet in via breth0
    Note over OVN Gateway Router (GR): route lookup
    OVN Gateway Router (GR)->>OVN Gateway Router (GR): Match static route: 10.128.0.0/23 via 100.65.0.1 (Node B's IP)
    OVN Gateway Router (GR)->>ovn_cluster_router: forwarded via the join switch
    Note over ovn_cluster_router: route lookup
    ovn_cluster_router->>ovn_cluster_router: Match the direct route to local pods
    ovn_cluster_router->>Pod1 on Node A: Packet to local Pod1 (10.128.0.5) via the node switch
```
...
sequenceDiagram
    participant Pod1 on Node A
    participant ovn_cluster_router (on Node A)
    participant Host VRF/Kernel (on Node A)
    participant Underlay Physical Network
    participant Node B

    Pod1 on Node A->>ovn_cluster_router (on Node A): Packet to remote Pod2 (10.128.2.11)
    ovn_cluster_router (on Node A)->>Host VRF/Kernel (on Node A): Forward packet to host via ovn-k8s-mp0
    ...
    Host VRF/Kernel (on Node A)->>Underlay Physical Network: Route packet out via breth0
    Underlay Physical Network->>Node B: Packet arrives at Node B
    Note over Node B: Traffic is delivered to Pod2
```
...
sequenceDiagram
    participant Node B
    participant Underlay Physical Network
    participant Host VRF/Kernel
    participant ovn_cluster_router
    participant Pod1 on Node A
    
    Note over Node B: Traffic is delivered from Pod2
    Node B->>Underlay Physical Network: Packet leaves Node B
    Underlay Physical Network->>Host VRF/Kernel: Route packet in via breth0
    Note over Host VRF/Kernel: route lookup
    Host VRF/Kernel->>Host VRF/Kernel: Match static route: 10.128.0.0/23 via 100.65.0.1 (Node B's IP)
    Host VRF/Kernel->>ovn_cluster_router: forwarded via mp0
    Note over ovn_cluster_router: route lookup
    ovn_cluster_router->>ovn_cluster_router: Match the direct route to local pods
    ovn_cluster_router->>Pod1 on Node A: Packet to local Pod1 (10.128.0.5) via the node switch
```
...
field will be introduced to the NAD spec that indicates the encapsulation of the
network. If a network is operating in no-overlay mode, this field will be set to
`none`. We don't support users modifying this field in a NAD manually to enable
no-overlay mode. This information can then be passed into the NetInfo object and
utilized by both the RouteAdvertisement controller and the route import
controller.
...
#### Conditional SNAT for Egress Traffic
...
(like a default internet gateway). So, instead of disabling SNAT for all pod
egress traffic, we shall do the following:

* For egress traffic forwarded via a BGP route, SNAT shall be disabled.
* For egress traffic forwarded via a non-BGP route, the egress traffic shall be
  SNATed to the node IP.

The conditional SNAT also enables a deployment where the user wants a no-overlay
network but has not published the pod network to any external BGP routers. The
pod-to-external traffic will be SNATed and forwarded by a default gateway as
before.

##### Shared Gateway Mode

To maintain SNAT for non-BGP-routed egress traffic, in the gateway router, the
default SNAT rule will be kept. We will add all the BGP route address prefixes
into an address set and use this address set to bypass SNAT for the pod egress
traffic routed to those destinations.
...
# per-pod SNAT rules in the Gateway Router, exclude traffic to BGP routers
...
external_port_range : ""
gateway_port        : []
logical_ip          : "10.128.0.7" # A local pod IP
logical_port        : []
...
will be kept. We will introduce a new controller. It will watch the host route
updates, and insert an nftables rule for each imported BGP route to bypass the
SNAT rule.
...
router can route the traffic correctly. In the strict mode, cross-UDN traffic is
...
currently supported by frr-k8s, which we use for integrating OVN-Kubernetes with FRR.
...
No-overlay mode focuses on the east-west traffic within a cluster. This MEG
feature allows cluster administrators to direct egress traffic from specific
namespaces to one or more external gateways. Function-wise, these two features
...
* API Testing Details
  * ClusterUserDefineNetworks reports expected status conditions
  * No overlay mode cannot be enabled for UserDefineNetworks
  * EgreeIP and EgressService CR reports expected status conditions
...
* Cross-Feature Testing Details - coverage for interaction with other features
  * UDN isolation between UDNs in no-overlay mode
    * Default mode
    * Loose mode
  * UDN isolation between UDNs in no-overlay and overlay mode
    * Default mode
    * Loose mode
  * NetworkPolicy
  * EgressFirewall
...
All the limitations of the BGP integration feature also apply to the
no-overlay feature.

As OVNI is no longer delivering pod-to-pod traffic end-to-end, it will
