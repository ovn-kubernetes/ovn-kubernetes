...
* By default, when BGP advertisement is enabled, the BGP feature disables SNAT
  ...
  `no-overlay-outbound-snat` flag for the default network.
...
For the default network transport, the following new flags will be added to
...
* The `no-overlay-outbound-snat` flag is a string flag to configure the SNAT
...
* The `no-overlay-routing` flag is a string flag to configure
  ...
  * `managed`: OVN-Kubernetes manages the routing for the no-overlay network.
    ...
    be used with `no-overlay-outbound-snat=enable`.
  ...
  * `unmanaged`: Users are responsible for managing the routing, including
    ...
    `no-overlay-outbound-snat=disable`.
...
mode will use, the configuration file will have a new section `[bgp]`, with the
following new flags:
...
* The `topology` flag specifies the BGP topology to use for the cluster internal
  BGP fabric, with supported values of `full-mesh` or `route-reflector`. It's
  optional.
  ...
  If set to `route-reflector`, ovnkube-cluster-manager will
  generate FRRConfigurations for nodes with the
  `k8s.ovn.org/internal-bgp-role=route-reflector` label to act as route
  reflectors, while other nodes are configured as clients. This topology is more
  scalable for large clusters. Support for this feature will be added to the
  FRR-K8s project.
...
[default]
transport = no-overlay
no-overlay-outbound-snat = enable
no-overlay-routing = managed
...
status:
  ...
  - type: TransportAccepted
    status: "True"
    reason: "NoOverlayRouteAdvertisementsAccepted"
    message: "RouteAdvertisements CR blue advertises the pod subnets, and its status is accepted."
```
...
status:
  ...
  - type: TransportAccepted
    status: "True"
    reason: "NoOverlayRouteAdvertisementsAccepted"
    message: "RouteAdvertisements CR my-cudn advertises the pod subnets, and its status is accepted."
```
...
traffic remains unaffected by SNAT. Therefore, the network controllers in
ovnkube-controller will need to add an address set for the pod subnets of each
CUDN and the default network. Then the network controllers will add an exemption
rule with the address set to the SNAT entries. We use the `exempted_ext_ips`
field instead of adding a negative match to the `match` field, as using a
negative match in an OVN SNAT rule is not efficient. Setting the
`exempted_ext_ips` field results in one OpenFlow rule being created for each pod
SNAT entry in the OVS bridge.
...
For the default network, the pod egress traffic will be forwarded to the host
kernel by the ovn-cluster-router directly, then SNATed to the node IP.
...
3. The default network controller in ovnkube-cluster-manager will create a base
   FRRConfiguration CR named `default` in the `frr-k8s-system` namespace, and a
   RouteAdvertisements CR named `default` that advertises the default network.
   The generated FRRConfiguration will configure BGP speakers on each node to
   advertise the pod subnets and set up BGP sessions between nodes.
...
4. The UDN controller in ovnkube-cluster-manager will create a
   RouteAdvertisements CR named `blue` that advertises the blue network.
