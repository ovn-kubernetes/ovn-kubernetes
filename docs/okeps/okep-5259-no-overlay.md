...
* **Primary Network** - The network which is used as the default gateway for the
...
* **Cluster Default Network** - This is the routed OVN network that pods attach
  to by default today as their primary network. The pods default route, service
...
cluster, but I don't want to advertise the pod network to the external and don't
...
* For east-west traffic:
  ...
  * Inter-node traffic will follow the same path as north-south traffic.
    ...
    SNATed to the nodeIP at the source node. This is to allow a pod to access
...
network: its ovn-cluster-router won't create to the transit switch anymore and
...
At the ovn_cluster_router, the pod-to-remote_pod egress traffic will be
...
SNAT rule to the gateway router.
...
API. The condition will be set to `False`, if users try to configure EgressIP
...
API. The condition will be set to `False`, if users try to configure EgressService
for a no-overlay network.
