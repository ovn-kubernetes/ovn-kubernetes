...
* Authors: Peng Liu
* Contributors: Riccardo Ravaioli
...
* For north-south traffic, we will follow the existing implementation of the BGP
  ...
  and the `--default-network-snat-outbound` flag for the default network.
...
For the default network, we will add new flags `--default-network-transport`,
`--default-network-no-overlay-options` and
`--managed-no-overlay-routing-options` to the `ovnkube` binary.

* The `--default-network-transport` flag accepts either `no-overlay` or
  ...
  `--default-network-transport=no-overlay` is set but
  no `RouteAdvertisements` CR is configured for advertising the default network,
  ovnkube-cluster-manager will exit with an error.

* The `--default-network-no-overlay-options` flag accepts a JSON string to
  configure the no-overlay options. It is only used when
  `--default-network-transport=no-overlay`. The JSON string should contain the same
  fields as the `noOverlayOptions` field in the CUDN CRD.

  For example: `--default-network-no-overlay-options='{"outboundSNAT":"Disable",
  "routing":"Unmanaged"}'`.

  If `--default-network-transport=no-overlay` is set but this flag is not provided,
  `outboundSNAT` defaults to `Disable`, and `routing` defaults to `Unmanaged`.

* The `--managed-no-overlay-routing-options` flag is used when no-overlay
  networks are configured with managed routing. It is shared by both the cluster
  default network and CUDNs. It accepts a JSON string to configure the managed
  routing options.

  For example: `--managed-no-overlay-routing-options='{"asNumber":"64514",
  "bgpTopology":"routeReflector"}'`.

  If this flag is not provided, the default AS number is `64512` and the default
  BGP topology is `fullMesh`.
...
* **`transport`**: Specifies the encapsulation method.
  * Supported values: `Geneve` (default) or `NoOverlay`
...
* **`noOverlayOptions`**: Contains configuration for no-overlay mode. This is
  ...
  * `routing`: specifies whether the pod network routing configuration is
    managed by OVN-Kubernetes or users.
    * Supported values: `Managed` or `Unmanaged`(default).
    * `Managed`: OVN-Kubernetes manages the routing of the no-overlay network.
      OVN-Kubernetes will generate the per-node BGP configuration to advertise
      the pod subnets, and set up BGP sessions between nodes.
...
```yaml
    network:
      ...
      - message: "transport 'NoOverlay' is only supported for Layer3 topology"
        rule: 'self.transport != "NoOverlay" || self.topology == "Layer3"'
      - message: "noOverlayOptions is only supported for no-overlay networks"
        rule: 'self.transport == "NoOverlay" || !has(self.noOverlayOptions)'
```

New status conditions will be added to the CUDN CR to indicate whether the
no-overlay routing is correctly configured. Here are the possible conditions:

* **`NoOverlay Routing Configured`**: Indicates whether the no-overlay routing is
  correctly configured.
  * `True`: The no-overlay routing is correctly configured.
  * `False`: The no-overlay routing is not correctly configured.
  * `Unknown`: The status of the no-overlay routing configuration is unknown.
  * This condition is only applicable when `spec.network.transport` is
    `NoOverlay`.

* **`NoOverlay local Topology Ready`**: Indicates whether the no-overlay local
  OVN logical topology is ready.
  * `True`: The no-overlay local topology is ready.
  * `False`: The no-overlay local topology is not ready.
  * `Unknown`: The status of the no-overlay local topology readiness is
    unknown.
  * This condition is only applicable when `spec.network.transport` is
    `NoOverlay`.
...
status:
  conditions:
  - type: NoOverlay Routing Configured
    status: "True"
    message: "RouteAdvertisements CR my-cudn-ra advertises the pod subnets, and its status is accepted."
```
...
status:
  ...
  - type: NoOverlay Routing Configured
    status: "True"
    message: "Per-node FRRConfiguration CRs are created to advertise the pod subnets."
```
...
ZoneInterconnectHandler will not create a connection to the transit switch. As a
result, static routes to remote pod subnets will not be added to the
`ovn_cluster_router`. Instead, traffic will be forwarded using a source-based
route, with the next hop determined by the cluster's gateway mode.
...
advertisement behavior, the routes to remote pod subnets will be imported to
...
according to a source-based route (10.128.0.0/24). The routing table of the
...
no‑overlay mode, this key is set to `"noOverlay"`. Users must not set this
manually; OVN‑Kubernetes manages it. This information can then be passed into
the NetInfo object and utilized by both the RouteAdvertisement controller and
the route import controller.
...
the FRRConfiguration that accepts the routes to pod pod subnets explicitly.
...
the pod IPs, which is often not true for destinations reached via non-BGP
routers (like a default gateway). To give users control over pod egress traffic
SNAT behavior, we introduce a new `outboundSNAT` field to the
ClusterUserDefinedNetwork CRD. When enabled, pod egress traffic will be SNATed
to the node IP before leaving the node.

The `outboundSNAT` field is intended for deployments where a user wants a
no-overlay network but has not advertised the pod network to be routable from
external networks. When enabled, pod-to-external traffic is SNATed and forwarded
by a default gateway, while pod-to-remote-pod traffic is not SNATed regardless
of the value of this field.

##### Shared Gateway Mode

If `outboundSNAT` is enabled for a network, in the gateway router, the default
...
In the following example, `10.128.0.0/24` is the pod subnet in the local node,
`10.128.0.0/16` is the CIDR of the pod network. `outboundSNAT` is enabled in for
the network.
...
4. The cluster admin enables no-overlay mode by running ovn-kubernetes with the
   configuration flags below:
    ...
    ```text
    --default-network-overlay=no-overlay
    ```

5. The route advertisement controller shall generate the per-node
   FRRConfiguration for the frr instances on each node to advertise the pod
   subnets.

6. The ovnkube-cluster-manager will screen the
   `k8s.ovn.org/route-advertisements` annotation in the NAD CR to determine if
   the RouteAdvertisements CR exists for the default network. If it exists,
   ovnkube-cluster-manager will ensure the RouteAdvertisements CR is correctly
   configured to advertise the pod networks and will validate the
   RouteAdvertisements CR status. If the RouteAdvertisements CR is missing or
   its status is not accepted, it shall exit with an error.

#### Enable No-overlay Mode for the Default Network with Managed Routing

1. In the Managed routing mode, OVN-Kubernetes will create the RouteAdvertisements
   CR and the base FRRConfiguration CR automatically. The cluster admin only needs
   to start ovn-kubernetes with the following flags:

   ```text
   --default-network-transport=NoOverlay
   --default-network-no-overlay-options='{"routing":"Managed"}'
   ```

   Optionally, the cluster admin can specify the managed routing options via
   `--managed-no-overlay-routing-options` flag. For example:

   ```text
   --managed-no-overlay-routing-options='{"asNumber":"64514", "bgpTopology":"fullMesh"}'
   ```

   If the bgpTopology is not specified, it defaults to `fullMesh`. If the AS
   number is not specified, it defaults to `64512`.

   If the bgpTopology is set to `routeReflector`, OVN-Kubernetes will configure
   the master nodes as the route reflector, and generate the FRRConfigurations
   for other nodes as clients. This topology is more scalable for large clusters.
   But it relies on the K8S-FRR project to support the route reflector feature in
   the future.

2. ovnkube-cluster-manager will create a base FRRConfiguration CR named
   `default` in the `frr-k8s-system` namespace, and a RouteAdvertisements CR
   named `default` that advertises the default network. The generated
   FRRConfiguration will configure BGP speakers on each node to advertise the
   pod subnets and set up BGP sessions between nodes.

   When nodes are added or removed, ovnkube-cluster-manager shall update the base
   FRRConfiguration CR accordingly, and the route advertisement controller will
   regenerate the per-node FRRConfiguration instances.

   For example, if the cluster has 3 nodes with pod subnets
   `10.128.0.0/24`, `10.128.1.0/24`, and `10.128.2.0/24`. The generated
   RouteAdvertisements CR will look like:
  ...
  The following steps are the same as steps 5 and 6 in the previous section.
...
6. The ovnkube-cluster-manager will screen the
   `k8s.ovn.org/route-advertisements` annotation in the NAD CR to determine if
   the RouteAdvertisements CR exists for the CUDN. If it exists,
   ovnkube-cluster-manager will ensure the RouteAdvertisements CR is correctly
   configured to advertise the pod networks and will validate the
   RouteAdvertisements CR status. If the RouteAdvertisements CR is missing or
   its status is not accepted, it shall update the CUDN status condition
   `NoOverlay Routing Configured` to `False` with an appropriate message.

   For example, if the RouteAdvertisements CR is missing, the condition will
   look like:
    ...
    status:
      ...
      - type: NoOverlay Routing Configured
        status: "False"
        message: "RouteAdvertisements CR blue-ra advertises the pod subnets, but its status is not accepted."
    ```
...
3. ovnkube-cluster-manager will create a base FRRConfiguration CR named
   `blue` in the `frr-k8s-system` namespace, and a RouteAdvertisements CR
   named `blue` that advertises the blue network. The generated FRRConfiguration
   will configure BGP speakers on each node to advertise the pod subnets and set
   up BGP sessions between nodes.

   When nodes are added or removed, ovnkube-cluster-manager shall update the
   base FRRConfiguration CR accordingly, and the route advertisement controller
   will regenerate the per-node FRRConfiguration instances.

   For example, if the cluster has 3 nodes with pod subnets `10.10.1.0/24`,
   `10.10.2.0/24`, and `10.10.3.0/24`, The generated RouteAdvertisements CR will
   look like:
   ...
   And the generated FRRConfiguration CRs will look like:
    ...
    spec:
      bgp:
        routers:
        # Default AS number; can be overridden via --managed-no-overlay-routing-options.
        - asn: 64512
          neighbors:
          - address: <node1-ip>
            disableMP: true
