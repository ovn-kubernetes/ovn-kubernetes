...
backholed or dropped takes a lot of time to solve (unless it's trivial).
...
* Support troubleshooting all features for OVN-Kubernetes
    ...
    * We must form a list of all common ovn-kubernetes bugs we have hit across
      those features (example all possible ways of running into staleSNATs)
...
* Phase3 includes this getting productized and shipped to end-users using
  ...
  So running this stack on a live production environment has been marked as
  non-goal.
...
├─ Live: oc debug node/worker-1 -- ovs-ofctl dump-flows br-int
├─ Live: oc debug node/worker-1 -- ip route show table all
...
can be re-used together with ovn-kubernetes-mcp server for wholistic
end user experience.
...
* **Security Controls**: Enables proper RBAC and read-only access enforcement
...
* `kubectl exec` and `oc debug node` operations require cluster-admin level
  privileges by design
* Alternative approach using custom debug containers
  (`oc debug node --image=<custom-image>`) with volume mounts to database
  files reduces some attack surface but remains intrusive
* **Mitigation**: Strict command allowlisting - expose only read operations
  (`get`, `list`, `describe`, `logs`) while blocking any modification commands
...
For the kubernetes layer, we are going to leverage the opensource
[kubernetes-mcp-server](https://github.com/containers/kubernetes-mcp-server).
So whatever security posture they use for now, can be adopted.
...
3. **Execution Context**: Execute host-level commands via
   `oc debug node/<node_name>` for direct host access
...
* Package the MCP server as a container image that the LLM can select for
  `oc debug node --image=<mcp-server-image>` operations
...
* **Tool Reference**: Available tools and their capabilities across all stack
  layers
* **Security Model**: RWarning around security considerations
...
`kubectl exec` or `oc debug` sessions, without building any specialized
tooling.
