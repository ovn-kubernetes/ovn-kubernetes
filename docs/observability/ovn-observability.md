...
There is a supported set of `Samplig_app` types, for example `acl-new` app is used to sample new connections matched by an ACL.
`Samplig_app.ID` is a way to identify the application that generated the sample.
...
`ovnkube-observ` binary has bad performance and eats a lot of CPU when many packets are sent, this needs more profiling. 
