...
- If the release coordinator is unable to fulfill their responsibilities, they may step down and a new release coordinator will be elected.
...
OVN-Kubernetes is supported until the next major or minor release.

For example, if the current release is `v1.1` it is supported until either `v2.0` is released or `v1.2` is released.

Patch releases will only be created for critical issues that are discovered after the release. In the case a patch release has been created, that release will be supported until the next major or minor release.

For example, if an issue was found in `v1.1.0` it would be fixed in `v1.1.1` and `v1.1.1` would be supported until `v1.2.0` is released. `v1.1.0` would not be supported after `v1.1.1` is released.
...
After release, once the feature has been used for a number of releases, a discussion should be held to determine if the feature should be graduated. All OKEPs are expected to be graduated eventually, although the time between release and graduation may vary.
